...
<title>mycsg SASnR2 0110_Install an R package</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Install an R package</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Base R refers to the core functions and features that come pre-installed with R</li>
<li>While powerful and self-sufficient, Base R can be extended with packages to improve efficiency and readability</li>
<li>An R package is an extension of Base R that provides additional functions to enhance R's capabilities</li>
<li>Think of it as an add-on that simplifies or extends what R can do
<ul>
<li>Functions: Predefined blocks of code that perform specific tasks<br>e.g., filter() in dplyr, ggplot() in ggplot2</li>
<li>Datasets: Built-in datasets that can be used for learning or testing<br>e.g., iris, mtcars</li>
<li>Documentation: Help files and snippets to explain how to use the functions and datasets</li>
<li>DESCRIPTION file: Metadata about the package &mdash; title, version, dependencies, author, etc.</li>
</ul>
</li>
<li>Below is the line of code that helps installing packages into our R environment</li>
</ul>
<p>&nbsp;</p>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-r">install.packages(&quot;tidyverse&quot;)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To install an R package, we call <code>install.packages("packagename")</code>.</li>
<li>This downloads the package from CRAN and saves it to our local R library &mdash; the on-disk equivalent of a SAS install.</li>
<li>We only need to run this once per machine. After install, <code>library(packagename)</code> attaches it to each new session.</li>
<li>The package name goes in quotes. We can install several at once by passing a character vector: <code>install.packages(c("dplyr", "ggplot2"))</code>.</li>
<li>This lesson has no separate tidyverse variant because <code>install.packages()</code> is a base R function &mdash; it is the same call whether we later plan to use tidyverse, data.table, or any other package.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"R (base)","code":"install.packages(\"tidyverse\")","language":"r","html":"<pre><code class=\"language-r\">install.packages(&quot;tidyverse&quot;)<\/code><\/pre>"}] || [];
