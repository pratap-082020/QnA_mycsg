...
<title>mycsg SASnR2 1600_Adding-new-variables</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Adding new variables</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Most analysis steps derive new variables from existing ones &mdash; BMI from height and weight, age bands from age, or a flag from a date comparison.</li>
<li>SAS does this inside a DATA step by assigning to a variable that does not yet exist. The new name appears as a column in the output.</li>
<li>In tidyverse we use <code>mutate()</code>, which adds one or more derived columns in a single call. In base R we assign to <code>df$new_col</code> directly.</li>
<li>This lesson derives BMI (weight in kg divided by height in metres squared) from the CLASS dataset. Height in the input is inches, weight is pounds, so we convert units in the formula too.</li>
<li>After running, the new column BMI holds a numeric BMI value for each subject.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data DM;
...
<li>To derive a new variable in SAS, we assign to it inside a DATA step: <code>BMI = weight / (height**2) * 703;</code>.</li>
<li>Any name that did not exist on the left side of an assignment before becomes a new column in the output dataset &mdash; no explicit declaration needed.</li>
<li>The formula uses the imperial BMI shortcut: weight (lb) divided by height (in) squared, times 703. We could equally convert to metric first and use the textbook formula.</li>
<li>SAS applies the formula to every row as it reads the input, so the computation is vectorised by default.</li>
<li>We could also derive many variables in one DATA step &mdash; just keep adding assignment lines before the <code>run;</code>.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-tidyverse">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-tidyverse" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (tidyverse)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">dm=60 ~ &quot;&gt;= 60 Years&quot;,
   between(age,20,59) ~ &quot;20 - &lt; 60 Years&quot;,
   age &lt; 20 ~ &quot;&lt; 20 Years&quot; )
)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In tidyverse, we add derived columns with <code>mutate()</code>.</li>
<li><code>mutate(class, BMI = Weight / (Height^2) * 703)</code> adds a new column named BMI computed from the existing Weight and Height columns.</li>
<li>The right-hand side is a regular R expression. It is evaluated row-wise because each column is a vector and R applies arithmetic element-wise.</li>
<li>We can add multiple derived columns in one call: <code>mutate(class, BMI = ..., BMI_cat = ifelse(BMI &gt; 25, "High", "Normal"))</code>. The second derivation can even reference the first.</li>
<li>The result is a tibble with the original columns plus each new one appended on the right.</li>
...
<li>In base R, we add a derived column by assigning to <code>df$new_name</code>.</li>
<li><code>class$BMI <- class$Weight / (class$Height^2) * 703</code> creates the BMI column from the existing Weight and Height.</li>
<li>Each reference to <code>class$column</code> returns a vector, and the arithmetic is applied element-wise &mdash; same row-by-row behaviour as SAS and tidyverse.</li>
<li>For multiple derivations, we add one line per new column. Each assignment can reference columns added by earlier lines.</li>
<li>The result modifies the <code>class</code> data frame in place, adding each new column on the right.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data DM;\n infile datalines dlm='|' dsd missover;\ninput subjid : $10. age : best32.;\nlabel ;\n format ;\n datalines4;\n1001|35\n1002|64\n1003|19\n;;;;\n run;\n\ndata dm02;\n     set dm;\n     agemon=age*12;\n    group=1;\nrun;\n\ndata dm02;\n     set dm;\n     length agegr1 $30;\n    if age lt 20 then agegr1=\"<20 Years\";\n    else if 20 le age lt 60 then agegr1=\"20 - <60 Years\";\n    else if age ge 60 then  agegr1=\">= 60 Years\";\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data DM;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput subjid : $10. age : best32.;\nlabel ;\n format ;\n datalines4;\n1001|35\n1002|64\n1003|19\n;;;;\n run;\n\ndata dm02;\n     set dm;\n     agemon=age*12;\n    group=1;\nrun;\n\ndata dm02;\n     set dm;\n     length agegr1 $30;\n    if age lt 20 then agegr1=&quot;&lt;20 Years&quot;;\n    else if 20 le age lt 60 then agegr1=&quot;20 - &lt;60 Years&quot;;\n    else if age ge 60 then  agegr1=&quot;&gt;= 60 Years&quot;;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"dm=60 ~ \">= 60 Years\",\r\n\u00a0 \u00a0between(age,20,59) ~ \"20 - < 60 Years\",\r\n\u00a0 \u00a0age < 20 ~ \"< 20 Years\" )\r\n)","language":"r","html":"<pre><code class=\"language-r\">dm=60 ~ &quot;&gt;= 60 Years&quot;,\r\n\u00a0 \u00a0between(age,20,59) ~ &quot;20 - &lt; 60 Years&quot;,\r\n\u00a0 \u00a0age &lt; 20 ~ &quot;&lt; 20 Years&quot; )\r\n)<\/code><\/pre>"},{"label":"R (base)","code":"dm <- data.frame(\r\n  subjid = c(1001, 1002, 1003),\r\n  age = c(35, 64, 19)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\ndm02 <- dm\r\n\r\ndm02$agemon <- dm02$age * 12\r\n\r\ndm02$group <- 1\r\n\r\ndm02$agegr1 <- ifelse(\r\n  dm02$age >= 60,\r\n  \">= 60 Years\",\r\n  ifelse(dm02$age >= 20 & dm02$age <= 59, \"20 - < 60 Years\", \"< 20 Years\")\r\n)","language":"r","html":"<pre><code class=\"language-r\">dm &lt;- data.frame(\r\n  subjid = c(1001, 1002, 1003),\r\n  age = c(35, 64, 19)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\ndm02 &lt;- dm\r\n\r\ndm02$agemon &lt;- dm02$age * 12\r\n\r\ndm02$group &lt;- 1\r\n\r\ndm02$agegr1 &lt;- ifelse(\r\n  dm02$age &gt;= 60,\r\n  &quot;&gt;= 60 Years&quot;,\r\n  ifelse(dm02$age &gt;= 20 &amp; dm02$age &lt;= 59, &quot;20 - &lt; 60 Years&quot;, &quot;&lt; 20 Years&quot;)\r\n)<\/code><\/pre>"}] || [];
