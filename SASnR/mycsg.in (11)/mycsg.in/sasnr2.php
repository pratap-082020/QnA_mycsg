...
<title>mycsg SASnR2 1700_Appending-datasets</title>
...
<li>To append rows in SAS, the classic approach is a DATA step with multiple datasets on the <code>SET</code> statement: <code>set class class2;</code>.</li>
<li>SAS reads every observation from <code>class</code> first, then every observation from <code>class2</code>, into the output dataset.</li>
<li>Variables with the same name and type are merged automatically. Variables that exist in only one side get missing values for the other side&rsquo;s rows.</li>
<li>An alternative is <code>PROC APPEND base=class data=class2;</code> &mdash; it writes <code>class2</code> onto the end of <code>class</code> in place, without creating a new dataset. Useful for very large data where we do not want to duplicate.</li>
<li>After running, the output has all rows from both inputs and every column from either side.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-tidyverse">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-tidyverse" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (tidyverse)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">males &lt;- tribble( ~Name,~Sex,~Age,~Height,~Weight,
&quot;Alfred&quot;,&quot;M&quot;,14,69,112.5,
&quot;Henry&quot;,&quot;M&quot;,14,63.5,102.5,
&quot;James&quot;,&quot;M&quot;,12,57.3,83,
)
...
females &lt;- tribble( ~Name,~Sex,~Age,~Height,~Weight,
&quot;Alice&quot;,&quot;F&quot;,13,56.5,84,
&quot;Barbara&quot;,&quot;F&quot;,13,65.3,98,
&quot;Carol&quot;,&quot;F&quot;,14,62.8,102.5, 
)

class &lt;- bind_rows(males,females)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In tidyverse, we append rows with <code>bind_rows()</code>.</li>
<li><code>bind_rows(class, class2)</code> returns a new tibble with every row from both inputs stacked top-to-bottom.</li>
<li>Like the SAS DATA step, <code>bind_rows()</code> matches columns by name. Columns that exist in only one side get <code>NA</code> on the other side&rsquo;s rows.</li>
<li>We can supply a named id column to tag where each row came from: <code>bind_rows(study_a = class, study_b = class2, .id = "source")</code> adds a <code>source</code> column with "study_a" or "study_b".</li>
<li>After running, the combined tibble has rows = rows_of_class + rows_of_class2.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">males &lt;- data.frame(
...
females &lt;- data.frame(
  Name = c(&quot;Alice&quot;, &quot;Barbara&quot;, &quot;Carol&quot;),
  Sex = c(&quot;F&quot;, &quot;F&quot;, &quot;F&quot;),
  Age = c(13, 13, 14),
  Height = c(56.5, 65.3, 62.8),
  Weight = c(84, 98, 102.5)
  , stringsAsFactors = FALSE
)

class &lt;- rbind(males, females)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, we append with <code>rbind()</code>: <code>combined <- rbind(class, class2)</code>.</li>
<li><code>rbind()</code> requires that both data frames have exactly the same columns in the same order. If they differ, we get an error &mdash; unlike <code>bind_rows()</code> which fills missing columns with <code>NA</code>.</li>
<li>For a safer append when columns may differ, we can build up via <code>rbind(class, class2[, names(class)])</code> or pre-pad the smaller frame with <code>NA</code> columns.</li>
<li><code>rbind()</code> is simple and fast when inputs are aligned, which is why it remains the idiomatic base R choice.</li>
<li>The result is a data frame with the union of rows from both inputs.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data MALES;\n infile datalines dlm='|' dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\n;;;;\n run;\n\ndata FEMALES;\n infile datalines dlm='|' dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nCarol|F|14|62.8|102.5\n;;;;\n run;\n\ndata class;\n     set males females;\n run;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data MALES;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\n;;;;\n run;\n\ndata FEMALES;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nCarol|F|14|62.8|102.5\n;;;;\n run;\n\ndata class;\n     set males females;\n run;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"males <- tribble( ~Name,~Sex,~Age,~Height,~Weight,\r\n\"Alfred\",\"M\",14,69,112.5,\r\n\"Henry\",\"M\",14,63.5,102.5,\r\n\"James\",\"M\",12,57.3,83,\r\n)\r\n\r\n\u00a0\r\n\r\nfemales <- tribble( ~Name,~Sex,~Age,~Height,~Weight,\r\n\"Alice\",\"F\",13,56.5,84,\r\n\"Barbara\",\"F\",13,65.3,98,\r\n\"Carol\",\"F\",14,62.8,102.5, \r\n)\r\n\r\nclass <- bind_rows(males,females)","language":"r","html":"<pre><code class=\"language-r\">males &lt;- tribble( ~Name,~Sex,~Age,~Height,~Weight,\r\n&quot;Alfred&quot;,&quot;M&quot;,14,69,112.5,\r\n&quot;Henry&quot;,&quot;M&quot;,14,63.5,102.5,\r\n&quot;James&quot;,&quot;M&quot;,12,57.3,83,\r\n)\r\n\r\n\u00a0\r\n\r\nfemales &lt;- tribble( ~Name,~Sex,~Age,~Height,~Weight,\r\n&quot;Alice&quot;,&quot;F&quot;,13,56.5,84,\r\n&quot;Barbara&quot;,&quot;F&quot;,13,65.3,98,\r\n&quot;Carol&quot;,&quot;F&quot;,14,62.8,102.5, \r\n)\r\n\r\nclass &lt;- bind_rows(males,females)<\/code><\/pre>"},{"label":"R (base)","code":"males <- data.frame(\r\n  Name = c(\"Alfred\", \"Henry\", \"James\"),\r\n  Sex = c(\"M\", \"M\", \"M\"),\r\n  Age = c(14, 14, 12),\r\n  Height = c(69, 63.5, 57.3),\r\n  Weight = c(112.5, 102.5, 83)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nfemales <- data.frame(\r\n  Name = c(\"Alice\", \"Barbara\", \"Carol\"),\r\n  Sex = c(\"F\", \"F\", \"F\"),\r\n  Age = c(13, 13, 14),\r\n  Height = c(56.5, 65.3, 62.8),\r\n  Weight = c(84, 98, 102.5)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nclass <- rbind(males, females)","language":"r","html":"<pre><code class=\"language-r\">males &lt;- data.frame(\r\n  Name = c(&quot;Alfred&quot;, &quot;Henry&quot;, &quot;James&quot;),\r\n  Sex = c(&quot;M&quot;, &quot;M&quot;, &quot;M&quot;),\r\n  Age = c(14, 14, 12),\r\n  Height = c(69, 63.5, 57.3),\r\n  Weight = c(112.5, 102.5, 83)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nfemales &lt;- data.frame(\r\n  Name = c(&quot;Alice&quot;, &quot;Barbara&quot;, &quot;Carol&quot;),\r\n  Sex = c(&quot;F&quot;, &quot;F&quot;, &quot;F&quot;),\r\n  Age = c(13, 13, 14),\r\n  Height = c(56.5, 65.3, 62.8),\r\n  Weight = c(84, 98, 102.5)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nclass &lt;- rbind(males, females)<\/code><\/pre>"}] || [];
