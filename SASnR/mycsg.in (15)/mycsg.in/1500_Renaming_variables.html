...
<title>mycsg SASnR2 1865_conditional_logic_case_when_default_missing</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>conditional logic - case_when - .default and missing values</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Many derivations are multi-branch: if X then A, else if Y then B, else C. A typical example is age banding &mdash; under 18, 18 to 59, 60 and over.</li>
<li>SAS handles this with <code>if-else if-else</code> inside a DATA step. Tidyverse offers <code>case_when()</code> from dplyr, which reads top-to-bottom like a truth table. Base R chains <code>ifelse()</code> calls.</li>
<li>We also have to think about missing values &mdash; they can be handled explicitly (a dedicated branch) or default to the final fallback.</li>
<li>This lesson builds an age band using both approaches: one where missing ages fall through to the default branch, and one where they get their own "Age Missing" category.</li>
<li>After running, <code>dm1</code> has four rows with agegrp values derived from the age, and <code>dm2</code> treats the missing age as a separate category.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-r-tidyverse">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-tidyverse" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (tidyverse)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-r">library(dplyr)
...
<li>For multi-branch logic in tidyverse, we reach for <code>case_when()</code> from dplyr.</li>
<li>Each line inside <code>case_when()</code> is a <code>condition ~ value</code> pair; dplyr evaluates them top-to-bottom and returns the value for the first matching condition.</li>
<li>In the first example, a row with <code>AGE = NA</code> does not match <code>AGE &lt; 18</code> or <code>AGE &gt;= 60</code> (NA comparisons return NA, not TRUE). So it falls through to <code>.default</code> and gets "18-59 Years" &mdash; probably not what we want.</li>
<li>In the second example, we add an explicit <code>is.na(AGE) ~ "Age Missing"</code> branch as the first line. Because case_when stops at the first match, missing ages get their own label and never reach the numeric comparisons.</li>
<li><code>.default =</code> replaces the older <code>TRUE ~ value</code> trick. Both still work; <code>.default</code> is clearer.</li>
<li>After running, we check that the missing row has the right label in each output.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r"># Base R equivalent using ifelse and is.na
...
dm$agegrp &lt;- agegrp</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, we emulate <code>case_when()</code> with nested <code>ifelse()</code> calls.</li>
<li>Each <code>ifelse(test, yes, no)</code> handles one condition; the <code>no</code> argument is itself another <code>ifelse()</code> for the next branch. The nesting gets deep fast.</li>
<li>To handle missing values explicitly, we put an <code>is.na(...)</code> check at the outermost layer &mdash; the first test &mdash; so NAs never reach the numeric comparisons.</li>
<li>Reading nested ifelse is harder than reading case_when, but it has zero dependencies and works in any R install.</li>
<li>For readability, we can break the expression across multiple lines, one per branch, as shown in the code above.</li>
<li>Test the result on every category (including NA) to be sure each row landed in the intended branch.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"R (tidyverse)","code":"library(dplyr)\n\n# Sample data\ndm <- tibble(\n  USUBJID = c(\"101-001\", \"101-002\", \"101-003\", \"101-004\"),\n  AGE = c(35, 64, NA, 17)\n)\n\n# .default used when no condition matches\ndm1 <- dm %>%\n  mutate(agegrp = case_when(\n    AGE < 18 ~ \"< 18 Years\",\n    AGE >= 60 ~ \">= 60 Years\",\n    .default = \"18-59 Years\"\n  ))\n\n# Missing values handled separately\ndm2 <- dm %>%\n  mutate(agegrp = case_when(\n    is.na(AGE) ~ \"Age Missing\",\n    AGE < 18 ~ \"< 18 Years\",\n    AGE >= 60 ~ \">= 60 Years\",\n    .default = \"18-59 Years\"\n  ))","language":"r","html":"<pre><code class=\"language-r\">library(dplyr)\n\n# Sample data\ndm &lt;- tibble(\n  USUBJID = c(&quot;101-001&quot;, &quot;101-002&quot;, &quot;101-003&quot;, &quot;101-004&quot;),\n  AGE = c(35, 64, NA, 17)\n)\n\n# .default used when no condition matches\ndm1 &lt;- dm %&gt;%\n  mutate(agegrp = case_when(\n    AGE &lt; 18 ~ &quot;&lt; 18 Years&quot;,\n    AGE &gt;= 60 ~ &quot;&gt;= 60 Years&quot;,\n    .default = &quot;18-59 Years&quot;\n  ))\n\n# Missing values handled separately\ndm2 &lt;- dm %&gt;%\n  mutate(agegrp = case_when(\n    is.na(AGE) ~ &quot;Age Missing&quot;,\n    AGE &lt; 18 ~ &quot;&lt; 18 Years&quot;,\n    AGE &gt;= 60 ~ &quot;&gt;= 60 Years&quot;,\n    .default = &quot;18-59 Years&quot;\n  ))<\/code><\/pre>"},{"label":"R (base)","code":"# Base R equivalent using ifelse and is.na\n\ndm <- data.frame(\n  USUBJID = c(\"101-001\", \"101-002\", \"101-003\", \"101-004\"),\n  AGE = c(35, 64, NA, 17),\n  stringsAsFactors = FALSE\n)\n\nagegrp <- ifelse(\n  is.na(dm$AGE), \"Age Missing\",\n  ifelse(dm$AGE < 18, \"< 18 Years\",\n    ifelse(dm$AGE >= 60, \">= 60 Years\", \"18-59 Years\")\n  )\n)\n\ndm$agegrp <- agegrp","language":"r","html":"<pre><code class=\"language-r\"># Base R equivalent using ifelse and is.na\n\ndm &lt;- data.frame(\n  USUBJID = c(&quot;101-001&quot;, &quot;101-002&quot;, &quot;101-003&quot;, &quot;101-004&quot;),\n  AGE = c(35, 64, NA, 17),\n  stringsAsFactors = FALSE\n)\n\nagegrp &lt;- ifelse(\n  is.na(dm$AGE), &quot;Age Missing&quot;,\n  ifelse(dm$AGE &lt; 18, &quot;&lt; 18 Years&quot;,\n    ifelse(dm$AGE &gt;= 60, &quot;&gt;= 60 Years&quot;, &quot;18-59 Years&quot;)\n  )\n)\n\ndm$agegrp &lt;- agegrp<\/code><\/pre>"}] || [];
