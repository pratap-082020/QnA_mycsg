...
<title>mycsg SASnR2 1870_conditional_logic_if_else</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>conditional logic - if_else</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>A simple two-branch condition &mdash; if X then A, else B &mdash; is the workhorse of derived variables.</li>
<li>SAS uses <code>if cond then var = "A"; else var = "B";</code> inside a DATA step.</li>
<li>Tidyverse offers <code>if_else()</code> from dplyr, which is strict about types. Base R uses <code>ifelse()</code>, which is looser about types but handles NAs differently.</li>
<li>This lesson flags subjects as 60+ or under 60 based on the age variable.</li>
<li>After running, every row has the right label in <code>agegr1</code> based on its age.</li>
...
dm02 &lt;- dm %&gt;%
  mutate(agegr1 = if_else(age &gt;= 60, &quot;&gt;= 60 Years&quot;, &quot;&lt; 60 Years&quot;))</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>For a two-branch condition in tidyverse, we use <code>if_else()</code> from dplyr.</li>
<li><code>if_else(age &gt;= 60, "&gt;= 60 Years", "&lt; 60 Years")</code> returns the first string where the condition is TRUE and the second where it is FALSE.</li>
<li>The function is strict: both sides of the branch must return the same type. That catches subtle bugs early &mdash; e.g. if one branch accidentally returns a number and the other a string.</li>
<li>For NA input, <code>if_else()</code> returns NA unless we supply an optional third argument specifying what to return for missing.</li>
<li>It is the tidyverse preferred alternative to base R <code>ifelse()</code> precisely because of that type strictness.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">dm &lt;- data.frame(
  USUBJID = c(&quot;101-001&quot;, &quot;101-002&quot;, &quot;101-003&quot;, &quot;101-004&quot;),
  age = c(35, 64, 19, 17),
  stringsAsFactors = FALSE
)

dm$agegr1 &lt;- ifelse(dm$age &gt;= 60, &quot;&gt;= 60 Years&quot;, &quot;&lt; 60 Years&quot;)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, we use <code>ifelse()</code> &mdash; the same function name but without the underscore.</li>
<li><code>ifelse(dm$age &gt;= 60, "&gt;= 60 Years", "&lt; 60 Years")</code> returns the first value where the condition is TRUE and the second where FALSE.</li>
<li>Base <code>ifelse()</code> is lenient about types, which is convenient but can silently coerce results. For example, mixing an integer and a character may give an unexpected character output.</li>
<li>For missing input, <code>ifelse()</code> returns NA by default &mdash; same as the tidyverse version.</li>
<li>Assigning the result to <code>dm$agegr1</code> adds the new column to the data frame in place.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"R (tidyverse)","code":"library(dplyr)\n\n# Sample data\ndm <- tibble(\n  USUBJID = c(\"101-001\", \"101-002\", \"101-003\", \"101-004\"),\n  age = c(35, 64, 19, 17)\n)\n\ndm02 <- dm %>%\n  mutate(agegr1 = if_else(age >= 60, \">= 60 Years\", \"< 60 Years\"))","language":"r","html":"<pre><code class=\"language-r\">library(dplyr)\n\n# Sample data\ndm &lt;- tibble(\n  USUBJID = c(&quot;101-001&quot;, &quot;101-002&quot;, &quot;101-003&quot;, &quot;101-004&quot;),\n  age = c(35, 64, 19, 17)\n)\n\ndm02 &lt;- dm %&gt;%\n  mutate(agegr1 = if_else(age &gt;= 60, &quot;&gt;= 60 Years&quot;, &quot;&lt; 60 Years&quot;))<\/code><\/pre>"},{"label":"R (base)","code":"dm <- data.frame(\n  USUBJID = c(\"101-001\", \"101-002\", \"101-003\", \"101-004\"),\n  age = c(35, 64, 19, 17),\n  stringsAsFactors = FALSE\n)\n\ndm$agegr1 <- ifelse(dm$age >= 60, \">= 60 Years\", \"< 60 Years\")","language":"r","html":"<pre><code class=\"language-r\">dm &lt;- data.frame(\n  USUBJID = c(&quot;101-001&quot;, &quot;101-002&quot;, &quot;101-003&quot;, &quot;101-004&quot;),\n  age = c(35, 64, 19, 17),\n  stringsAsFactors = FALSE\n)\n\ndm$agegr1 &lt;- ifelse(dm$age &gt;= 60, &quot;&gt;= 60 Years&quot;, &quot;&lt; 60 Years&quot;)<\/code><\/pre>"}] || [];
