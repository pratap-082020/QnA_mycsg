...
<title>mycsg SASnR2 1875_conditional_logic_if_else_nested</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>conditional logic - if_else - nested conditions</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>When a derivation needs three or more categories &mdash; say, under 20, 20-59, and 60+ &mdash; we can nest if/else calls instead of reaching for case_when.</li>
<li>SAS stacks <code>if/else if/else</code> clauses one after another in a DATA step.</li>
<li>Tidyverse nests <code>if_else()</code> inside the <code>no</code> slot of an outer <code>if_else()</code>. Base R does the same with <code>ifelse()</code>.</li>
<li>For more than 3-4 branches, <code>case_when()</code> (tidyverse) becomes noticeably easier to read.</li>
<li>This lesson bands age into three categories and assigns the label to <code>agegr1</code>.</li>
<li>After running, rows fall into "&gt;= 60 Years", "20 - &lt; 60 Years", or "&lt; 20 Years".</li>
...
<li>For three-branch logic in tidyverse, we can nest <code>if_else()</code> calls &mdash; the "else" value of the outer call is itself another <code>if_else()</code>.</li>
<li>The outer <code>if_else(age &gt;= 60, ..., ...)</code> handles the 60+ case. The inner <code>if_else(between(age, 20, 59), ..., ...)</code> handles the 20-59 and everything-else cases.</li>
<li><code>between(age, 20, 59)</code> is a dplyr helper equivalent to <code>age &gt;= 20 &amp; age &lt;= 59</code> &mdash; a small readability boost for the simple case.</li>
<li>Each branch must return the same type, same as the non-nested <code>if_else()</code>.</li>
<li>For more branches, prefer <code>case_when()</code> &mdash; see the case_when lesson (1865). Nesting gets hard to read past three levels.</li>
...
<li>In base R, nested <code>ifelse()</code> works exactly like the tidyverse version &mdash; we just drop the underscore and use an explicit AND condition for the middle band.</li>
<li><code>ifelse(dm$age &gt;= 60, "&gt;= 60 Years", ifelse(dm$age &gt;= 20 &amp; dm$age &lt;= 59, "20 - &lt; 60 Years", "&lt; 20 Years"))</code>.</li>
<li>We write the middle-band condition explicitly as <code>dm$age &gt;= 20 &amp; dm$age &lt;= 59</code> because base R has no <code>between()</code> helper.</li>
<li>The nested structure is hard to scan past 2-3 levels &mdash; a good motivation to reach for <code>switch()</code> or a vectorised lookup when the branches grow.</li>
<li>After running, we verify each age landed in the expected band.</li>
