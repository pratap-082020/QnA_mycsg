...
<title>mycsg SASnR2 1900_Sorting-observations</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Sorting observations</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Arranging rows into a meaningful order is a common first step before any downstream summary or merge.</li>
<li>Typical reasons: sort by subject and visit, sort descending by date, or order a frequency output by count.</li>
<li>SAS uses <code>proc sort</code> with a <code>by</code> statement; tidyverse uses <code>arrange()</code> from dplyr; base R uses <code>order()</code> on the data frame.</li>
<li>This lesson sorts CLASS by Age ascending, then Height descending, so we can compare the three syntaxes side by side.</li>
<li>After running, the first row should be the youngest subject; within ties on Age, the taller subject should come first.</li>
...
<span style="color:#000000;background-color:#ffffff;"><br></span><span style="color:#89632b;background-color:#ffffc0;">Alfred|M|14|69|112.5<br>Henry|M|14|63.5|102.5<br>James|M|12|57.3|83<br></span><span style="color:#000000;background-color:#ffffff;">;;;;<br></span>
<b><span style="color:#000080;background-color:#ffffff;">run</span></b>;
<span style="color:#000000;background-color:#ffffff;"><br><br></span><b><span style="color:#000080;background-color:#ffffff;">proc</span></b> <b><span style="color:#000080;background-color:#ffffff;">sort</span></b> <span style="color:#0000ff;background-color:#ffffff;">data</span><span style="color:#000000;background-color:#ffffff;">=class </span><span style="color:#0000ff;background-color:#ffffff;">out</span><span style="color:#000000;background-color:#ffffff;">=sorted;<br></span>
<span style="color:#000000;background-color:#ffffff;">    </span><span style="color:#0000ff;background-color:#ffffff;">by</span><span style="color:#000000;background-color:#ffffff;"> age </span><span style="color:#0000ff;background-color:#ffffff;">descending</span><span style="color:#000000;background-color:#ffffff;"> height;<br></span>
<b><span style="color:#000080;background-color:#ffffff;">run</span></b>;
<span style="color:#000000;background-color:#ffffff;"><br><br> </span></div>      </div><div class="variant-description mt-2"><ul>
<li>To sort a dataset in SAS, we call <code>proc sort</code> and name the sort keys in a <code>by</code> statement.</li>
<li><code>proc sort data=class out=sorted;</code> reads from CLASS and writes the sorted result to SORTED &mdash; the input is left untouched.</li>
<li>Without <code>out=</code>, SAS would overwrite CLASS in place, which we usually avoid in production code.</li>
<li><code>by age descending height;</code> sorts first on Age ascending, then breaks ties on Height descending.</li>
<li>The <code>descending</code> keyword goes <em>before</em> the variable it applies to &mdash; it does not carry over to later variables.</li>
<li>After running, open SORTED and check the youngest subject sits on top and ties on Age fall in Height-descending order.</li>
...
sorted&lt;-arrange(class,Age,desc(Height))</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To sort a tibble in tidyverse, we use <code>arrange()</code> from dplyr.</li>
<li><code>arrange(class, Age, desc(Height))</code> sorts by Age ascending, then by Height descending within ties on Age.</li>
<li>Columns default to ascending; wrap any column in <code>desc()</code> to flip it &mdash; the dplyr equivalent of the SAS <code>descending</code> keyword.</li>
<li><code>arrange()</code> returns a new tibble and leaves the original data frame untouched, similar to using <code>out=</code> in SAS.</li>
<li>Missing values go to the bottom regardless of sort direction &mdash; see lesson 1901 for explicit control over NA placement.</li>
<li>We run and confirm the row count is unchanged and the sort keys move in the expected directions.</li>
...
sorted &lt;- class[order(class$Age, -class$Height), ]</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, sorting is a two-step idea: <code>order()</code> returns a vector of row indices in sorted order, and we index the data frame with those indices.</li>
<li><code>class[order(class$Age, -class$Height), ]</code> reads: order rows by Age ascending, then by Height descending.</li>
<li>Prefixing a numeric column with <code>-</code> flips it to descending &mdash; this works because sorting negative numbers reverses the direction.</li>
<li>For character columns, <code>-col</code> does not work; use <code>order(..., decreasing = TRUE)</code> or <code>order(-xtfrm(col))</code> instead.</li>
<li>The trailing comma inside the brackets keeps all columns; drop or rearrange column names there if we want fewer columns in the output.</li>
<li>We run and check that the row count matches CLASS and the sort direction is correct.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"\ndata\u00a0CLASS;\ninfile\u00a0datalines\u00a0dlm='|'\u00a0dsd\u00a0missover;\ninput\u00a0Name\u00a0:\u00a0$8.\u00a0Sex\u00a0:\u00a0$1.\u00a0Age\u00a0:\u00a0best32.\u00a0Height\u00a0:\u00a0best32.\u00a0Weight\u00a0:\u00a0best32.;\nlabel\u00a0;\nformat\u00a0;\ndatalines4;\nAlfred|M|14|69|112.5Henry|M|14|63.5|102.5James|M|12|57.3|83;;;;\nrun;\nproc\u00a0sort\u00a0data=class\u00a0out=sorted;\n\u00a0\u00a0\u00a0\u00a0by\u00a0age\u00a0descending\u00a0height;\nrun;\n\u00a0","language":"sas","html":"<div style=\"font-family: Courier New; font-size: 13pt\">\n<b><span style=\"color:#000080;background-color:#ffffff;\">data<\/span><\/b><span style=\"color:#000000;background-color:#ffffff;\">\u00a0CLASS;<br><\/span>\n<span style=\"color:#0000ff;background-color:#ffffff;\">infile<\/span>\u00a0<span style=\"color:#0000ff;background-color:#ffffff;\">datalines<\/span>\u00a0<span style=\"color:#0000ff;background-color:#ffffff;\">dlm<\/span><span style=\"color:#000000;background-color:#ffffff;\">=<\/span><span style=\"color:#800080;background-color:#ffffff;\">'|'<\/span>\u00a0<span style=\"color:#0000ff;background-color:#ffffff;\">dsd<\/span>\u00a0<span style=\"color:#0000ff;background-color:#ffffff;\">missover<\/span>;\n<span style=\"color:#000000;background-color:#ffffff;\"><br><\/span><span style=\"color:#0000ff;background-color:#ffffff;\">input<\/span><span style=\"color:#000000;background-color:#ffffff;\">\u00a0Name\u00a0:\u00a0<\/span><span style=\"color:#08726d;background-color:#ffffff;\">$8.<\/span><span style=\"color:#000000;background-color:#ffffff;\">\u00a0Sex\u00a0:\u00a0<\/span><span style=\"color:#08726d;background-color:#ffffff;\">$1.<\/span><span style=\"color:#000000;background-color:#ffffff;\">\u00a0Age\u00a0:\u00a0<\/span><span style=\"color:#08726d;background-color:#ffffff;\">best32.<\/span><span style=\"color:#000000;background-color:#ffffff;\">\u00a0Height\u00a0:\u00a0<\/span><span style=\"color:#08726d;background-color:#ffffff;\">best32.<\/span><span style=\"color:#000000;background-color:#ffffff;\">\u00a0Weight\u00a0:\u00a0<\/span><span style=\"color:#08726d;background-color:#ffffff;\">best32.<\/span>;\n<span style=\"color:#000000;background-color:#ffffff;\"><br><\/span><span style=\"color:#0000ff;background-color:#ffffff;\">label<\/span><span style=\"color:#000000;background-color:#ffffff;\">\u00a0;<br><\/span>\n<span style=\"color:#0000ff;background-color:#ffffff;\">format<\/span><span style=\"color:#000000;background-color:#ffffff;\">\u00a0;<br><\/span>\n<span style=\"color:#0000ff;background-color:#ffffff;\">datalines4<\/span>;\n<span style=\"color:#000000;background-color:#ffffff;\"><br><\/span><span style=\"color:#89632b;background-color:#ffffc0;\">Alfred|M|14|69|112.5<br>Henry|M|14|63.5|102.5<br>James|M|12|57.3|83<br><\/span><span style=\"color:#000000;background-color:#ffffff;\">;;;;<br><\/span>\n<b><span style=\"color:#000080;background-color:#ffffff;\">run<\/span><\/b>;\n<span style=\"color:#000000;background-color:#ffffff;\"><br><br><\/span><b><span style=\"color:#000080;background-color:#ffffff;\">proc<\/span><\/b>\u00a0<b><span style=\"color:#000080;background-color:#ffffff;\">sort<\/span><\/b>\u00a0<span style=\"color:#0000ff;background-color:#ffffff;\">data<\/span><span style=\"color:#000000;background-color:#ffffff;\">=class\u00a0<\/span><span style=\"color:#0000ff;background-color:#ffffff;\">out<\/span><span style=\"color:#000000;background-color:#ffffff;\">=sorted;<br><\/span>\n<span style=\"color:#000000;background-color:#ffffff;\">\u00a0\u00a0\u00a0\u00a0<\/span><span style=\"color:#0000ff;background-color:#ffffff;\">by<\/span><span style=\"color:#000000;background-color:#ffffff;\">\u00a0age\u00a0<\/span><span style=\"color:#0000ff;background-color:#ffffff;\">descending<\/span><span style=\"color:#000000;background-color:#ffffff;\">\u00a0height;<br><\/span>\n<b><span style=\"color:#000080;background-color:#ffffff;\">run<\/span><\/b>;\n<span style=\"color:#000000;background-color:#ffffff;\"><br><br>\u00a0<\/span><\/div>"},{"label":"R (tidyverse)","code":"library(tidyverse)\n\nclass<-tribble(\n~Name,~Sex,~Age,~Height,~Weight,\n\"Alfred\",\"M\",14,69,112.5,\n\"Henry\",\"M\",14,63.5,102.5,\n\"James\",\"M\",12,57.3,83,\n)\n\nsorted<-arrange(class,Age,desc(Height))","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\n\nclass&lt;-tribble(\n~Name,~Sex,~Age,~Height,~Weight,\n&quot;Alfred&quot;,&quot;M&quot;,14,69,112.5,\n&quot;Henry&quot;,&quot;M&quot;,14,63.5,102.5,\n&quot;James&quot;,&quot;M&quot;,12,57.3,83,\n)\n\nsorted&lt;-arrange(class,Age,desc(Height))<\/code><\/pre>"},{"label":"R (base)","code":"class <- data.frame(\n  Name = c(\"Alfred\", \"Henry\", \"James\"),\n  Sex = c(\"M\", \"M\", \"M\"),\n  Age = c(14, 14, 12),\n  Height = c(69, 63.5, 57.3),\n  Weight = c(112.5, 102.5, 83)\n  , stringsAsFactors = FALSE\n)\n\nsorted <- class[order(class$Age, -class$Height), ]","language":"r","html":"<pre><code class=\"language-r\">class &lt;- data.frame(\n  Name = c(&quot;Alfred&quot;, &quot;Henry&quot;, &quot;James&quot;),\n  Sex = c(&quot;M&quot;, &quot;M&quot;, &quot;M&quot;),\n  Age = c(14, 14, 12),\n  Height = c(69, 63.5, 57.3),\n  Weight = c(112.5, 102.5, 83)\n  , stringsAsFactors = FALSE\n)\n\nsorted &lt;- class[order(class$Age, -class$Height), ]<\/code><\/pre>"}] || [];
