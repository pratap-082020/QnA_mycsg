...
<title>mycsg SASnR2 1901_Sort_Order_With_Missing_Values</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Sort Order with Missing Values</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Sorting becomes trickier the moment missing values enter the picture &mdash; do NAs belong at the top, at the bottom, or should they be dropped?</li>
<li>SAS PROC SORT places missing values first in ascending order and last in descending order.</li>
<li>Tidyverse <code>arrange()</code> always pushes NAs to the bottom by default, regardless of sort direction, so we steer them explicitly.</li>
<li>Base R <code>order()</code> drops NAs by default; we can change that with the <code>na.last</code> argument or a small trick on missingness.</li>
<li>This lesson sorts a small dataset where two rows have a missing score, so the placement of the NAs is easy to see in each language.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data sort_missing;
    input id $ score;
datalines;
A 85
B .
...
<li>In SAS, missing values are treated as smaller than any number in ascending order.</li>
<li><code>proc sort data=sort_missing out=sorted; by score;</code> sorts on score ascending and writes the sorted rows to SORTED.</li>
<li>Because missing is the smallest value, the two rows with <code>.</code> come out first, followed by the numeric scores in increasing order.</li>
<li>For descending sort, <code>by descending score;</code> flips the order and the missing-value rows drop to the bottom.</li>
<li>This behaviour is built into PROC SORT &mdash; we never have to code a special rule to get NAs first.</li>
<li>After running, open SORTED and confirm the two missing rows sit at the top.</li>
...
data_sorted &lt;- data %&gt;%
   arrange(!is.na(score), score)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>By default, <code>arrange()</code> from dplyr pushes NAs to the bottom regardless of sort direction &mdash; the opposite of the SAS ascending default.</li>
<li><code>arrange(!is.na(score), score)</code> is a two-key trick: sort first on a logical flag, then on score itself.</li>
<li><code>!is.na(score)</code> is FALSE for NA rows and TRUE for non-NA rows &mdash; FALSE sorts before TRUE, so NAs land on top.</li>
<li>For the opposite placement, use <code>arrange(is.na(score), desc(score))</code> to keep NAs at the bottom while sorting values descending.</li>
<li>Both forms are explicit, which makes the intent obvious to the next person reading the code.</li>
<li>We run and verify the two NA rows are where we expect them to be.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r"># Example dataset
...
data_sorted &lt;- data[order(!is.na(data$score), data$score), ]</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, we use the same missingness-first trick inside <code>order()</code>.</li>
<li><code>data[order(!is.na(data$score), data$score), ]</code> sorts by the NA flag first, putting NAs on top, then sorts by score within each group.</li>
<li>The bracket expression uses the row indices returned by <code>order()</code> &mdash; the trailing comma keeps all columns.</li>
<li>We can flip it to <code>order(is.na(data$score), data$score)</code> to send NAs to the bottom instead.</li>
<li>A cleaner one-line alternative: <code>order(data$score, na.last = FALSE)</code> for NAs first, or <code>na.last = TRUE</code> for NAs last.</li>
<li>We run and check both NA rows land where we intended &mdash; top or bottom, depending on the form we used.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data sort_missing;\n    input id $ score;\ndatalines;\nA 85\nB .\nC 90\nD 70\nE .\nF 95\nG 80\n;\n\nrun;\n\nproc sort data=sort_missing out=sorted;\n    by score;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data sort_missing;\n    input id $ score;\ndatalines;\nA 85\nB .\nC 90\nD 70\nE .\nF 95\nG 80\n;\n\nrun;\n\nproc sort data=sort_missing out=sorted;\n    by score;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\r\n\r\n\r\ndata <- tibble( \r\nid = c(\"A\", \"B\", \"C\", \"D\", \"E\", \"F\", \"G\"), \r\nscore = c(85, NA, 90, 70, NA, 95, 80) \r\n)\r\n\r\n\r\ndata_sorted <- data %>%\r\n   arrange(!is.na(score), score)","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\r\n\r\n\r\ndata &lt;- tibble( \r\nid = c(&quot;A&quot;, &quot;B&quot;, &quot;C&quot;, &quot;D&quot;, &quot;E&quot;, &quot;F&quot;, &quot;G&quot;), \r\nscore = c(85, NA, 90, 70, NA, 95, 80) \r\n)\r\n\r\n\r\ndata_sorted &lt;- data %&gt;%\r\n   arrange(!is.na(score), score)<\/code><\/pre>"},{"label":"R (base)","code":"# Example dataset\r\n\r\ndata <- data.frame(\r\n  id = c(\"A\", \"B\", \"C\", \"D\", \"E\", \"F\", \"G\"),\r\n  score = c(85, NA, 90, 70, NA, 95, 80)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\ndata_sorted <- data[order(!is.na(data$score), data$score), ]","language":"r","html":"<pre><code class=\"language-r\"># Example dataset\r\n\r\ndata &lt;- data.frame(\r\n  id = c(&quot;A&quot;, &quot;B&quot;, &quot;C&quot;, &quot;D&quot;, &quot;E&quot;, &quot;F&quot;, &quot;G&quot;),\r\n  score = c(85, NA, 90, 70, NA, 95, 80)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\ndata_sorted &lt;- data[order(!is.na(data$score), data$score), ]<\/code><\/pre>"}] || [];
