...
<title>mycsg SASnR2 2100_Descriptive-statistics-for-numeric-variables</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Descriptive statistics for numeric variables</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Descriptive statistics &mdash; n, mean, standard deviation, min, max &mdash; are the everyday summary of any numeric variable.</li>
<li>Typical requests: height summary across all subjects, then the same summary broken down by sex or by treatment arm.</li>
<li>SAS offers <code>proc summary</code> (or the closely related <code>proc means</code>) to compute these on the full dataset or within class levels.</li>
<li>Tidyverse uses <code>summarize()</code> with <code>group_by()</code> for the grouped form &mdash; a compact, pipe-friendly pattern.</li>
<li>Base R uses <code>aggregate()</code> with a small custom function to return several statistics at once, plus an unpacking step.</li>
<li>This lesson computes n, mean, and sd for Height on CLASS, first overall and then split by Sex.</li>
...
<li><code>proc summary data=class;</code> runs on CLASS; <code>var height;</code> names the analysis variable.</li>
<li><code>output out=stats01 n=n mean=mean std=sd;</code> writes one row with columns named <code>n</code>, <code>mean</code>, and <code>sd</code>.</li>
<li>For a grouped run, we add <code>class sex;</code> plus the <code>nway</code> option.</li>
<li><code>nway</code> restricts the output to the finest-class combinations; without it, SAS would also add overall-total rows we do not want here.</li>
<li>After running, STATS01 should have one overall summary row and STATS02 should have one row per Sex level.</li>
<li>Open both datasets and confirm the n for STATS02 adds up to the n in STATS01.</li>
...
<li>In tidyverse, <code>summarize()</code> reduces a tibble to a single row of statistics; combined with <code>group_by()</code>, it produces one summary row per group.</li>
<li><code>summarize(class, n = n(), mean = mean(Height), sd = sd(Height))</code> gives the overall summary &mdash; the equivalent of the ungrouped <code>proc summary</code> call.</li>
<li><code>n()</code> is the dplyr helper that counts rows in the current group; <code>mean()</code> and <code>sd()</code> come from base R and work on the tibble column transparently.</li>
<li>For the grouped form, pipe the data through <code>group_by(Sex)</code> first, then call <code>summarize()</code> &mdash; we get one row per Sex level.</li>
<li>The result is a tibble, ready to pipe into a plot, a join, or a report without an explicit <code>out=</code>.</li>
<li>We run and compare the tibble rows against SAS STATS01 and STATS02 &mdash; they should match row for row.</li>
...
names(stats02) &lt;- c(&quot;Sex&quot;, &quot;n&quot;, &quot;mean&quot;, &quot;sd&quot;)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, the overall summary is assembled directly with a one-row <code>data.frame</code>: <code>data.frame(n = length(class$Height), mean = mean(class$Height), sd = sd(class$Height))</code>.</li>
<li>For the grouped version, <code>aggregate(Height ~ Sex, data = class, FUN = function(x) c(n = length(x), mean = mean(x), sd = sd(x)))</code> runs the custom function once per Sex level.</li>
<li><code>aggregate()</code> returns multiple statistics as a matrix tucked inside a single column &mdash; it looks like separate columns in the RStudio viewer but structurally it is one.</li>
<li><code>do.call(data.frame, stats02)</code> unpacks that matrix into real, independent columns we can rename and work with normally.</li>
<li><code>names(stats02) &lt;- c("Sex", "n", "mean", "sd")</code> gives the columns analyst-friendly labels.</li>
<li>The extra unpacking step is the main ergonomic difference from the tidyverse form &mdash; a good reason to reach for dplyr when the same analysis repeats often.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data CLASS;\n infile datalines dlm='|' dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\n;;;;\n run;\n\nproc summary data=class;\n var height     ;\n output out=stats01 n=n mean=mean std=sd;\n run;\n\nproc summary data=class nway;\nclass sex    ;\n var height     ;\n output out=stats02 n=n mean=mean std=sd;\n run;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data CLASS;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\n;;;;\n run;\n\nproc summary data=class;\n var height     ;\n output out=stats01 n=n mean=mean std=sd;\n run;\n\nproc summary data=class nway;\nclass sex    ;\n var height     ;\n output out=stats02 n=n mean=mean std=sd;\n run;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"class<-tribble( \r\n~Name,~Sex,~Age,~Height,~Weight, \r\n\"Alfred\",\"M\",14,69,112.5, \r\n\"Alice\",\"F\",13,56.5,84, \r\n\"Barbara\",\"F\",13,65.3,98, \r\n\"Henry\",\"M\",14,63.5,102.5, \r\n\"James\",\"M\",12,57.3,83, ) \r\n\r\nstats01<-summarize(class,\r\nn=n( ), \r\nmean=mean(Height),\r\nsd=sd(Height)) \r\n\r\nstats02<-class %>% \r\ngroup_by(Sex) %>% \r\nsummarize(\r\nn=n( ), \r\nmean=mean(Height),\r\nsd=sd(Height)\r\n)","language":"r","html":"<pre><code class=\"language-r\">class&lt;-tribble( \r\n~Name,~Sex,~Age,~Height,~Weight, \r\n&quot;Alfred&quot;,&quot;M&quot;,14,69,112.5, \r\n&quot;Alice&quot;,&quot;F&quot;,13,56.5,84, \r\n&quot;Barbara&quot;,&quot;F&quot;,13,65.3,98, \r\n&quot;Henry&quot;,&quot;M&quot;,14,63.5,102.5, \r\n&quot;James&quot;,&quot;M&quot;,12,57.3,83, ) \r\n\r\nstats01&lt;-summarize(class,\r\nn=n( ), \r\nmean=mean(Height),\r\nsd=sd(Height)) \r\n\r\nstats02&lt;-class %&gt;% \r\ngroup_by(Sex) %&gt;% \r\nsummarize(\r\nn=n( ), \r\nmean=mean(Height),\r\nsd=sd(Height)\r\n)<\/code><\/pre>"},{"label":"R (base)","code":"class <- data.frame(\r\n  Name = c(\"Alfred\", \"Alice\", \"Barbara\", \"Henry\", \"James\"),\r\n  Sex = c(\"M\", \"F\", \"F\", \"M\", \"M\"),\r\n  Age = c(14, 13, 13, 14, 12),\r\n  Height = c(69, 56.5, 65.3, 63.5, 57.3),\r\n  Weight = c(112.5, 84, 98, 102.5, 83)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nstats01 <- data.frame(\r\n  n = length(class$Height),\r\n  mean = mean(class$Height),\r\n  sd = sd(class$Height)\r\n)\r\n\r\nstats02 <- aggregate(Height ~ Sex, data = class, FUN = function(x) {\r\n  c(n = length(x), mean = mean(x), sd = sd(x))\r\n})\r\n\r\nstats02 <- do.call(data.frame, stats02)\r\nnames(stats02) <- c(\"Sex\", \"n\", \"mean\", \"sd\")","language":"r","html":"<pre><code class=\"language-r\">class &lt;- data.frame(\r\n  Name = c(&quot;Alfred&quot;, &quot;Alice&quot;, &quot;Barbara&quot;, &quot;Henry&quot;, &quot;James&quot;),\r\n  Sex = c(&quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;M&quot;),\r\n  Age = c(14, 13, 13, 14, 12),\r\n  Height = c(69, 56.5, 65.3, 63.5, 57.3),\r\n  Weight = c(112.5, 84, 98, 102.5, 83)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nstats01 &lt;- data.frame(\r\n  n = length(class$Height),\r\n  mean = mean(class$Height),\r\n  sd = sd(class$Height)\r\n)\r\n\r\nstats02 &lt;- aggregate(Height ~ Sex, data = class, FUN = function(x) {\r\n  c(n = length(x), mean = mean(x), sd = sd(x))\r\n})\r\n\r\nstats02 &lt;- do.call(data.frame, stats02)\r\nnames(stats02) &lt;- c(&quot;Sex&quot;, &quot;n&quot;, &quot;mean&quot;, &quot;sd&quot;)<\/code><\/pre>"}] || [];
