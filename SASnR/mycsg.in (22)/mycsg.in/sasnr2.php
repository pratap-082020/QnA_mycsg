...
<title>mycsg SASnR2 2110_summary_stats_by_treatment</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Summarize Age by Treatment Group with Descriptive Statistics</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Clinical summary tables almost always need descriptive statistics for each treatment group plus a Total column covering all treatments combined.</li>
<li>Standard request: n, nmiss, mean, sd, min, Q1, median, Q3, max &mdash; per treatment level and overall.</li>
<li>In SAS we lean on <code>proc summary</code> with a <code>class</code> statement, <code>preloadfmt</code>, and <code>completetypes</code> so every planned level appears in the output even when it has zero rows.</li>
<li>In tidyverse we <code>group_by()</code> the treatment and call <code>summarize()</code>, duplicating records in advance to add the Total band.</li>
<li>In base R we use <code>aggregate()</code> with a custom function, then unpack the matrix column and rename the results.</li>
<li>This lesson summarizes Age for the flagged analysis population (<code>fasfl = "Y"</code>) and shows the pattern in all three languages.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data adsl;
...
<li><code>data adsl02;</code> filters to the flagged rows (<code>where fasfl = "Y"</code>) and duplicates each record &mdash; once with its real treatment, once with <code>treatment = 4</code> (the Total band).</li>
<li><code>proc format; value treatment 1=1 2=2 3=3 4=4;</code> declares that 1-4 are the valid treatment codes; this matters because we ask PROC SUMMARY to use the format later.</li>
<li><code>proc summary data=adsl02 nway completetypes; class treatment / preloadfmt;</code> tells SAS to include every formatted level in the output, even ones with no rows.</li>
<li><code>var age;</code> names the analysis variable; the <code>output</code> statement lists the statistics we want.</li>
<li><code>output out=stats01 n= nmiss= mean= std= min= q1= median= q3= max= / autoname;</code> writes one statistic per column with auto-generated names like <code>age_n</code>, <code>age_mean</code>.</li>
<li>After running, STATS01 has one row per treatment band &mdash; 1, 3, and 4 (Total) in this sample; adjust <code>adsl02</code> or the format if other levels appear in the production data.</li>
...
<li><code>adsl</code> is built with <code>tribble()</code> so the sample data reads like a table right in the code.</li>
<li><code>group_by(trt01pn, trt01p)</code> sets each treatment level as its own group; grouping on both the numeric code and the label keeps both in the final tibble.</li>
<li><code>summarize()</code> computes the full set in one call: <code>nrecs</code>, <code>nmiss</code>, <code>n</code>, <code>mean</code>, <code>stddev</code>, and the five-number summary.</li>
<li><code>quantile(age, 0.25, type = 2, na.rm = TRUE)</code> uses <code>type = 2</code> because that matches the SAS default definition of Q1 and Q3 &mdash; the R default (<code>type = 7</code>) would give slightly different values.</li>
<li><code>na.rm = TRUE</code> lets <code>mean</code>, <code>sd</code>, <code>min</code>, and <code>max</code> skip missing values rather than returning NA for the whole group.</li>
<li>We run and compare row-for-row against SAS STATS01 &mdash; numbers should match when the same type 2 quantile definition is in play.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">adsl &lt;- data.frame(
...
<li>Base R <code>aggregate(age ~ trt01pn + trt01p, data = adsl, FUN = function(x) {...})</code> runs the custom function once per treatment combination &mdash; the base R analogue of <code>group_by()</code> + <code>summarize()</code>.</li>
<li>Inside the function we assign intermediate names prefixed with <code>tmp_</code> to make clear these are not the final report columns.</li>
<li>All statistics come back together as a single named numeric vector, which <code>aggregate()</code> stores as a matrix tucked inside one result column.</li>
<li><code>do.call(data.frame, stats01)</code> unpacks that matrix into real, independent columns we can rename.</li>
<li><code>aggregate()</code> prefixes every summary column with the analysis-variable name (<code>age.</code>); we strip the prefix with <code>sub("^age\\.", "", names(stats01))</code>.</li>
<li>A small named-lookup vector (<code>final_map</code>) translates <code>tmp_</code> names into the tidyverse-matching column names like <code>nrecs</code>, <code>mean</code>, <code>stddev</code>, and so on.</li>
<li>The grouping variables (<code>trt01pn</code>, <code>trt01p</code>) pass through untouched, so the final data frame lines up structurally with the tidyverse output.</li>
<li>We run and confirm the column names and values match the tidyverse summary; quantile type 2 matters here too for Q1 and Q3.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data adsl;\n  input studyid $ usubjid $ trt01pn age fasfl $ trt01p :$20.;\n  datalines;\nMYCSG MYCSG-1001 1 23 Y \"Dose level 1\"\nMYCSG MYCSG-1002 3 68 Y \"Dose level 3\"\nMYCSG MYCSG-1003 3 .  Y \"Dose level 3\"\nMYCSG MYCSG-1004 3 35 Y \"Dose level 3\"\nMYCSG MYCSG-1006 3 54 Y \"Dose level 3\"\nMYCSG MYCSG-1007 1 63 N \"Dose level 1\"\n;\nrun;\n\nproc format;\n    value treatment\n    1=1\n    2=2\n    3=3\n    4=4\n    ;\nrun;\n\ndata adsl02;\n    set adsl;\n    where fasfl=\"Y\";\n    treatment=trt01pn;\n    output;\n    treatment=4;\n    output;\nrun;\n\nproc summary data=adsl02 nway completetypes;\n    class treatment\/preloadfmt;\n    var  age;\n    output out=stats01 n= nmiss= mean= std= min= q1= median= q3= max= \/autoname;\n    format treatment treatment.;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data adsl;\n  input studyid $ usubjid $ trt01pn age fasfl $ trt01p :$20.;\n  datalines;\nMYCSG MYCSG-1001 1 23 Y &quot;Dose level 1&quot;\nMYCSG MYCSG-1002 3 68 Y &quot;Dose level 3&quot;\nMYCSG MYCSG-1003 3 .  Y &quot;Dose level 3&quot;\nMYCSG MYCSG-1004 3 35 Y &quot;Dose level 3&quot;\nMYCSG MYCSG-1006 3 54 Y &quot;Dose level 3&quot;\nMYCSG MYCSG-1007 1 63 N &quot;Dose level 1&quot;\n;\nrun;\n\nproc format;\n    value treatment\n    1=1\n    2=2\n    3=3\n    4=4\n    ;\nrun;\n\ndata adsl02;\n    set adsl;\n    where fasfl=&quot;Y&quot;;\n    treatment=trt01pn;\n    output;\n    treatment=4;\n    output;\nrun;\n\nproc summary data=adsl02 nway completetypes;\n    class treatment\/preloadfmt;\n    var  age;\n    output out=stats01 n= nmiss= mean= std= min= q1= median= q3= max= \/autoname;\n    format treatment treatment.;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\r\nadsl <-\r\ntribble(\r\n~studyid, ~usubjid, ~trt01pn, ~age, ~fasfl, ~trt01p,\r\n\"MYCSG\", \"MYCSG-1001\", 1, 23, \"Y\", \"Dose level 1\",\r\n\"MYCSG\", \"MYCSG-1002\", 3, 68, \"Y\", \"Dose level 3\",\r\n\"MYCSG\", \"MYCSG-1003\", 3, NA, \"Y\", \"Dose level 3\",\r\n\"MYCSG\", \"MYCSG-1004\", 3, 35, \"Y\", \"Dose level 3\",\r\n\"MYCSG\", \"MYCSG-1006\", 3, 54, \"Y\", \"Dose level 3\",\r\n\"MYCSG\", \"MYCSG-1007\", 1, 63, \"N\", \"Dose level 1\"\r\n)\r\n\r\nstats01 <- adsl %>%\r\ngroup_by(trt01pn, trt01p) %>%\r\nsummarize(\r\nnrecs = n(),\r\nnmiss = sum(is.na(age)),\r\nn = nrecs - nmiss,\r\nmean = mean(age, na.rm = TRUE),\r\nstddev = sd(age, na.rm = TRUE),\r\nmin = min(age, na.rm = TRUE),\r\nq1 = quantile(age, 0.25, type = 2, na.rm = TRUE),\r\nmedian = median(age, na.rm = TRUE),\r\nq3 = quantile(age, 0.75, type = 2, na.rm = TRUE),\r\nmax = max(age, na.rm = TRUE) \r\n)","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\r\nadsl &lt;-\r\ntribble(\r\n~studyid, ~usubjid, ~trt01pn, ~age, ~fasfl, ~trt01p,\r\n&quot;MYCSG&quot;, &quot;MYCSG-1001&quot;, 1, 23, &quot;Y&quot;, &quot;Dose level 1&quot;,\r\n&quot;MYCSG&quot;, &quot;MYCSG-1002&quot;, 3, 68, &quot;Y&quot;, &quot;Dose level 3&quot;,\r\n&quot;MYCSG&quot;, &quot;MYCSG-1003&quot;, 3, NA, &quot;Y&quot;, &quot;Dose level 3&quot;,\r\n&quot;MYCSG&quot;, &quot;MYCSG-1004&quot;, 3, 35, &quot;Y&quot;, &quot;Dose level 3&quot;,\r\n&quot;MYCSG&quot;, &quot;MYCSG-1006&quot;, 3, 54, &quot;Y&quot;, &quot;Dose level 3&quot;,\r\n&quot;MYCSG&quot;, &quot;MYCSG-1007&quot;, 1, 63, &quot;N&quot;, &quot;Dose level 1&quot;\r\n)\r\n\r\nstats01 &lt;- adsl %&gt;%\r\ngroup_by(trt01pn, trt01p) %&gt;%\r\nsummarize(\r\nnrecs = n(),\r\nnmiss = sum(is.na(age)),\r\nn = nrecs - nmiss,\r\nmean = mean(age, na.rm = TRUE),\r\nstddev = sd(age, na.rm = TRUE),\r\nmin = min(age, na.rm = TRUE),\r\nq1 = quantile(age, 0.25, type = 2, na.rm = TRUE),\r\nmedian = median(age, na.rm = TRUE),\r\nq3 = quantile(age, 0.75, type = 2, na.rm = TRUE),\r\nmax = max(age, na.rm = TRUE) \r\n)<\/code><\/pre>"},{"label":"R (base)","code":"adsl <- data.frame(\r\n  studyid = c(\"MYCSG\", \"MYCSG\", \"MYCSG\", \"MYCSG\", \"MYCSG\", \"MYCSG\"),\r\n  usubjid = c(\"MYCSG-1001\", \"MYCSG-1002\", \"MYCSG-1003\", \"MYCSG-1004\", \"MYCSG-1006\", \"MYCSG-1007\"),\r\n  trt01pn = c(1, 3, 3, 3, 3, 1),\r\n  age = c(23, 68, NA, 35, 54, 63),\r\n  fasfl = c(\"Y\", \"Y\", \"Y\", \"Y\", \"Y\", \"N\"),\r\n  trt01p = c(\"Dose level 1\", \"Dose level 3\", \"Dose level 3\", \"Dose level 3\", \"Dose level 3\", \"Dose level 1\")\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\n#==============================================================================\r\n# Base R summary (aggregate) with intermediate names + final tidyverse-style names\r\n#==============================================================================\r\n\r\nstats01 <- aggregate(age ~ trt01pn + trt01p, data = adsl, FUN = function(x) {\r\n\r\n  tmp_nrecs  <- length(x)\r\n  tmp_nmiss  <- sum(is.na(x))\r\n  tmp_n      <- tmp_nrecs - tmp_nmiss\r\n\r\n  c(\r\n    tmp_nrecs  = tmp_nrecs,\r\n    tmp_nmiss  = tmp_nmiss,\r\n    tmp_n      = tmp_n,\r\n    tmp_mean   = mean(x, na.rm = TRUE),\r\n    tmp_stddev = sd(x, na.rm = TRUE),\r\n    tmp_min    = min(x, na.rm = TRUE),\r\n    tmp_q1     = unname(quantile(x, 0.25, type = 2, na.rm = TRUE)),\r\n    tmp_median = median(x, na.rm = TRUE),\r\n    tmp_q3     = unname(quantile(x, 0.75, type = 2, na.rm = TRUE)),\r\n    tmp_max    = max(x, na.rm = TRUE)\r\n  )\r\n})\r\n\r\n \r\n# Flatten the matrix column to real columns\r\nstats01 <- do.call(data.frame, stats01)\r\n\r\n \r\n# Remove analysis-variable prefix added by aggregate() (age.)\r\nnames(stats01) <- sub(\"^age\\\\.\", \"\", names(stats01))\r\n\r\n \r\n# Map intermediate names to final tidyverse-style names\r\nfinal_map <- c(\r\n  tmp_nrecs  = \"nrecs\",\r\n  tmp_nmiss  = \"nmiss\",\r\n  tmp_n      = \"n\",\r\n  tmp_mean   = \"mean\",\r\n  tmp_stddev = \"stddev\",\r\n  tmp_min    = \"min\",\r\n  tmp_q1     = \"q1\",\r\n  tmp_median = \"median\",\r\n  tmp_q3     = \"q3\",\r\n  tmp_max    = \"max\"\r\n)\r\n\r\n \r\nnames(stats01) <- ifelse(\r\n  names(stats01) %in% names(final_map),\r\n  final_map[names(stats01)],\r\n  names(stats01)  # keep trt01pn, trt01p as-is\r\n)","language":"r","html":"<pre><code class=\"language-r\">adsl &lt;- data.frame(\r\n  studyid = c(&quot;MYCSG&quot;, &quot;MYCSG&quot;, &quot;MYCSG&quot;, &quot;MYCSG&quot;, &quot;MYCSG&quot;, &quot;MYCSG&quot;),\r\n  usubjid = c(&quot;MYCSG-1001&quot;, &quot;MYCSG-1002&quot;, &quot;MYCSG-1003&quot;, &quot;MYCSG-1004&quot;, &quot;MYCSG-1006&quot;, &quot;MYCSG-1007&quot;),\r\n  trt01pn = c(1, 3, 3, 3, 3, 1),\r\n  age = c(23, 68, NA, 35, 54, 63),\r\n  fasfl = c(&quot;Y&quot;, &quot;Y&quot;, &quot;Y&quot;, &quot;Y&quot;, &quot;Y&quot;, &quot;N&quot;),\r\n  trt01p = c(&quot;Dose level 1&quot;, &quot;Dose level 3&quot;, &quot;Dose level 3&quot;, &quot;Dose level 3&quot;, &quot;Dose level 3&quot;, &quot;Dose level 1&quot;)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\n#==============================================================================\r\n# Base R summary (aggregate) with intermediate names + final tidyverse-style names\r\n#==============================================================================\r\n\r\nstats01 &lt;- aggregate(age ~ trt01pn + trt01p, data = adsl, FUN = function(x) {\r\n\r\n  tmp_nrecs  &lt;- length(x)\r\n  tmp_nmiss  &lt;- sum(is.na(x))\r\n  tmp_n      &lt;- tmp_nrecs - tmp_nmiss\r\n\r\n  c(\r\n    tmp_nrecs  = tmp_nrecs,\r\n    tmp_nmiss  = tmp_nmiss,\r\n    tmp_n      = tmp_n,\r\n    tmp_mean   = mean(x, na.rm = TRUE),\r\n    tmp_stddev = sd(x, na.rm = TRUE),\r\n    tmp_min    = min(x, na.rm = TRUE),\r\n    tmp_q1     = unname(quantile(x, 0.25, type = 2, na.rm = TRUE)),\r\n    tmp_median = median(x, na.rm = TRUE),\r\n    tmp_q3     = unname(quantile(x, 0.75, type = 2, na.rm = TRUE)),\r\n    tmp_max    = max(x, na.rm = TRUE)\r\n  )\r\n})\r\n\r\n \r\n# Flatten the matrix column to real columns\r\nstats01 &lt;- do.call(data.frame, stats01)\r\n\r\n \r\n# Remove analysis-variable prefix added by aggregate() (age.)\r\nnames(stats01) &lt;- sub(&quot;^age\\\\.&quot;, &quot;&quot;, names(stats01))\r\n\r\n \r\n# Map intermediate names to final tidyverse-style names\r\nfinal_map &lt;- c(\r\n  tmp_nrecs  = &quot;nrecs&quot;,\r\n  tmp_nmiss  = &quot;nmiss&quot;,\r\n  tmp_n      = &quot;n&quot;,\r\n  tmp_mean   = &quot;mean&quot;,\r\n  tmp_stddev = &quot;stddev&quot;,\r\n  tmp_min    = &quot;min&quot;,\r\n  tmp_q1     = &quot;q1&quot;,\r\n  tmp_median = &quot;median&quot;,\r\n  tmp_q3     = &quot;q3&quot;,\r\n  tmp_max    = &quot;max&quot;\r\n)\r\n\r\n \r\nnames(stats01) &lt;- ifelse(\r\n  names(stats01) %in% names(final_map),\r\n  final_map[names(stats01)],\r\n  names(stats01)  # keep trt01pn, trt01p as-is\r\n)<\/code><\/pre>"}] || [];
