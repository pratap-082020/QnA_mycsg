...
<title>mycsg SASnR2 2200_First-dot-concept</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>First dot concept</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Many analyses ask for the first record of each subject, first visit of each subject, first event of each term &mdash; the idea of picking the first row inside a group.</li>
<li>In SAS, once the data is sorted by the grouping variable, the automatic <code>first.</code> flag becomes TRUE on the first row of each group.</li>
<li>Tidyverse reaches the same idea with <code>group_by()</code> + <code>slice(1)</code> or <code>row_number()</code>.</li>
<li>Base R uses <code>!duplicated(group)</code> after sorting to keep the first occurrence of each group.</li>
<li>This lesson shows two small examples on CLASS: a within-group counter, and the lowest-Height subject within each Sex.</li>
<li>Both examples rely on sorting first &mdash; the definition of "first" only makes sense after we pick an order.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data CLASS;
...
<li>In SAS, once a dataset is sorted by a variable, the DATA step exposes automatic flags called <code>first.var</code> and <code>last.var</code> when that variable is named in a <code>by</code> statement.</li>
<li><code>first.sex</code> is TRUE on the first record of each Sex group and FALSE otherwise &mdash; exactly the signal we want for a within-group counter.</li>
<li>For the counter example, <code>proc sort data=class; by sex height weight name;</code> sets a stable order, then the DATA step reads CLASS with that same <code>by</code> list.</li>
<li><code>if first.sex then counter=1; else counter+1;</code> uses the sum statement form &mdash; <code>counter+1</code> automatically retains the counter across rows, so we never have to write an explicit <code>retain</code>.</li>
<li>For the lowest-Height example, sorting by <code>sex height</code> and writing <code>if first.sex;</code> on its own line keeps only the first row of each Sex &mdash; the one with the smallest Height.</li>
<li>After running, COUNTER has an incrementing counter within each Sex; LOWESTHEIGHT has one row per Sex with the smallest Height.</li>
...
group_by(Sex) %&gt;% 
slice(1) %&gt;% 
select(Name,Sex,Height)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In tidyverse, the "first row within each group" idea is expressed with <code>group_by()</code> combined with either <code>row_number()</code> or <code>slice(1)</code>.</li>
<li>For the counter: <code>arrange(Sex, Height, Weight, Name) %&gt;% group_by(Sex) %&gt;% mutate(counter = row_number())</code> assigns 1, 2, 3, ... within each Sex group after sorting.</li>
<li><code>row_number()</code> inside a grouped mutate is the direct tidyverse equivalent of the SAS <code>first.sex then counter=1; else counter+1;</code> pattern.</li>
<li>For the lowest-Height example: <code>arrange(Sex, Height) %&gt;% group_by(Sex) %&gt;% slice(1) %&gt;% select(Name, Sex, Height)</code> returns one row per Sex after sorting by Height ascending.</li>
<li><code>slice(1)</code> keeps the first row in the current group; combined with the earlier sort, that is the row with the smallest Height.</li>
<li>We run and confirm the counter climbs from 1 within each Sex, and the lowest-Height tibble has one row per Sex with the smallest Height value.</li>
...
lowestheight &lt;- lowestheight_tmp</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, the within-group counter is built with <code>ave()</code>: <code>ave(df$Sex, df$Sex, FUN = seq_along)</code> returns a vector that restarts at 1 within each Sex group.</li>
<li><code>ave(x, group, FUN = f)</code> runs <code>f</code> separately on each slice of <code>x</code> defined by <code>group</code>, then lines the results back up row-for-row &mdash; the direct base-R analogue of a grouped mutate.</li>
<li>Before computing the counter we sort the data frame with <code>order(Sex, Height, Weight, Name)</code> so the counter climbs in a meaningful order.</li>
<li>For the lowest-Height example, we sort by <code>Sex</code> and <code>Height</code> ascending, then apply <code>!duplicated(Sex)</code> to keep only the first occurrence of each Sex &mdash; which is the lowest-Height row after the sort.</li>
<li><code>df[, c("Name", "Sex", "Height")]</code> then trims the result to the columns the report needs.</li>
<li>We run and compare the counter and the one-row-per-Sex data frame against the tidyverse output &mdash; they should match row for row.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data CLASS;\n infile datalines dlm='|' dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nCarol|F|14|62.8|102.5\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\nJane|F|12|59.8|84.5\nJanet|F|15|62.5|112.5\nJeffrey|M|13|62.5|84\nJohn|M|12|59|99.5\nJoyce|F|11|51.3|50.5\nJudy|F|14|64.3|90\nLouise|F|12|56.3|77\nMary|F|15|66.5|112\nPhilip|M|16|72|150\nRobert|M|12|64.8|128\nRonald|M|15|67|133\nThomas|M|11|57.5|85\nWilliam|M|15|66.5|112\n;;;;\n run;\n\n*------------------------------------------------------------------------------;\n*counter;\n*------------------------------------------------------------------------------;\n\nproc sort data=class;\n     by sex height weight name;\n run;\n\ndata counter;\n     set class;\n     by sex height weight name;\n     if first.sex then counter=1;\n    else counter+1;\nrun;\n\n*------------------------------------------------------------------------------;\n*subset of lowest height;\n*------------------------------------------------------------------------------;\n\nproc sort data=class;\n     by sex height;\n run;\n\ndata lowestheight;\n     set class;\n     by sex height;\n     if first.sex;\n     keep sex height name;\n run;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data CLASS;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nCarol|F|14|62.8|102.5\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\nJane|F|12|59.8|84.5\nJanet|F|15|62.5|112.5\nJeffrey|M|13|62.5|84\nJohn|M|12|59|99.5\nJoyce|F|11|51.3|50.5\nJudy|F|14|64.3|90\nLouise|F|12|56.3|77\nMary|F|15|66.5|112\nPhilip|M|16|72|150\nRobert|M|12|64.8|128\nRonald|M|15|67|133\nThomas|M|11|57.5|85\nWilliam|M|15|66.5|112\n;;;;\n run;\n\n*------------------------------------------------------------------------------;\n*counter;\n*------------------------------------------------------------------------------;\n\nproc sort data=class;\n     by sex height weight name;\n run;\n\ndata counter;\n     set class;\n     by sex height weight name;\n     if first.sex then counter=1;\n    else counter+1;\nrun;\n\n*------------------------------------------------------------------------------;\n*subset of lowest height;\n*------------------------------------------------------------------------------;\n\nproc sort data=class;\n     by sex height;\n run;\n\ndata lowestheight;\n     set class;\n     by sex height;\n     if first.sex;\n     keep sex height name;\n run;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse) \r\nclass<-tribble( \r\n~Name,~Sex,~Age,~Height,~Weight, \r\n\"Alfred\",\"M\",14,69,112.5, \r\n\"Alice\",\"F\",13,56.5,84, \r\n\"Barbara\",\"F\",13,65.3,98, \r\n\"Carol\",\"F\",14,62.8,102.5, \r\n\"Henry\",\"M\",14,63.5,102.5, \r\n\"James\",\"M\",12,57.3,83, \r\n\"Jane\",\"F\",12,59.8,84.5, \r\n\"Janet\",\"F\",15,62.5,112.5, \r\n\"Jeffrey\",\"M\",13,62.5,84, \r\n\"John\",\"M\",12,59,99.5, \r\n\"Joyce\",\"F\",11,51.3,50.5, \r\n\"Judy\",\"F\",14,64.3,90, \r\n\"Louise\",\"F\",12,56.3,77, \r\n\"Mary\",\"F\",15,66.5,112, \r\n\"Philip\",\"M\",16,72,150, \r\n\"Robert\",\"M\",12,64.8,128, \r\n\"Ronald\",\"M\",15,67,133, \r\n\"Thomas\",\"M\",11,57.5,85, \r\n\"William\",\"M\",15,66.5,112, \r\n) \r\n\r\ncounter<-class %>% \r\narrange(Sex,Height,Weight,Name) \r\n%>% group_by(Sex) \r\n%>% mutate(counter=row_number()) \r\n\r\nlowestheight<-class %>% \r\narrange(Sex,Height) %>% \r\ngroup_by(Sex) %>% \r\nslice(1) %>% \r\nselect(Name,Sex,Height)","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse) \r\nclass&lt;-tribble( \r\n~Name,~Sex,~Age,~Height,~Weight, \r\n&quot;Alfred&quot;,&quot;M&quot;,14,69,112.5, \r\n&quot;Alice&quot;,&quot;F&quot;,13,56.5,84, \r\n&quot;Barbara&quot;,&quot;F&quot;,13,65.3,98, \r\n&quot;Carol&quot;,&quot;F&quot;,14,62.8,102.5, \r\n&quot;Henry&quot;,&quot;M&quot;,14,63.5,102.5, \r\n&quot;James&quot;,&quot;M&quot;,12,57.3,83, \r\n&quot;Jane&quot;,&quot;F&quot;,12,59.8,84.5, \r\n&quot;Janet&quot;,&quot;F&quot;,15,62.5,112.5, \r\n&quot;Jeffrey&quot;,&quot;M&quot;,13,62.5,84, \r\n&quot;John&quot;,&quot;M&quot;,12,59,99.5, \r\n&quot;Joyce&quot;,&quot;F&quot;,11,51.3,50.5, \r\n&quot;Judy&quot;,&quot;F&quot;,14,64.3,90, \r\n&quot;Louise&quot;,&quot;F&quot;,12,56.3,77, \r\n&quot;Mary&quot;,&quot;F&quot;,15,66.5,112, \r\n&quot;Philip&quot;,&quot;M&quot;,16,72,150, \r\n&quot;Robert&quot;,&quot;M&quot;,12,64.8,128, \r\n&quot;Ronald&quot;,&quot;M&quot;,15,67,133, \r\n&quot;Thomas&quot;,&quot;M&quot;,11,57.5,85, \r\n&quot;William&quot;,&quot;M&quot;,15,66.5,112, \r\n) \r\n\r\ncounter&lt;-class %&gt;% \r\narrange(Sex,Height,Weight,Name) \r\n%&gt;% group_by(Sex) \r\n%&gt;% mutate(counter=row_number()) \r\n\r\nlowestheight&lt;-class %&gt;% \r\narrange(Sex,Height) %&gt;% \r\ngroup_by(Sex) %&gt;% \r\nslice(1) %&gt;% \r\nselect(Name,Sex,Height)<\/code><\/pre>"},{"label":"R (base)","code":"class <- data.frame(\r\n  Name = c(\"Alfred\", \"Alice\", \"Barbara\", \"Carol\", \"Henry\", \"James\", \"Jane\", \"Janet\", \"Jeffrey\", \"John\", \"Joyce\", \"Judy\", \"Louise\", \"Mary\", \"Philip\", \"Robert\", \"Ronald\", \"Thomas\", \"William\"),\r\n  Sex = c(\"M\", \"F\", \"F\", \"F\", \"M\", \"M\", \"F\", \"F\", \"M\", \"M\", \"F\", \"F\", \"F\", \"F\", \"M\", \"M\", \"M\", \"M\", \"M\"),\r\n  Age = c(14, 13, 13, 14, 14, 12, 12, 15, 13, 12, 11, 14, 12, 15, 16, 12, 15, 11, 15),\r\n  Height = c(69, 56.5, 65.3, 62.8, 63.5, 57.3, 59.8, 62.5, 62.5, 59, 51.3, 64.3, 56.3, 66.5, 72, 64.8, 67, 57.5, 66.5),\r\n  Weight = c(112.5, 84, 98, 102.5, 102.5, 83, 84.5, 112.5, 84, 99.5, 50.5, 90, 77, 112, 150, 128, 133, 85, 112)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\n#==============================================================================;\r\n#Counter;\r\n#==============================================================================;\r\n\r\n\u00a0\r\ncounter_tmp <- class\r\ncounter_tmp <- counter_tmp[order(counter_tmp$Sex, counter_tmp$Height, counter_tmp$Weight, counter_tmp$Name), ]\r\ncounter_tmp$counter <- ave(counter_tmp$Sex, counter_tmp$Sex, FUN = seq_along)\r\ncounter <- counter_tmp\r\n\r\n\u00a0\r\n#==============================================================================;\r\n#Subset lowest height student within each sex;\r\n#==============================================================================;\r\n\r\n\u00a0\r\nlowestheight_tmp <- class\r\nlowestheight_tmp <- lowestheight_tmp[order(lowestheight_tmp$Sex, lowestheight_tmp$Height), ]\r\nlowestheight_tmp <- lowestheight_tmp[!duplicated(lowestheight_tmp$Sex), ]\r\nlowestheight_tmp <- lowestheight_tmp[, c(\"Name\", \"Sex\", \"Height\")]\r\nlowestheight <- lowestheight_tmp","language":"r","html":"<pre><code class=\"language-r\">class &lt;- data.frame(\r\n  Name = c(&quot;Alfred&quot;, &quot;Alice&quot;, &quot;Barbara&quot;, &quot;Carol&quot;, &quot;Henry&quot;, &quot;James&quot;, &quot;Jane&quot;, &quot;Janet&quot;, &quot;Jeffrey&quot;, &quot;John&quot;, &quot;Joyce&quot;, &quot;Judy&quot;, &quot;Louise&quot;, &quot;Mary&quot;, &quot;Philip&quot;, &quot;Robert&quot;, &quot;Ronald&quot;, &quot;Thomas&quot;, &quot;William&quot;),\r\n  Sex = c(&quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;M&quot;, &quot;M&quot;, &quot;M&quot;, &quot;M&quot;),\r\n  Age = c(14, 13, 13, 14, 14, 12, 12, 15, 13, 12, 11, 14, 12, 15, 16, 12, 15, 11, 15),\r\n  Height = c(69, 56.5, 65.3, 62.8, 63.5, 57.3, 59.8, 62.5, 62.5, 59, 51.3, 64.3, 56.3, 66.5, 72, 64.8, 67, 57.5, 66.5),\r\n  Weight = c(112.5, 84, 98, 102.5, 102.5, 83, 84.5, 112.5, 84, 99.5, 50.5, 90, 77, 112, 150, 128, 133, 85, 112)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\n#==============================================================================;\r\n#Counter;\r\n#==============================================================================;\r\n\r\n\u00a0\r\ncounter_tmp &lt;- class\r\ncounter_tmp &lt;- counter_tmp[order(counter_tmp$Sex, counter_tmp$Height, counter_tmp$Weight, counter_tmp$Name), ]\r\ncounter_tmp$counter &lt;- ave(counter_tmp$Sex, counter_tmp$Sex, FUN = seq_along)\r\ncounter &lt;- counter_tmp\r\n\r\n\u00a0\r\n#==============================================================================;\r\n#Subset lowest height student within each sex;\r\n#==============================================================================;\r\n\r\n\u00a0\r\nlowestheight_tmp &lt;- class\r\nlowestheight_tmp &lt;- lowestheight_tmp[order(lowestheight_tmp$Sex, lowestheight_tmp$Height), ]\r\nlowestheight_tmp &lt;- lowestheight_tmp[!duplicated(lowestheight_tmp$Sex), ]\r\nlowestheight_tmp &lt;- lowestheight_tmp[, c(&quot;Name&quot;, &quot;Sex&quot;, &quot;Height&quot;)]\r\nlowestheight &lt;- lowestheight_tmp<\/code><\/pre>"}] || [];
