...
<title>mycsg SASnR2 2210_Last-dot-concept</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Last dot concept</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>The natural counterpart to "first of each group" is "last of each group" &mdash; the last visit per subject, the latest lab per test, the oldest age in each Sex.</li>
<li>In SAS, once a dataset is sorted by the grouping variable, the automatic flag <code>last.var</code> is TRUE on the last row of each group.</li>
<li>Tidyverse uses <code>group_by()</code> + <code>slice(n())</code> or <code>slice_tail()</code> to keep the last row within each group.</li>
<li>Base R uses <code>!duplicated(group, fromLast = TRUE)</code> after sorting to keep the last occurrence per group.</li>
<li>This lesson picks the subject with the highest Height within each Sex on CLASS &mdash; the mirror image of lesson 2200.</li>
<li>As always, "last" is defined by the sort order, so we sort first and then take the last row.</li>
...
<li>In SAS, <code>last.var</code> is the automatic flag that turns TRUE on the last row of each group, once the dataset is sorted and the variable is named in a <code>by</code> statement.</li>
<li><code>proc sort data=class; by sex height;</code> sorts ascending by Sex then Height, so within each Sex the tallest subject sits at the bottom of the group.</li>
<li>In the DATA step, <code>set class; by sex height;</code> reads the sorted data and exposes <code>first.sex</code> / <code>last.sex</code> / <code>first.height</code> / <code>last.height</code>.</li>
<li><code>if last.sex;</code> on its own line is a subsetting IF &mdash; only rows where that flag is TRUE are written out.</li>
<li><code>keep sex height name;</code> trims the output to the columns the report needs.</li>
<li>After running, HIGHESTHEIGHT has one row per Sex with the tallest subject; confirm the Name column lists the expected subjects.</li>
...
<li>In tidyverse, the "last row within each group" idea uses <code>slice(n())</code> or <code>slice_tail(n = 1)</code> after <code>group_by()</code>.</li>
<li><code>arrange(Sex, Height) %&gt;% group_by(Sex) %&gt;% slice(n()) %&gt;% select(Name, Sex, Height)</code> returns one row per Sex &mdash; the row with the largest Height after the sort.</li>
<li><code>n()</code> gives the number of rows in the current group, so <code>slice(n())</code> picks the final row of that group.</li>
<li>The sort direction matters: if we sort Height ascending, the last row is the tallest; if we sort descending, the last row becomes the shortest.</li>
<li>We can drop <code>group_by()</code> and the call returns only the overall last row instead of one per Sex.</li>
<li>We run and confirm one row per Sex with the tallest Height &mdash; the tidyverse counterpart of <code>if last.sex</code> in SAS.</li>
...
<li>In base R, <code>!duplicated(x, fromLast = TRUE)</code> is the trick that keeps the last occurrence of each group &mdash; the mirror image of the "first" trick from lesson 2200.</li>
<li>We first sort the data with <code>order(Sex, Height)</code>; within each Sex group, the tallest subject lands at the bottom of the group.</li>
<li><code>duplicated(Sex, fromLast = TRUE)</code> scans from the bottom up, so only the final row of each Sex survives the <code>!</code> negation.</li>
<li><code>df[!duplicated(df$Sex, fromLast = TRUE), ]</code> uses that logical vector to subset rows and keep the last row per Sex.</li>
<li>We can flip the argument to <code>fromLast = FALSE</code> (the default) and the call becomes the "first dot" version from the previous lesson.</li>
<li>We run and verify the tallest subject per Sex comes through; the output should match the tidyverse <code>slice(n())</code> result.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data CLASS;\n infile datalines dlm='|' dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nCarol|F|14|62.8|102.5\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\nJane|F|12|59.8|84.5\nJanet|F|15|62.5|112.5\nJeffrey|M|13|62.5|84\nJohn|M|12|59|99.5\nJoyce|F|11|51.3|50.5\nJudy|F|14|64.3|90\nLouise|F|12|56.3|77\nMary|F|15|66.5|112\nPhilip|M|16|72|150\nRobert|M|12|64.8|128\nRonald|M|15|67|133\nThomas|M|11|57.5|85\nWilliam|M|15|66.5|112\n;;;;\n run;\n\n*------------------------------------------------------------------------------;\n*subset of highest height;\n*------------------------------------------------------------------------------;\n\nproc sort data=class;\n     by sex height;\n run;\n\ndata highestheight;\n     set class;\n     by sex height;\n     if last.sex;\n     keep sex height name;\n run;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data CLASS;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nCarol|F|14|62.8|102.5\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\nJane|F|12|59.8|84.5\nJanet|F|15|62.5|112.5\nJeffrey|M|13|62.5|84\nJohn|M|12|59|99.5\nJoyce|F|11|51.3|50.5\nJudy|F|14|64.3|90\nLouise|F|12|56.3|77\nMary|F|15|66.5|112\nPhilip|M|16|72|150\nRobert|M|12|64.8|128\nRonald|M|15|67|133\nThomas|M|11|57.5|85\nWilliam|M|15|66.5|112\n;;;;\n run;\n\n*------------------------------------------------------------------------------;\n*subset of highest height;\n*------------------------------------------------------------------------------;\n\nproc sort data=class;\n     by sex height;\n run;\n\ndata highestheight;\n     set class;\n     by sex height;\n     if last.sex;\n     keep sex height name;\n run;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse) \r\nclass<-tribble(\r\n~Name,~Sex,~Age,~Height,~Weight, \r\n\"Alfred\",\"M\",14,69,112.5, \r\n\"Alice\",\"F\",13,56.5,84, \r\n\"Barbara\",\"F\",13,65.3,98, \r\n\"Carol\",\"F\",14,62.8,102.5, \r\n\"Henry\",\"M\",14,63.5,102.5, \r\n\"James\",\"M\",12,57.3,83,\r\n \"Jane\",\"F\",12,59.8,84.5, \r\n\"Janet\",\"F\",15,62.5,112.5, \r\n\"Jeffrey\",\"M\",13,62.5,84, \r\n\"John\",\"M\",12,59,99.5, \r\n\"Joyce\",\"F\",11,51.3,50.5, \r\n\"Judy\",\"F\",14,64.3,90,\r\n \"Louise\",\"F\",12,56.3,77, \r\n\"Mary\",\"F\",15,66.5,112, \r\n\"Philip\",\"M\",16,72,150, \r\n\"Robert\",\"M\",12,64.8,128, \r\n\"Ronald\",\"M\",15,67,133, \r\n\"Thomas\",\"M\",11,57.5,85, \r\n\"William\",\"M\",15,66.5,112, \r\n) \r\n\r\nhighestheight<-class %>% \r\narrange(Sex,Height) %>% \r\ngroup_by(Sex) %>% \r\nslice(n()) %>% \r\nselect(Name,Sex,Height)","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse) \r\nclass&lt;-tribble(\r\n~Name,~Sex,~Age,~Height,~Weight, \r\n&quot;Alfred&quot;,&quot;M&quot;,14,69,112.5, \r\n&quot;Alice&quot;,&quot;F&quot;,13,56.5,84, \r\n&quot;Barbara&quot;,&quot;F&quot;,13,65.3,98, \r\n&quot;Carol&quot;,&quot;F&quot;,14,62.8,102.5, \r\n&quot;Henry&quot;,&quot;M&quot;,14,63.5,102.5, \r\n&quot;James&quot;,&quot;M&quot;,12,57.3,83,\r\n &quot;Jane&quot;,&quot;F&quot;,12,59.8,84.5, \r\n&quot;Janet&quot;,&quot;F&quot;,15,62.5,112.5, \r\n&quot;Jeffrey&quot;,&quot;M&quot;,13,62.5,84, \r\n&quot;John&quot;,&quot;M&quot;,12,59,99.5, \r\n&quot;Joyce&quot;,&quot;F&quot;,11,51.3,50.5, \r\n&quot;Judy&quot;,&quot;F&quot;,14,64.3,90,\r\n &quot;Louise&quot;,&quot;F&quot;,12,56.3,77, \r\n&quot;Mary&quot;,&quot;F&quot;,15,66.5,112, \r\n&quot;Philip&quot;,&quot;M&quot;,16,72,150, \r\n&quot;Robert&quot;,&quot;M&quot;,12,64.8,128, \r\n&quot;Ronald&quot;,&quot;M&quot;,15,67,133, \r\n&quot;Thomas&quot;,&quot;M&quot;,11,57.5,85, \r\n&quot;William&quot;,&quot;M&quot;,15,66.5,112, \r\n) \r\n\r\nhighestheight&lt;-class %&gt;% \r\narrange(Sex,Height) %&gt;% \r\ngroup_by(Sex) %&gt;% \r\nslice(n()) %&gt;% \r\nselect(Name,Sex,Height)<\/code><\/pre>"},{"label":"R (base)","code":"class <- data.frame(\r\n  Name = c(\"Alfred\", \"Alice\", \"Barbara\", \"Carol\", \"Henry\", \"James\", \"Jane\", \"Janet\", \"Jeffrey\", \"John\", \"Joyce\", \"Judy\", \"Louise\", \"Mary\", \"Philip\", \"Robert\", \"Ronald\", \"Thomas\", \"William\"),\r\n  Sex = c(\"M\", \"F\", \"F\", \"F\", \"M\", \"M\", \"F\", \"F\", \"M\", \"M\", \"F\", \"F\", \"F\", \"F\", \"M\", \"M\", \"M\", \"M\", \"M\"),\r\n  Age = c(14, 13, 13, 14, 14, 12, 12, 15, 13, 12, 11, 14, 12, 15, 16, 12, 15, 11, 15),\r\n  Height = c(69, 56.5, 65.3, 62.8, 63.5, 57.3, 59.8, 62.5, 62.5, 59, 51.3, 64.3, 56.3, 66.5, 72, 64.8, 67, 57.5, 66.5),\r\n  Weight = c(112.5, 84, 98, 102.5, 102.5, 83, 84.5, 112.5, 84, 99.5, 50.5, 90, 77, 112, 150, 128, 133, 85, 112)\r\n  , stringsAsFactors = FALSE\r\n)\u00a0\r\n\r\nhighestheight_tmp <- class\r\nhighestheight_tmp <- highestheight_tmp[\r\n\u00a0 order(highestheight_tmp$Sex, highestheight_tmp$Height),\r\n]\r\nhighestheight_tmp <- highestheight_tmp[\r\n\u00a0 !duplicated(highestheight_tmp$Sex, fromLast = TRUE),\r\n]","language":"r","html":"<pre><code class=\"language-r\">class &lt;- data.frame(\r\n  Name = c(&quot;Alfred&quot;, &quot;Alice&quot;, &quot;Barbara&quot;, &quot;Carol&quot;, &quot;Henry&quot;, &quot;James&quot;, &quot;Jane&quot;, &quot;Janet&quot;, &quot;Jeffrey&quot;, &quot;John&quot;, &quot;Joyce&quot;, &quot;Judy&quot;, &quot;Louise&quot;, &quot;Mary&quot;, &quot;Philip&quot;, &quot;Robert&quot;, &quot;Ronald&quot;, &quot;Thomas&quot;, &quot;William&quot;),\r\n  Sex = c(&quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;M&quot;, &quot;M&quot;, &quot;M&quot;, &quot;M&quot;),\r\n  Age = c(14, 13, 13, 14, 14, 12, 12, 15, 13, 12, 11, 14, 12, 15, 16, 12, 15, 11, 15),\r\n  Height = c(69, 56.5, 65.3, 62.8, 63.5, 57.3, 59.8, 62.5, 62.5, 59, 51.3, 64.3, 56.3, 66.5, 72, 64.8, 67, 57.5, 66.5),\r\n  Weight = c(112.5, 84, 98, 102.5, 102.5, 83, 84.5, 112.5, 84, 99.5, 50.5, 90, 77, 112, 150, 128, 133, 85, 112)\r\n  , stringsAsFactors = FALSE\r\n)\u00a0\r\n\r\nhighestheight_tmp &lt;- class\r\nhighestheight_tmp &lt;- highestheight_tmp[\r\n\u00a0 order(highestheight_tmp$Sex, highestheight_tmp$Height),\r\n]\r\nhighestheight_tmp &lt;- highestheight_tmp[\r\n\u00a0 !duplicated(highestheight_tmp$Sex, fromLast = TRUE),\r\n]<\/code><\/pre>"}] || [];
