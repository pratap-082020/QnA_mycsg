...
<title>mycsg SASnR2 2220_First-dot-and-last-dot-concept</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>First dot and last dot concept</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Sometimes we want rows where a group has exactly one record &mdash; subjects with a single visit, ages that appear only once, terms reported by only one subject.</li>
<li>In SAS, <code>first.var</code> AND <code>last.var</code> on the same row is the flag combination that marks a group of size one &mdash; the row is both the first and the last in its group.</li>
<li>Tidyverse counts group size with <code>mutate(nrows = n())</code> after <code>group_by()</code>, then filters to <code>nrows == 1</code>.</li>
<li>Base R uses <code>ave(..., FUN = length)</code> to compute the group size per row, then subsets to rows where the size is 1.</li>
<li>This lesson picks ages that appear in exactly one row of CLASS &mdash; the ones where the Age group has only a single subject.</li>
<li>After running, inspect the result and confirm each surviving Age value shows up only once in CLASS.</li>
...
<li>When both <code>first.var</code> and <code>last.var</code> are TRUE for the same row, that row is the only one in its group.</li>
<li>After <code>proc sort data=class; by age;</code>, the DATA step reads CLASS with <code>by age;</code> so the <code>first.age</code> / <code>last.age</code> flags are available.</li>
<li><code>if first.age and last.age;</code> is a subsetting IF &mdash; SAS writes out the row only when the condition is TRUE, which happens only when the Age group has exactly one row.</li>
<li>No <code>output</code> or <code>retain</code> statement is needed; the subsetting IF is all we need to thin the output to singleton groups.</li>
<li>After running, ONLY_ONE_IN_GROUP contains those CLASS rows whose Age appears only once in the sample.</li>
<li>A good sanity check: a frequency count of Age on CLASS should show frequency 1 for exactly the Age values we see in ONLY_ONE_IN_GROUP.</li>
...
group_by(age) %&gt;% 
mutate(nrows=n()) %&gt;% 
filter(nrows==1)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In tidyverse, "group of size one" is expressed with a grouped count followed by a filter.</li>
<li><code>group_by(age) %&gt;% mutate(nrows = n()) %&gt;% filter(nrows == 1)</code> first attaches the group size to every row, then keeps only rows where the size equals 1.</li>
<li><code>n()</code> inside a grouped <code>mutate()</code> returns the number of rows in the current group &mdash; the direct tidyverse equivalent of counting with PROC FREQ.</li>
<li>Because <code>mutate()</code> keeps the row count, the resulting tibble shows the same columns as the input, with the <code>nrows</code> helper column added.</li>
<li>A small cleanup step is to drop the helper column with <code>select(-nrows)</code> or to use <code>filter(n() == 1)</code> directly without creating the helper in the first place.</li>
<li>We run and compare the survivors against the SAS ONLY_ONE_IN_GROUP dataset &mdash; the same rows should come through in both languages.</li>
...
only_one_in_group &lt;- only_one_in_group_tmp</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, the grouped count is built with <code>ave()</code>: <code>ave(df$age, df$age, FUN = length)</code> returns the size of each age group, lined up row for row.</li>
<li>Assigning that vector to <code>df$nrows</code> attaches the group size to each row.</li>
<li><code>df[df$nrows == 1, ]</code> then subsets to rows where the count is exactly 1 &mdash; the base R analogue of <code>filter(nrows == 1)</code>.</li>
<li>We use a temporary copy (<code>only_one_in_group_tmp</code>) to keep the original data frame unchanged while building the helper column and filtering.</li>
<li>The overall logic mirrors the tidyverse form: compute group size &rarr; attach to each row &rarr; keep only the rows whose group has a single observation.</li>
<li>We run and confirm the result is the same set of rows as the tidyverse version and the SAS ONLY_ONE_IN_GROUP dataset.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data CLASS;\n infile datalines dlm='|' dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nCarol|F|14|62.8|102.5\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\nJane|F|12|59.8|84.5\nJanet|F|15|62.5|112.5\nJeffrey|M|13|62.5|84\nJohn|M|12|59|99.5\nJoyce|F|11|51.3|50.5\nJudy|F|14|64.3|90\nLouise|F|12|56.3|77\nMary|F|15|66.5|112\nPhilip|M|16|72|150\nRobert|M|12|64.8|128\nRonald|M|15|67|133\nThomas|M|11|57.5|85\nWilliam|M|15|66.5|112\n;;;;\n run;\n\nproc sort data=class;\n    by age;\n run;\n\ndata only_one_in_group;\n     set class;\n     by age;\n     if first.age and last.age;\n run;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data CLASS;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nCarol|F|14|62.8|102.5\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\nJane|F|12|59.8|84.5\nJanet|F|15|62.5|112.5\nJeffrey|M|13|62.5|84\nJohn|M|12|59|99.5\nJoyce|F|11|51.3|50.5\nJudy|F|14|64.3|90\nLouise|F|12|56.3|77\nMary|F|15|66.5|112\nPhilip|M|16|72|150\nRobert|M|12|64.8|128\nRonald|M|15|67|133\nThomas|M|11|57.5|85\nWilliam|M|15|66.5|112\n;;;;\n run;\n\nproc sort data=class;\n    by age;\n run;\n\ndata only_one_in_group;\n     set class;\n     by age;\n     if first.age and last.age;\n run;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse) \r\nclass<-tribble(\r\n~Name,~Sex,~Age,~Height,~Weight, \r\n\"Alfred\",\"M\",14,69,112.5, \r\n\"Alice\",\"F\",13,56.5,84, \r\n\"Barbara\",\"F\",13,65.3,98, \r\n\"Carol\",\"F\",14,62.8,102.5, \r\n\"Henry\",\"M\",14,63.5,102.5, \r\n\"James\",\"M\",12,57.3,83,\r\n \"Jane\",\"F\",12,59.8,84.5, \r\n\"Janet\",\"F\",15,62.5,112.5, \r\n\"Jeffrey\",\"M\",13,62.5,84, \r\n\"John\",\"M\",12,59,99.5, \r\n\"Joyce\",\"F\",11,51.3,50.5, \r\n\"Judy\",\"F\",14,64.3,90,\r\n \"Louise\",\"F\",12,56.3,77, \r\n\"Mary\",\"F\",15,66.5,112, \r\n\"Philip\",\"M\",16,72,150, \r\n\"Robert\",\"M\",12,64.8,128, \r\n\"Ronald\",\"M\",15,67,133, \r\n\"Thomas\",\"M\",11,57.5,85, \r\n\"William\",\"M\",15,66.5,112, \r\n) \r\nonly_one_in_group<-class %>% \r\ngroup_by(age) %>% \r\nmutate(nrows=n()) %>% \r\nfilter(nrows==1)","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse) \r\nclass&lt;-tribble(\r\n~Name,~Sex,~Age,~Height,~Weight, \r\n&quot;Alfred&quot;,&quot;M&quot;,14,69,112.5, \r\n&quot;Alice&quot;,&quot;F&quot;,13,56.5,84, \r\n&quot;Barbara&quot;,&quot;F&quot;,13,65.3,98, \r\n&quot;Carol&quot;,&quot;F&quot;,14,62.8,102.5, \r\n&quot;Henry&quot;,&quot;M&quot;,14,63.5,102.5, \r\n&quot;James&quot;,&quot;M&quot;,12,57.3,83,\r\n &quot;Jane&quot;,&quot;F&quot;,12,59.8,84.5, \r\n&quot;Janet&quot;,&quot;F&quot;,15,62.5,112.5, \r\n&quot;Jeffrey&quot;,&quot;M&quot;,13,62.5,84, \r\n&quot;John&quot;,&quot;M&quot;,12,59,99.5, \r\n&quot;Joyce&quot;,&quot;F&quot;,11,51.3,50.5, \r\n&quot;Judy&quot;,&quot;F&quot;,14,64.3,90,\r\n &quot;Louise&quot;,&quot;F&quot;,12,56.3,77, \r\n&quot;Mary&quot;,&quot;F&quot;,15,66.5,112, \r\n&quot;Philip&quot;,&quot;M&quot;,16,72,150, \r\n&quot;Robert&quot;,&quot;M&quot;,12,64.8,128, \r\n&quot;Ronald&quot;,&quot;M&quot;,15,67,133, \r\n&quot;Thomas&quot;,&quot;M&quot;,11,57.5,85, \r\n&quot;William&quot;,&quot;M&quot;,15,66.5,112, \r\n) \r\nonly_one_in_group&lt;-class %&gt;% \r\ngroup_by(age) %&gt;% \r\nmutate(nrows=n()) %&gt;% \r\nfilter(nrows==1)<\/code><\/pre>"},{"label":"R (base)","code":"class <- data.frame(\r\n  name = c(\"Alfred\", \"Alice\", \"Barbara\", \"Carol\", \"Henry\", \"James\", \"Jane\", \"Janet\", \"Jeffrey\", \"John\", \"Joyce\", \"Judy\", \"Louise\", \"Mary\", \"Philip\", \"Robert\", \"Ronald\", \"Thomas\", \"William\"),\r\n  sex = c(\"M\", \"F\", \"F\", \"F\", \"M\", \"M\", \"F\", \"F\", \"M\", \"M\", \"F\", \"F\", \"F\", \"F\", \"M\", \"M\", \"M\", \"M\", \"M\"),\r\n  age = c(14, 13, 13, 14, 14, 12, 12, 15, 13, 12, 11, 14, 12, 15, 16, 12, 15, 11, 15),\r\n  height = c(69, 56.5, 65.3, 62.8, 63.5, 57.3, 59.8, 62.5, 62.5, 59, 51.3, 64.3, 56.3, 66.5, 72, 64.8, 67, 57.5, 66.5),\r\n  weight = c(112.5, 84, 98, 102.5, 102.5, 83, 84.5, 112.5, 84, 99.5, 50.5, 90, 77, 112, 150, 128, 133, 85, 112)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\n\r\nonly_one_in_group_tmp <- class\r\n\r\n\u00a0\r\nonly_one_in_group_tmp$nrows <- ave(\r\n\u00a0 only_one_in_group_tmp$age,\r\n\u00a0 only_one_in_group_tmp$age,\r\n\u00a0 FUN = length\r\n)\r\n\r\nonly_one_in_group_tmp <- only_one_in_group_tmp[only_one_in_group_tmp$nrows == 1, ]\r\n\r\nonly_one_in_group <- only_one_in_group_tmp","language":"r","html":"<pre><code class=\"language-r\">class &lt;- data.frame(\r\n  name = c(&quot;Alfred&quot;, &quot;Alice&quot;, &quot;Barbara&quot;, &quot;Carol&quot;, &quot;Henry&quot;, &quot;James&quot;, &quot;Jane&quot;, &quot;Janet&quot;, &quot;Jeffrey&quot;, &quot;John&quot;, &quot;Joyce&quot;, &quot;Judy&quot;, &quot;Louise&quot;, &quot;Mary&quot;, &quot;Philip&quot;, &quot;Robert&quot;, &quot;Ronald&quot;, &quot;Thomas&quot;, &quot;William&quot;),\r\n  sex = c(&quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;M&quot;, &quot;M&quot;, &quot;M&quot;, &quot;M&quot;),\r\n  age = c(14, 13, 13, 14, 14, 12, 12, 15, 13, 12, 11, 14, 12, 15, 16, 12, 15, 11, 15),\r\n  height = c(69, 56.5, 65.3, 62.8, 63.5, 57.3, 59.8, 62.5, 62.5, 59, 51.3, 64.3, 56.3, 66.5, 72, 64.8, 67, 57.5, 66.5),\r\n  weight = c(112.5, 84, 98, 102.5, 102.5, 83, 84.5, 112.5, 84, 99.5, 50.5, 90, 77, 112, 150, 128, 133, 85, 112)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\n\r\nonly_one_in_group_tmp &lt;- class\r\n\r\n\u00a0\r\nonly_one_in_group_tmp$nrows &lt;- ave(\r\n\u00a0 only_one_in_group_tmp$age,\r\n\u00a0 only_one_in_group_tmp$age,\r\n\u00a0 FUN = length\r\n)\r\n\r\nonly_one_in_group_tmp &lt;- only_one_in_group_tmp[only_one_in_group_tmp$nrows == 1, ]\r\n\r\nonly_one_in_group &lt;- only_one_in_group_tmp<\/code><\/pre>"}] || [];
