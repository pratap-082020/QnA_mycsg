...
<title>mycsg SASnR2 2500_Transpose-data---wide-to-long</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Transpose data - wide to long</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>The reverse of long-to-wide is wide-to-long &mdash; taking one row per subject with many measurement columns and stacking those columns into rows.</li>
<li>Long shape is what analytic tools usually want: one row per subject per measurement, with a column naming the measurement and another holding the value.</li>
<li>SAS uses <code>proc transpose</code> with <code>by</code>, <code>var</code>, and the <code>name=</code>/<code>rename=</code> options to control the new column names.</li>
<li>Tidyverse uses <code>pivot_longer()</code> from tidyr, specifying the columns to stack and the names of the new key/value columns.</li>
<li>Base R uses <code>reshape()</code> with <code>direction = "long"</code>, providing <code>varying</code>, <code>v.names</code>, <code>timevar</code>, <code>times</code>, and <code>idvar</code>.</li>
<li>This lesson takes a WIDE dataset with one row per subject and columns HGB, ALT, AST, and pivots it into a LONG dataset with <code>lbtestcd</code> and <code>lbstresn</code>.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data WIDE;
...
<li>PROC TRANSPOSE works in both directions &mdash; for wide-to-long the pattern uses <code>var</code> to list the columns to stack and <code>name=</code> / <code>rename=</code> to control the new column names.</li>
<li>Sort first: <code>proc sort data=wide; by usubjid;</code> &mdash; PROC TRANSPOSE expects BY variables to be sorted.</li>
<li><code>proc transpose data=wide out=long(rename=(col1=lbstresn)) name=lbtestcd;</code> names the input and the output, and tells SAS how to label the new key/value columns.</li>
<li><code>name=lbtestcd</code> renames the automatic "_NAME_" column &mdash; the one that holds the old column names &mdash; to <code>lbtestcd</code>.</li>
<li><code>rename=(col1=lbstresn)</code> renames the automatic <code>COL1</code> column (the one that holds the stacked values) to <code>lbstresn</code>.</li>
<li><code>by usubjid;</code> keeps the subject identifier fixed across the stacked rows, so each subject appears once per lab test.</li>
<li><code>var HGB ALT AST;</code> names the columns whose values we want to stack into the long layout.</li>
<li>After running, LONG has one row per <code>usubjid</code> per lab test, with columns <code>usubjid</code>, <code>lbtestcd</code>, and <code>lbstresn</code>.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-tidyverse">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-tidyverse" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (tidyverse)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">wide&lt;-tribble( 
~usubjid,~HGB,~ALT,~AST, 
&quot;1001&quot;,13,30,23, 
&quot;1002&quot;,12,28,15, 
) 
...
<li>In tidyverse, <code>pivot_longer()</code> from tidyr is the direct counterpart of PROC TRANSPOSE for wide-to-long reshaping.</li>
<li><code>cols = c(HGB, ALT, AST)</code> names the columns to stack; the remaining columns stay fixed across the stacked rows.</li>
<li><code>names_to = "lbtestcd"</code> names the new column that will hold the old column names &mdash; HGB, ALT, AST become values in <code>lbtestcd</code>.</li>
<li><code>values_to = "lbstresn"</code> names the new column that will hold the values from the selected columns.</li>
<li>For columns with a naming pattern, <code>pivot_longer()</code> also accepts <code>names_pattern</code> or <code>names_sep</code> to split a single name into multiple key columns.</li>
<li>We run and compare: LONG should have one row per <code>usubjid</code> per lab test, matching the SAS PROC TRANSPOSE output row for row.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">wide &lt;- data.frame(
  usubjid = c(1001, 1002),
  HGB = c(13, 12),
  ALT = c(30, 28),
  AST = c(23, 15)
...
row.names(long) &lt;- NULL</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>Base R handles wide-to-long with <code>reshape(..., direction = "long")</code> &mdash; the same function as long-to-wide, just with a different direction.</li>
<li><code>varying = c("HGB", "ALT", "AST")</code> lists the wide columns that will be stacked into rows.</li>
<li><code>v.names = "lbstresn"</code> names the column in the long output that will hold the stacked values.</li>
<li><code>timevar = "lbtestcd"</code> names the column that identifies which measurement each value belongs to.</li>
<li><code>times = c("HGB", "ALT", "AST")</code> supplies the actual labels that populate <code>lbtestcd</code> &mdash; matching the column names listed in <code>varying</code>.</li>
<li><code>idvar = "usubjid"</code> names the subject identifier; it stays fixed across the stacked rows.</li>
<li><code>row.names(long) &lt;- NULL</code> resets auto-generated row names so the final data frame has clean sequential numbering.</li>
<li>We run and confirm the shape matches the tidyverse and SAS outputs &mdash; same rows, same values.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data WIDE;\n infile datalines dlm='|' dsd missover;\ninput usubjid : $4. HGB : best32. ALT : best32. AST : best32.;\nlabel ;\n format ;\n datalines4;\n1001|13|30|23\n1002|12|28|15\n;;;;\n run;\n\nproc sort data=wide;\n     by usubjid;\n run;\n\nproc transpose data=wide\nout=long(rename=(col1=lbstresn))\nname=lbtestcd ;\n    by usubjid;\n    var HGB ALT AST;\n run;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data WIDE;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput usubjid : $4. HGB : best32. ALT : best32. AST : best32.;\nlabel ;\n format ;\n datalines4;\n1001|13|30|23\n1002|12|28|15\n;;;;\n run;\n\nproc sort data=wide;\n     by usubjid;\n run;\n\nproc transpose data=wide\nout=long(rename=(col1=lbstresn))\nname=lbtestcd ;\n    by usubjid;\n    var HGB ALT AST;\n run;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"wide<-tribble( \r\n~usubjid,~HGB,~ALT,~AST, \r\n\"1001\",13,30,23, \r\n\"1002\",12,28,15, \r\n) \r\n\r\nlong<-pivot_longer(wide, \r\ncols=c(HGB,ALT,AST), \r\nnames_to = \"lbtestcd\", \r\nvalues_to = \"lbstresn\"\r\n)","language":"r","html":"<pre><code class=\"language-r\">wide&lt;-tribble( \r\n~usubjid,~HGB,~ALT,~AST, \r\n&quot;1001&quot;,13,30,23, \r\n&quot;1002&quot;,12,28,15, \r\n) \r\n\r\nlong&lt;-pivot_longer(wide, \r\ncols=c(HGB,ALT,AST), \r\nnames_to = &quot;lbtestcd&quot;, \r\nvalues_to = &quot;lbstresn&quot;\r\n)<\/code><\/pre>"},{"label":"R (base)","code":"wide <- data.frame(\r\n  usubjid = c(1001, 1002),\r\n  HGB = c(13, 12),\r\n  ALT = c(30, 28),\r\n  AST = c(23, 15)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nlong <- reshape(\r\n  wide,\r\n  varying = c(\"HGB\", \"ALT\", \"AST\"),\r\n  v.names = \"lbstresn\",\r\n  timevar = \"lbtestcd\",\r\n  times = c(\"HGB\", \"ALT\", \"AST\"),\r\n  idvar = \"usubjid\",\r\n  direction = \"long\"\r\n)\r\n\r\nrow.names(long) <- NULL","language":"r","html":"<pre><code class=\"language-r\">wide &lt;- data.frame(\r\n  usubjid = c(1001, 1002),\r\n  HGB = c(13, 12),\r\n  ALT = c(30, 28),\r\n  AST = c(23, 15)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nlong &lt;- reshape(\r\n  wide,\r\n  varying = c(&quot;HGB&quot;, &quot;ALT&quot;, &quot;AST&quot;),\r\n  v.names = &quot;lbstresn&quot;,\r\n  timevar = &quot;lbtestcd&quot;,\r\n  times = c(&quot;HGB&quot;, &quot;ALT&quot;, &quot;AST&quot;),\r\n  idvar = &quot;usubjid&quot;,\r\n  direction = &quot;long&quot;\r\n)\r\n\r\nrow.names(long) &lt;- NULL<\/code><\/pre>"}] || [];
