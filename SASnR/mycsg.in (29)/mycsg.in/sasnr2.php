...
<title>mycsg SASnR2 2600_Arrays-repeat-same-logic-on-multiple-variables</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Arrays-repeat same logic on multiple variables</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>A common chore: apply the same transformation to several columns at once &mdash; strip a character from every raw date field, convert every numeric column to a character, replace a sentinel value across multiple columns.</li>
<li>Writing the logic once and looping it over a list of columns keeps the code short and easy to extend when new columns arrive.</li>
<li>SAS expresses this with the ARRAY statement inside a DATA step, plus a DO loop indexed by <code>dim()</code>.</li>
<li>Tidyverse uses <code>across()</code> inside <code>mutate()</code>, paired with a tidyselect helper like <code>where()</code> or an explicit column list.</li>
<li>Base R uses <code>lapply()</code> over a column vector, assigning the result back to the same columns of the data frame.</li>
<li>This lesson replaces forward slashes with spaces across three raw date columns on an enrollment dataset.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data ENRLMENT;
...
<li>In SAS, the ARRAY statement lets us name a list of variables once and then refer to them by an index inside a DO loop.</li>
<li><code>array rvars[*] icdt_raw enrldt_raw randdt_raw;</code> creates an array named <code>rvars</code> that points at the three raw date columns; <code>[*]</code> tells SAS to size the array from the variable list.</li>
<li><code>do i = 1 to dim(rvars);</code> loops from 1 to the number of elements in the array &mdash; <code>dim(rvars)</code> is more robust than hard-coding 3.</li>
<li><code>rvars[i] = translate(rvars[i], ' ', '/');</code> runs the TRANSLATE function on the i-th column &mdash; replacing <code>/</code> with a space.</li>
<li>The array members are aliases for the real columns, so assigning back through the array updates the underlying variables in the output dataset.</li>
<li>After running, ENRL01 has the same columns as ENRLMENT, with every <code>/</code> in the raw date columns replaced by a space.</li>
<li>Extending the logic to more columns is a one-line edit &mdash; add names to the ARRAY statement and the loop keeps working.</li>
...
enrl01&lt;-enrlment %&gt;% 
  mutate(across(c(icdt_raw, enrldt_raw, randdt_raw), ~ str_replace_all(., &quot;/&quot;, &quot; &quot;)))</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In tidyverse, <code>across()</code> inside <code>mutate()</code> applies the same function to several columns at once &mdash; the direct counterpart of a SAS ARRAY + DO loop.</li>
<li><code>across(c(icdt_raw, enrldt_raw, randdt_raw), ~ str_replace_all(., "/", " "))</code> selects the three raw date columns and runs the same <code>str_replace_all()</code> call on each.</li>
<li>The <code>~</code> introduces an anonymous function; the <code>.</code> placeholder stands for the current column.</li>
<li>Tidyselect helpers like <code>where(is.character)</code>, <code>starts_with("dt_")</code>, or <code>matches("raw$")</code> make it easy to select columns by pattern instead of listing them one by one.</li>
<li><code>str_replace_all()</code> is from stringr; it replaces every occurrence of the pattern in each value &mdash; perfect for turning every <code>/</code> into a space.</li>
<li>We run and confirm ENRL01 has the forward slashes replaced across the three raw-date columns; other columns should be unchanged.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">enrlment &lt;- data.frame(
...
<li>In base R, the repeat-across-columns idea uses <code>lapply()</code> over a named vector of column names.</li>
<li>We first make a working copy: <code>enrl01 &lt;- enrlment</code>; then define the target columns: <code>cols &lt;- c("icdt_raw", "enrldt_raw", "randdt_raw")</code>.</li>
<li><code>enrl01[cols] &lt;- lapply(enrl01[cols], function(x) gsub("/", " ", x))</code> runs <code>gsub()</code> column by column and assigns the results back to the same columns.</li>
<li><code>enrl01[cols]</code> on the left-hand side writes the returned list of vectors back into those named columns &mdash; the base R idiom for "mutate several columns in place".</li>
<li><code>gsub("/", " ", x)</code> replaces every <code>/</code> in <code>x</code> with a space; it is base R's analogue of <code>str_replace_all()</code>.</li>
<li>We run and compare ENRL01 against the SAS and tidyverse outputs &mdash; the three raw-date columns should have slashes replaced, other columns untouched.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data ENRLMENT;\n infile datalines dlm='|' dsd missover;\ninput STUDY : $6. PT : $4. ICDT_RAW : $11. ENRLDT_RAW : $11. RANDDT_RAW : $11.;\nlabel STUDY ='Study Number' PT ='Subject Identifier' ICDT_RAW ='Informed Consent Date' ENRLDT_RAW ='Enrollment Date' RANDDT_RAW ='Randomization Date';\nformat ;\n datalines4;\nCSG001|1001|1\/JAN\/2010||\nCSG001|1002|1\/JAN\/2010|4\/JAN\/2010|\nCSG001|1003|1\/JAN\/2010|3\/JAN\/2010|3\/JAN\/2010\nCSG001|1004|1\/JAN\/2010|4\/JAN\/2010|5\/JAN\/2010\nCSG001|1005|15\/JAN\/2010|1\/FEB\/2010|5\/FEB\/2010\nCSG001|1006|18\/FEB\/2010|1\/MAR\/2010|1\/MAR\/2010\nCSG001|1007|4\/APR\/2010|14\/APR\/2010|14\/APR\/2010\nCSG001|1008|20\/JUN\/2010|26\/JUN\/2010|27\/JUN\/2010\n;;;;\n run;\n\ndata enrl01;\n     set enrlment;\n     array rvars[*] icdt_raw enrldt_raw randdt_raw;\n     do i=1 to dim(rvars);\n         rvars[i]=translate(rvars[i],' ','\/');\n     end;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data ENRLMENT;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput STUDY : $6. PT : $4. ICDT_RAW : $11. ENRLDT_RAW : $11. RANDDT_RAW : $11.;\nlabel STUDY =&#039;Study Number&#039; PT =&#039;Subject Identifier&#039; ICDT_RAW =&#039;Informed Consent Date&#039; ENRLDT_RAW =&#039;Enrollment Date&#039; RANDDT_RAW =&#039;Randomization Date&#039;;\nformat ;\n datalines4;\nCSG001|1001|1\/JAN\/2010||\nCSG001|1002|1\/JAN\/2010|4\/JAN\/2010|\nCSG001|1003|1\/JAN\/2010|3\/JAN\/2010|3\/JAN\/2010\nCSG001|1004|1\/JAN\/2010|4\/JAN\/2010|5\/JAN\/2010\nCSG001|1005|15\/JAN\/2010|1\/FEB\/2010|5\/FEB\/2010\nCSG001|1006|18\/FEB\/2010|1\/MAR\/2010|1\/MAR\/2010\nCSG001|1007|4\/APR\/2010|14\/APR\/2010|14\/APR\/2010\nCSG001|1008|20\/JUN\/2010|26\/JUN\/2010|27\/JUN\/2010\n;;;;\n run;\n\ndata enrl01;\n     set enrlment;\n     array rvars[*] icdt_raw enrldt_raw randdt_raw;\n     do i=1 to dim(rvars);\n         rvars[i]=translate(rvars[i],&#039; &#039;,&#039;\/&#039;);\n     end;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\r\nlibrary(stringr)\r\nlibrary(purrr)\r\n\r\nenrlment<-tribble(\r\n~study,~pt,~icdt_raw,~enrldt_raw,~randdt_raw,\r\n\"CSG001\",\"1001\",\"1\/JAN\/2010\",\"\",\"\",\r\n\"CSG001\",\"1002\",\"1\/JAN\/2010\",\"4\/JAN\/2010\",\"\",\r\n\"CSG001\",\"1003\",\"1\/JAN\/2010\",\"3\/JAN\/2010\",\"3\/JAN\/2010\",\r\n\"CSG001\",\"1004\",\"1\/JAN\/2010\",\"4\/JAN\/2010\",\"5\/JAN\/2010\",\r\n\"CSG001\",\"1005\",\"15\/JAN\/2010\",\"1\/FEB\/2010\",\"5\/FEB\/2010\",\r\n\"CSG001\",\"1006\",\"18\/FEB\/2010\",\"1\/MAR\/2010\",\"1\/MAR\/2010\",\r\n\"CSG001\",\"1007\",\"4\/APR\/2010\",\"14\/APR\/2010\",\"14\/APR\/2010\",\r\n\"CSG001\",\"1008\",\"20\/JUN\/2010\",\"26\/JUN\/2010\",\"27\/JUN\/2010\",\r\n)\r\n\r\nenrl01<-enrlment %>% \r\n  mutate(across(c(icdt_raw, enrldt_raw, randdt_raw), ~ str_replace_all(., \"\/\", \" \")))","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\r\nlibrary(stringr)\r\nlibrary(purrr)\r\n\r\nenrlment&lt;-tribble(\r\n~study,~pt,~icdt_raw,~enrldt_raw,~randdt_raw,\r\n&quot;CSG001&quot;,&quot;1001&quot;,&quot;1\/JAN\/2010&quot;,&quot;&quot;,&quot;&quot;,\r\n&quot;CSG001&quot;,&quot;1002&quot;,&quot;1\/JAN\/2010&quot;,&quot;4\/JAN\/2010&quot;,&quot;&quot;,\r\n&quot;CSG001&quot;,&quot;1003&quot;,&quot;1\/JAN\/2010&quot;,&quot;3\/JAN\/2010&quot;,&quot;3\/JAN\/2010&quot;,\r\n&quot;CSG001&quot;,&quot;1004&quot;,&quot;1\/JAN\/2010&quot;,&quot;4\/JAN\/2010&quot;,&quot;5\/JAN\/2010&quot;,\r\n&quot;CSG001&quot;,&quot;1005&quot;,&quot;15\/JAN\/2010&quot;,&quot;1\/FEB\/2010&quot;,&quot;5\/FEB\/2010&quot;,\r\n&quot;CSG001&quot;,&quot;1006&quot;,&quot;18\/FEB\/2010&quot;,&quot;1\/MAR\/2010&quot;,&quot;1\/MAR\/2010&quot;,\r\n&quot;CSG001&quot;,&quot;1007&quot;,&quot;4\/APR\/2010&quot;,&quot;14\/APR\/2010&quot;,&quot;14\/APR\/2010&quot;,\r\n&quot;CSG001&quot;,&quot;1008&quot;,&quot;20\/JUN\/2010&quot;,&quot;26\/JUN\/2010&quot;,&quot;27\/JUN\/2010&quot;,\r\n)\r\n\r\nenrl01&lt;-enrlment %&gt;% \r\n  mutate(across(c(icdt_raw, enrldt_raw, randdt_raw), ~ str_replace_all(., &quot;\/&quot;, &quot; &quot;)))<\/code><\/pre>"},{"label":"R (base)","code":"enrlment <- data.frame(\r\n  study = c(\"CSG001\", \"CSG001\", \"CSG001\", \"CSG001\", \"CSG001\", \"CSG001\", \"CSG001\", \"CSG001\"),\r\n  pt = c(1001, 1002, 1003, 1004, 1005, 1006, 1007, 1008),\r\n  icdt_raw = c(\"1\/JAN\/2010\", \"1\/JAN\/2010\", \"1\/JAN\/2010\", \"1\/JAN\/2010\", \"15\/JAN\/2010\", \"18\/FEB\/2010\", \"4\/APR\/2010\", \"20\/JUN\/2010\"),\r\n  enrldt_raw = c(\"\", \"4\/JAN\/2010\", \"3\/JAN\/2010\", \"4\/JAN\/2010\", \"1\/FEB\/2010\", \"1\/MAR\/2010\", \"14\/APR\/2010\", \"26\/JUN\/2010\"),\r\n  randdt_raw = c(\"\", \"\", \"3\/JAN\/2010\", \"5\/JAN\/2010\", \"5\/FEB\/2010\", \"1\/MAR\/2010\", \"14\/APR\/2010\", \"27\/JUN\/2010\")\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nenrl01 <- enrlment\r\n\r\ncols <- c(\"icdt_raw\", \"enrldt_raw\", \"randdt_raw\")\r\n\r\nenrl01[cols] <- lapply(\r\n  enrl01[cols],\r\n  function(x) gsub(\"\/\", \" \", x)\r\n)","language":"r","html":"<pre><code class=\"language-r\">enrlment &lt;- data.frame(\r\n  study = c(&quot;CSG001&quot;, &quot;CSG001&quot;, &quot;CSG001&quot;, &quot;CSG001&quot;, &quot;CSG001&quot;, &quot;CSG001&quot;, &quot;CSG001&quot;, &quot;CSG001&quot;),\r\n  pt = c(1001, 1002, 1003, 1004, 1005, 1006, 1007, 1008),\r\n  icdt_raw = c(&quot;1\/JAN\/2010&quot;, &quot;1\/JAN\/2010&quot;, &quot;1\/JAN\/2010&quot;, &quot;1\/JAN\/2010&quot;, &quot;15\/JAN\/2010&quot;, &quot;18\/FEB\/2010&quot;, &quot;4\/APR\/2010&quot;, &quot;20\/JUN\/2010&quot;),\r\n  enrldt_raw = c(&quot;&quot;, &quot;4\/JAN\/2010&quot;, &quot;3\/JAN\/2010&quot;, &quot;4\/JAN\/2010&quot;, &quot;1\/FEB\/2010&quot;, &quot;1\/MAR\/2010&quot;, &quot;14\/APR\/2010&quot;, &quot;26\/JUN\/2010&quot;),\r\n  randdt_raw = c(&quot;&quot;, &quot;&quot;, &quot;3\/JAN\/2010&quot;, &quot;5\/JAN\/2010&quot;, &quot;5\/FEB\/2010&quot;, &quot;1\/MAR\/2010&quot;, &quot;14\/APR\/2010&quot;, &quot;27\/JUN\/2010&quot;)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nenrl01 &lt;- enrlment\r\n\r\ncols &lt;- c(&quot;icdt_raw&quot;, &quot;enrldt_raw&quot;, &quot;randdt_raw&quot;)\r\n\r\nenrl01[cols] &lt;- lapply(\r\n  enrl01[cols],\r\n  function(x) gsub(&quot;\/&quot;, &quot; &quot;, x)\r\n)<\/code><\/pre>"}] || [];
