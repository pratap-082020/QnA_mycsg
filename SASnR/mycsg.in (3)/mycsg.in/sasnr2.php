...
<title>mycsg SASnR2 1000_Create_Sample_Data</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Create Sample Data</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Before we can run any analysis, we need data in memory. In a teaching setting, the quickest way is to build a tiny dataset inline inside the program itself &mdash; no external files, no study database.</li>
<li>SAS programmers typically do this with a <code>DATA</code> step plus <code>datalines</code>. R offers two natural alternatives: <code>tribble()</code> for a readable row-wise form and <code>data.frame()</code> for a column-wise form.</li>
<li>This lesson shows the same 3-row CLASS dataset built in each environment so we can compare the syntax side by side.</li>
<li>After running any of the three, we should have a table with columns Name, Sex, Age, Height and Weight, and three observations representing Alfred, Alice and Barbara.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code>data CLASS;
...
<li>To create an inline dataset in SAS, we use a <code>DATA</code> step followed by <code>infile datalines</code> and an <code>input</code> statement describing each variable.</li>
<li><code>dlm='|'</code> tells SAS the values are pipe-separated. <code>dsd</code> activates delimiter-sensitive data handling so empty fields are treated as missing. <code>missover</code> prevents SAS from reading past the end of a short line.</li>
<li>The <code>input</code> statement declares each variable with its informat &mdash; <code>$8.</code> for an 8-character name, <code>$1.</code> for sex, <code>best32.</code> for flexible numeric reads.</li>
<li>After the <code>datalines;</code> statement we list the raw rows, terminated by a bare semicolon.</li>
<li><code>run;</code> executes the step and writes the resulting CLASS dataset to the WORK library.</li>
...
<li>To build a small dataset inline in tidyverse, we use <code>tribble()</code> &mdash; a row-wise constructor that reads top-to-bottom the same way we would write it on paper.</li>
<li>Column names come first, each prefixed with <code>~</code> to mark them as tibble variables.</li>
<li>Each subsequent line of values becomes one observation. Commas separate fields within a row; the structure is far more scannable than the equivalent <code>data.frame()</code> call.</li>
<li><code>tribble()</code> is especially handy for writing unit tests and teaching examples where we care about readability more than raw performance.</li>
<li>The result is a tibble &mdash; tidyverse&rsquo;s drop-in replacement for a data frame, with nicer printing and stricter subset behaviour.</li>
...
<li>To build the same dataset in base R, we call <code>data.frame()</code> with one named vector per column.</li>
<li>Each <code>c(...)</code> vector supplies every row&rsquo;s value for that column. The vectors must all be the same length &mdash; one slot per observation.</li>
<li>We pass <code>stringsAsFactors = FALSE</code> so character columns stay as character. Without it, older versions of R would auto-convert them to factors, which often surprises us downstream.</li>
<li>The column-wise shape is how <code>data.frame()</code> naturally works. It is more verbose than <code>tribble()</code> for small examples but involves zero extra packages.</li>
<li>The result is a data frame with the same five columns and three rows as the SAS and tidyverse versions.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data CLASS;\n  infile datalines dlm='|' dsd missover;\n  input Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\n  datalines;\n  Alfred|M|14|69|112.5\n  Alice|F|13|56.5|84\n  Barbara|F|13|65.3|98\n;\nrun;\n","language":"sas","html":"<pre><code>data CLASS;\n  infile datalines dlm='|' dsd missover;\n  input Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\n  datalines;\n  Alfred|M|14|69|112.5\n  Alice|F|13|56.5|84\n  Barbara|F|13|65.3|98\n;\nrun;\n<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\n\nclass <- tribble(\n  ~Name, ~Sex, ~Age, ~Height, ~Weight,\n  \"Alfred\", \"M\", 14, 69, 112.5,\n  \"Alice\",  \"F\", 13, 56.5, 84,\n  \"Barbara\", \"F\", 13, 65.3, 98\n)","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\n\nclass &lt;- tribble(\n  ~Name, ~Sex, ~Age, ~Height, ~Weight,\n  &quot;Alfred&quot;, &quot;M&quot;, 14, 69, 112.5,\n  &quot;Alice&quot;,  &quot;F&quot;, 13, 56.5, 84,\n  &quot;Barbara&quot;, &quot;F&quot;, 13, 65.3, 98\n)<\/code><\/pre>"},{"label":"R (base)","code":"class <- data.frame(\n  Name = c(\"Alfred\", \"Alice\", \"Barbara\"),\n  Sex = c(\"M\", \"F\", \"F\"),\n  Age = c(14, 13, 13),\n  Height = c(69, 56.5, 65.3),\n  Weight = c(112.5, 84, 98)\n  , stringsAsFactors = FALSE\n)","language":"r","html":"<pre><code class=\"language-r\">class &lt;- data.frame(\n  Name = c(&quot;Alfred&quot;, &quot;Alice&quot;, &quot;Barbara&quot;),\n  Sex = c(&quot;M&quot;, &quot;F&quot;, &quot;F&quot;),\n  Age = c(14, 13, 13),\n  Height = c(69, 56.5, 65.3),\n  Weight = c(112.5, 84, 98)\n  , stringsAsFactors = FALSE\n)<\/code><\/pre>"}] || [];
