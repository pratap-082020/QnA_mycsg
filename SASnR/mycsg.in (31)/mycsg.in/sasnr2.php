...
<title>mycsg SASnR2 2611_Arrays---Example---replace-a-value-in-all-numeric-variables</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Arrays - Example - replace a value in all numeric variables</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>The numeric counterpart of the previous lesson: replace a sentinel number (say, 9909 for "not done") with 0 across every numeric column.</li>
<li>As before, we write the rule once and apply it to all numeric columns so the code is short and resilient as new columns appear.</li>
<li>SAS uses an ARRAY with the shortcut <code>_numeric_</code> to target every numeric variable in one line.</li>
<li>Tidyverse uses <code>mutate(across(where(is.numeric), ...))</code> plus <code>if_else()</code> for the replacement.</li>
<li>Base R uses <code>sapply(df, is.numeric)</code> to build a numeric-column mask, then <code>lapply()</code> for the per-column replacement.</li>
<li>This lesson replaces every 9909 in the sample data with 0 across all five numeric columns.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data numericdata;
...
<li>In SAS, <code>_numeric_</code> is the reserved keyword that stands for "every numeric variable in the current dataset".</li>
<li><code>array xnum[*] _numeric_;</code> builds an array pointing at every numeric column &mdash; no need to list them by name.</li>
<li><code>do i = 1 to dim(xnum);</code> loops over each element; <code>dim()</code> returns the number of numeric columns automatically.</li>
<li><code>if xnum[i] = 9909 then xnum[i] = 0;</code> compares the current column's value to the sentinel and, when it matches, writes 0 in its place.</li>
<li><code>drop i;</code> removes the loop index variable from the final dataset so only the data columns remain.</li>
<li>We run and open NUMERICDATA01 &mdash; every 9909 across the numeric columns should now be 0; other values are untouched.</li>
<li>The same ARRAY pattern scales: adding new numeric columns upstream requires no change to this code.</li>
...
numericdata01 &lt;- numericdata %&gt;%
  mutate(across(where(is.numeric), ~ if_else(. == 9909, 0, .)))</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In tidyverse, <code>across(where(is.numeric), ~ if_else(. == 9909, 0, .))</code> applies the same replacement across every numeric column.</li>
<li><code>where(is.numeric)</code> is the tidyselect helper that picks columns based on a type check &mdash; the analogue of <code>_numeric_</code> in SAS.</li>
<li>The <code>~</code> introduces an anonymous function; <code>.</code> stands for the current column's values.</li>
<li><code>if_else(. == 9909, 0, .)</code> returns 0 where the value equals the sentinel and keeps the original value everywhere else.</li>
<li><code>if_else()</code> is strict about types &mdash; both sides must be numeric here, which they are, so the call is safe.</li>
<li>We run and inspect NUMERICDATA01 &mdash; every 9909 should now be 0, with no change to values that were not the sentinel.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">numericdata &lt;- data.frame(
  var1 = c(9909, 5, 9),
  var2 = c(1, 9909, 3),
  var3 = c(2, 6, 9909),
  var4 = c(3, 7, 2),
...
num_cols &lt;- sapply(numericdata01, is.numeric)

numericdata01[num_cols] &lt;- lapply(
  numericdata01[num_cols],
  function(x) ifelse(x == 9909, 0, x)
)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, we build a logical mask of numeric columns with <code>num_cols &lt;- sapply(df, is.numeric)</code>.</li>
<li><code>sapply()</code> runs <code>is.numeric()</code> on each column and returns a named logical vector &mdash; TRUE for numeric columns, FALSE otherwise.</li>
<li><code>df[num_cols] &lt;- lapply(df[num_cols], function(x) ifelse(x == 9909, 0, x))</code> runs the replacement column by column and writes the results back into the same columns.</li>
<li>Inside the helper, <code>ifelse(x == 9909, 0, x)</code> returns 0 where the value equals the sentinel and keeps the original value elsewhere.</li>
<li>Base <code>ifelse()</code> returns a vector, which <code>lapply()</code> collects into a list and assigns back through <code>df[num_cols]</code>.</li>
<li>We run and compare NUMERICDATA01 against the tidyverse and SAS outputs &mdash; every 9909 should now be 0 across the numeric columns.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data numericdata;\n infile datalines dlm='|' dsd missover;\ninput var1 : best32. var2 : best32. var3 : best32. var4 : best32. var5 : best32.;\nlabel ;\n format ;\n datalines4;\n9909|1|2|3|4\n5|9909|6|7|8\n9|3|9909|2|7\n;;;;\n run;\n\ndata numericdata01;\n     set numericdata;\n\n    array xnum[*] _numeric_;\n\n    do i=1 to dim(xnum);\n         if xnum[i]= 9909 then xnum[i]=0;\n    end;\n\n    drop i;\n run;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data numericdata;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput var1 : best32. var2 : best32. var3 : best32. var4 : best32. var5 : best32.;\nlabel ;\n format ;\n datalines4;\n9909|1|2|3|4\n5|9909|6|7|8\n9|3|9909|2|7\n;;;;\n run;\n\ndata numericdata01;\n     set numericdata;\n\n    array xnum[*] _numeric_;\n\n    do i=1 to dim(xnum);\n         if xnum[i]= 9909 then xnum[i]=0;\n    end;\n\n    drop i;\n run;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\r\n\r\nnumericdata<-tribble(\r\n~var1,~var2,~var3,~var4,~var5,\r\n9909,1,2,3,4,\r\n5,9909,6,7,8,\r\n9,3,9909,2,7,\r\n)\r\n\r\n\r\nnumericdata01 <- numericdata %>%\r\n  mutate(across(where(is.numeric), ~ if_else(. == 9909, 0, .)))","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\r\n\r\nnumericdata&lt;-tribble(\r\n~var1,~var2,~var3,~var4,~var5,\r\n9909,1,2,3,4,\r\n5,9909,6,7,8,\r\n9,3,9909,2,7,\r\n)\r\n\r\n\r\nnumericdata01 &lt;- numericdata %&gt;%\r\n  mutate(across(where(is.numeric), ~ if_else(. == 9909, 0, .)))<\/code><\/pre>"},{"label":"R (base)","code":"numericdata <- data.frame(\r\n  var1 = c(9909, 5, 9),\r\n  var2 = c(1, 9909, 3),\r\n  var3 = c(2, 6, 9909),\r\n  var4 = c(3, 7, 2),\r\n  var5 = c(4, 8, 7),\r\n  stringsAsFactors = FALSE\r\n)\r\n\r\n\r\nnumericdata01 <- numericdata\r\n\r\nnum_cols <- sapply(numericdata01, is.numeric)\r\n\r\nnumericdata01[num_cols] <- lapply(\r\n  numericdata01[num_cols],\r\n  function(x) ifelse(x == 9909, 0, x)\r\n)","language":"r","html":"<pre><code class=\"language-r\">numericdata &lt;- data.frame(\r\n  var1 = c(9909, 5, 9),\r\n  var2 = c(1, 9909, 3),\r\n  var3 = c(2, 6, 9909),\r\n  var4 = c(3, 7, 2),\r\n  var5 = c(4, 8, 7),\r\n  stringsAsFactors = FALSE\r\n)\r\n\r\n\r\nnumericdata01 &lt;- numericdata\r\n\r\nnum_cols &lt;- sapply(numericdata01, is.numeric)\r\n\r\nnumericdata01[num_cols] &lt;- lapply(\r\n  numericdata01[num_cols],\r\n  function(x) ifelse(x == 9909, 0, x)\r\n)<\/code><\/pre>"}] || [];
