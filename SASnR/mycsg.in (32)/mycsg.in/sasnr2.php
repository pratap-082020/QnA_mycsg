...
<title>mycsg SASnR2 2612_Arrays---Example---replace-a-value-in-all-numeric-and-character-variables</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Arrays - Example - replace a value in all numeric and character variables</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>This lesson combines the previous two: run a character-sentinel replacement on every character column and a numeric-sentinel replacement on every numeric column, in one pass.</li>
<li>Two independent arrays &mdash; one for character, one for numeric &mdash; apply their own rule; neither rule touches columns of the wrong type.</li>
<li>SAS declares two ARRAYs in the same DATA step (<code>_character_</code> and <code>_numeric_</code>) and uses two DO loops with different indices.</li>
<li>Tidyverse stacks two <code>across()</code> calls inside a single <code>mutate()</code>, each with its own <code>where()</code> type filter and replacement rule.</li>
<li>Base R builds two logical masks (<code>char_cols</code>, <code>num_cols</code>), each followed by its own <code>lapply()</code> write-back.</li>
<li>This lesson replaces <code>"_missing_"</code> with an empty string in every character column and 9909 with 0 in every numeric column on a mixed-type dataset.</li>
...
<li>In SAS, we keep the two replacements clearly separated by declaring two arrays: one for characters, one for numerics.</li>
<li><code>array xchar[*] _character_;</code> plus a DO loop handles the character replacement; <code>_character_</code> is the shortcut for every character variable.</li>
<li>Inside that loop, <code>if lowcase(xchar[i]) = "_missing_" then xchar[i] = "";</code> clears the sentinel in every character column case-insensitively.</li>
<li><code>array xnum[*] _numeric_;</code> plus a second DO loop handles the numeric replacement; <code>_numeric_</code> is the shortcut for every numeric variable.</li>
<li>Inside that loop, <code>if xnum[j] = 9909 then xnum[j] = 0;</code> swaps the numeric sentinel for 0.</li>
<li>We use two different loop indices (<code>i</code>, <code>j</code>) so the two loops do not interfere with each other.</li>
<li><code>drop i j;</code> removes both loop indices from the final dataset.</li>
<li>We run and confirm ALLDATA01 has every <code>"_missing_"</code> replaced across character columns and every 9909 replaced across numeric columns.</li>
...
<li>In tidyverse, we stack two <code>across()</code> calls inside one <code>mutate()</code> &mdash; each targets a different column type and applies its own rule.</li>
<li><code>across(where(is.character), ~ if_else(tolower(.x) == "_missing_", "", .x))</code> replaces the sentinel string in every character column, case-insensitively.</li>
<li><code>across(where(is.numeric), ~ if_else(.x == 9909, 0, .x))</code> replaces 9909 with 0 in every numeric column.</li>
<li>Using <code>.x</code> instead of <code>.</code> is purely a readability choice &mdash; both refer to the current column's values inside an anonymous tidyverse function.</li>
<li>The two <code>across()</code> calls are independent because their <code>where()</code> filters pick disjoint column sets &mdash; no risk of cross-talk between character and numeric logic.</li>
<li>We run and inspect ALLDATA01 &mdash; the character columns should have their sentinel cleared and the numeric columns should have 9909 replaced with 0.</li>
  ...
  var4 = c(3, 7, 2),
  var5 = c(4, 8, 7),
  var6 = c(&quot;_missing_&quot;, &quot;text4&quot;, &quot;text8&quot;),
  var7 = c(&quot;text1&quot;, &quot;text5&quot;, &quot;_missing_&quot;),
  var8 = c(&quot;text2&quot;, &quot;_missing_&quot;, &quot;text9&quot;),
  var9 = c(&quot;_missing_&quot;, &quot;text6&quot;, &quot;text10&quot;),
  var10 = c(&quot;text3&quot;, &quot;text7&quot;, &quot;text11&quot;)
  , stringsAsFactors = FALSE
)

alldata01 &lt;- alldata

char_cols &lt;- sapply(alldata01, is.character)
num_cols &lt;- sapply(alldata01, is.numeric)

alldata01[char_cols] &lt;- lapply(alldata01[char_cols], function(x) {
  x[tolower(x) == &quot;_missing_&quot;] &lt;- &quot;&quot;
  x
})

alldata01[num_cols] &lt;- lapply(alldata01[num_cols], function(x) {
  x[x == 9909] &lt;- 0
  x
})</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, we build two logical masks up front: <code>char_cols &lt;- sapply(df, is.character)</code> and <code>num_cols &lt;- sapply(df, is.numeric)</code>.</li>
<li>For the character replacement: <code>df[char_cols] &lt;- lapply(df[char_cols], function(x) { x[tolower(x) == "_missing_"] &lt;- ""; x })</code> clears the sentinel across every character column.</li>
<li>Inside that helper, <code>x[tolower(x) == "_missing_"] &lt;- ""</code> uses logical subsetting to overwrite only the matching elements.</li>
<li>For the numeric replacement: <code>df[num_cols] &lt;- lapply(df[num_cols], function(x) { x[x == 9909] &lt;- 0; x })</code> swaps the numeric sentinel for 0.</li>
<li>The two <code>lapply()</code> calls are independent &mdash; <code>char_cols</code> and <code>num_cols</code> pick disjoint columns by construction.</li>
<li>We run and confirm the end-state: character columns have the sentinel cleared, numeric columns have 9909 replaced with 0, everything else unchanged.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data alldata;\n infile datalines dlm='|' dsd missover;\ninput var1 : best32. var2 : best32. var3 : best32. var4 : best32. var5 : best32. var6 : $10. var7 : $10. var8 : $10. var9 : $10. var10 : $10.;\nlabel ;\n format ;\n datalines4;\n9909|1|2|3|4|_missing_|text1|text2|_missing_|text3\n5|9909|6|7|8|text4|text5|_missing_|text6|text7\n9|3|9909|2|7|text8|_missing_|text9|text10|text11\n;;;;\n run;\n\ndata alldata01;\n     set alldata;\n\n    *------------------------------------------------------------------------------;\n    *character variables;\n    *------------------------------------------------------------------------------;\n\n    array xchar[*] _character_;\n\n    do i=1 to dim(xchar);\n         if lowcase(xchar[i])=\"_missing_\" then xchar[i]=\"\";\n    end;\n\n    *------------------------------------------------------------------------------;\n    *numeric variables;\n    *------------------------------------------------------------------------------;\n\n    array xnum[*] _numeric_;\n\n    do j=1 to dim(xnum);\n         if xnum[j]= 9909 then xnum[j]=0;\n    end;\n\n    drop i j;\n\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data alldata;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput var1 : best32. var2 : best32. var3 : best32. var4 : best32. var5 : best32. var6 : $10. var7 : $10. var8 : $10. var9 : $10. var10 : $10.;\nlabel ;\n format ;\n datalines4;\n9909|1|2|3|4|_missing_|text1|text2|_missing_|text3\n5|9909|6|7|8|text4|text5|_missing_|text6|text7\n9|3|9909|2|7|text8|_missing_|text9|text10|text11\n;;;;\n run;\n\ndata alldata01;\n     set alldata;\n\n    *------------------------------------------------------------------------------;\n    *character variables;\n    *------------------------------------------------------------------------------;\n\n    array xchar[*] _character_;\n\n    do i=1 to dim(xchar);\n         if lowcase(xchar[i])=&quot;_missing_&quot; then xchar[i]=&quot;&quot;;\n    end;\n\n    *------------------------------------------------------------------------------;\n    *numeric variables;\n    *------------------------------------------------------------------------------;\n\n    array xnum[*] _numeric_;\n\n    do j=1 to dim(xnum);\n         if xnum[j]= 9909 then xnum[j]=0;\n    end;\n\n    drop i j;\n\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\r\n\r\nalldata<-tribble(\r\n~var1,~var2,~var3,~var4,~var5,~var6,~var7,~var8,~var9,~var10,\r\n9909,1,2,3,4,\"_missing_\",\"text1\",\"text2\",\"_missing_\",\"text3\",\r\n5,9909,6,7,8,\"text4\",\"text5\",\"_missing_\",\"text6\",\"text7\",\r\n9,3,9909,2,7,\"text8\",\"_missing_\",\"text9\",\"text10\",\"text11\",\r\n)\r\n\r\nalldata01 <- alldata %>%\r\n  mutate(\r\n    across(where(is.character), ~ if_else(tolower(.x) == \"_missing_\", \"\", .x)),\r\n    across(where(is.numeric), ~ if_else(.x == 9909, 0, .x))\r\n  )","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\r\n\r\nalldata&lt;-tribble(\r\n~var1,~var2,~var3,~var4,~var5,~var6,~var7,~var8,~var9,~var10,\r\n9909,1,2,3,4,&quot;_missing_&quot;,&quot;text1&quot;,&quot;text2&quot;,&quot;_missing_&quot;,&quot;text3&quot;,\r\n5,9909,6,7,8,&quot;text4&quot;,&quot;text5&quot;,&quot;_missing_&quot;,&quot;text6&quot;,&quot;text7&quot;,\r\n9,3,9909,2,7,&quot;text8&quot;,&quot;_missing_&quot;,&quot;text9&quot;,&quot;text10&quot;,&quot;text11&quot;,\r\n)\r\n\r\nalldata01 &lt;- alldata %&gt;%\r\n  mutate(\r\n    across(where(is.character), ~ if_else(tolower(.x) == &quot;_missing_&quot;, &quot;&quot;, .x)),\r\n    across(where(is.numeric), ~ if_else(.x == 9909, 0, .x))\r\n  )<\/code><\/pre>"},{"label":"R (base)","code":"alldata <- data.frame(\r\n  var1 = c(9909, 5, 9),\r\n  var2 = c(1, 9909, 3),\r\n  var3 = c(2, 6, 9909),\r\n  var4 = c(3, 7, 2),\r\n  var5 = c(4, 8, 7),\r\n  var6 = c(\"_missing_\", \"text4\", \"text8\"),\r\n  var7 = c(\"text1\", \"text5\", \"_missing_\"),\r\n  var8 = c(\"text2\", \"_missing_\", \"text9\"),\r\n  var9 = c(\"_missing_\", \"text6\", \"text10\"),\r\n  var10 = c(\"text3\", \"text7\", \"text11\")\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nalldata01 <- alldata\r\n\r\nchar_cols <- sapply(alldata01, is.character)\r\nnum_cols <- sapply(alldata01, is.numeric)\r\n\r\nalldata01[char_cols] <- lapply(alldata01[char_cols], function(x) {\r\n  x[tolower(x) == \"_missing_\"] <- \"\"\r\n  x\r\n})\r\n\r\nalldata01[num_cols] <- lapply(alldata01[num_cols], function(x) {\r\n  x[x == 9909] <- 0\r\n  x\r\n})","language":"r","html":"<pre><code class=\"language-r\">alldata &lt;- data.frame(\r\n  var1 = c(9909, 5, 9),\r\n  var2 = c(1, 9909, 3),\r\n  var3 = c(2, 6, 9909),\r\n  var4 = c(3, 7, 2),\r\n  var5 = c(4, 8, 7),\r\n  var6 = c(&quot;_missing_&quot;, &quot;text4&quot;, &quot;text8&quot;),\r\n  var7 = c(&quot;text1&quot;, &quot;text5&quot;, &quot;_missing_&quot;),\r\n  var8 = c(&quot;text2&quot;, &quot;_missing_&quot;, &quot;text9&quot;),\r\n  var9 = c(&quot;_missing_&quot;, &quot;text6&quot;, &quot;text10&quot;),\r\n  var10 = c(&quot;text3&quot;, &quot;text7&quot;, &quot;text11&quot;)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nalldata01 &lt;- alldata\r\n\r\nchar_cols &lt;- sapply(alldata01, is.character)\r\nnum_cols &lt;- sapply(alldata01, is.numeric)\r\n\r\nalldata01[char_cols] &lt;- lapply(alldata01[char_cols], function(x) {\r\n  x[tolower(x) == &quot;_missing_&quot;] &lt;- &quot;&quot;\r\n  x\r\n})\r\n\r\nalldata01[num_cols] &lt;- lapply(alldata01[num_cols], function(x) {\r\n  x[x == 9909] &lt;- 0\r\n  x\r\n})<\/code><\/pre>"}] || [];
