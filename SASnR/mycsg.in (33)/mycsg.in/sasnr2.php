...
<title>mycsg SASnR2 2700_Retain_first_value_of_subject_onto_other_records</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Retain first value of a subject onto other records</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>A common derivation in clinical data: capture the first score (baseline, first visit, first entry) for each subject and copy that value onto every other record of the same subject.</li>
<li>The output keeps the same number of rows as the input; each row just gains an extra column holding the subject's first-visit value.</li>
<li>SAS does this in a DATA step using <code>RETAIN</code> plus the <code>first.</code> automatic variable on the BY group.</li>
<li>Tidyverse uses <code>group_by()</code> + <code>fill()</code> (or the shorter <code>first()</code>) to push the first value down through the group.</li>
<li>Base R uses <code>ave()</code> with a custom function that always returns the first element of each group.</li>
<li>This lesson copies each subject's visit-1 score onto visits 2 and 3 so the result is easy to inspect.</li>
...
<li>To carry the first score of each subject forward onto every record, we sort by subject and visit, then use <code>RETAIN</code> with <code>first.usubjid</code>.</li>
<li>SAS uses BY-group processing: after <code>PROC SORT</code> by <code>usubjid visitnum</code> and a <code>by</code> statement in the DATA step, SAS exposes automatic flags <code>first.usubjid</code> and <code>last.usubjid</code>.</li>
<li><code>retain first_score;</code> tells SAS not to reset <code>first_score</code> to missing at the start of each row &mdash; it holds whatever value we assigned on the previous row.</li>
<li><code>if first.usubjid then first_score = score;</code> captures the score from the very first row of each subject. Because of the retain, that value stays in place for every subsequent row of the same subject.</li>
<li>After running, we expect <code>fs01</code> to have all six input rows plus a new <code>first_score</code> column equal to the first visit&rsquo;s score within each subject.</li>
...
fs01 &lt;- fs %&gt;% 
  ...
  fill(first_score,.direction=&quot;down&quot;)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To carry the first score of each subject forward in tidyverse, we arrange, group and then fill the missing values downward.</li>
<li><code>arrange(usubjid, visitnum)</code> is the tidyverse analogue of <code>PROC SORT BY usubjid visitnum</code>.</li>
<li><code>group_by(usubjid)</code> scopes the next operations to each subject &mdash; equivalent to the SAS <code>by usubjid</code>.</li>
<li>Inside <code>mutate()</code>, we use <code>row_number()</code> to tag the first row of each subject. We assign the score to <code>first_score</code> only for the first row; other rows get <code>NA</code>.</li>
<li><code>fill(first_score, .direction = "down")</code> then propagates that first value downward, filling every subsequent NA within the same group &mdash; the tidyverse equivalent of the SAS <code>RETAIN</code>.</li>
<li>After running, we check <code>fs01</code> and confirm <code>first_score</code> equals the first visit&rsquo;s score for every row of the subject.</li>
<li>A shorter alternative is <code>mutate(first_score = first(score))</code>, which gives the same result without the <code>seq</code>/<code>fill</code> dance &mdash; worth keeping in mind for real code.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">fs &lt;- data.frame(
...
fs01$first_score &lt;- ave(fs01$score, fs01$usubjid, FUN = function(x) x[1])</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, we sort first with <code>order(fs$usubjid, fs$visitnum)</code> &mdash; the definition of "first" depends on the sort order.</li>
<li><code>fs01$first_score &lt;- ave(fs01$score, fs01$usubjid, FUN = function(x) x[1])</code> is the one-line version.</li>
<li><code>ave(x, group, FUN = f)</code> runs <code>f</code> on each slice of <code>x</code> defined by <code>group</code>, then lines the result up row-for-row &mdash; perfect for attaching a per-subject value onto every row.</li>
<li>The helper <code>function(x) x[1]</code> simply returns the first element of each subject's score vector, which is then broadcast to every row of that subject.</li>
<li>We run and verify <code>first_score</code> equals the visit-1 score for every row of the same subject, matching the SAS and tidyverse outputs.</li>
<li>A handy sanity check: grouping by <code>usubjid</code> and comparing <code>first_score</code> with the actual first-row <code>score</code> should show them equal on every row.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"*=============================================================================;\n* Create the input dataset;\n*=============================================================================;\n\ndata fs;\n    input usubjid visitnum score;\n    datalines;\n101 1 20\n101 2 10\n101 3 30\n102 1 90\n102 2 10\n102 3 100\n;\nrun;\n\n*=============================================================================;\n* Sort the dataset by usubjid and visitnum;\n*=============================================================================;\n\nproc sort data=fs;\n    by usubjid visitnum;\nrun;\n\n*=============================================================================;\n*Retain the first score onto all records;\n*=============================================================================;\n\ndata fs01;\n    set fs;\n    by usubjid visitnum;\n\n    retain first_score;\n    if first.usubjid then first_score = score;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">*=============================================================================;\n* Create the input dataset;\n*=============================================================================;\n\ndata fs;\n    input usubjid visitnum score;\n    datalines;\n101 1 20\n101 2 10\n101 3 30\n102 1 90\n102 2 10\n102 3 100\n;\nrun;\n\n*=============================================================================;\n* Sort the dataset by usubjid and visitnum;\n*=============================================================================;\n\nproc sort data=fs;\n    by usubjid visitnum;\nrun;\n\n*=============================================================================;\n*Retain the first score onto all records;\n*=============================================================================;\n\ndata fs01;\n    set fs;\n    by usubjid visitnum;\n\n    retain first_score;\n    if first.usubjid then first_score = score;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"#==============================================================================;\r\n#Retain first score;\r\n#==============================================================================;\r\n\r\nfs <- tribble(\r\n  ~usubjid,~visitnum,~score,\r\n  101,1,20,\r\n  101,2,10,\r\n  101,3,30,\r\n  102,1,90,\r\n  102,2,10,\r\n  102,3,100\r\n)\r\n\r\nfs01 <- fs %>% \r\n  arrange(usubjid,visitnum) %>% \r\n  group_by(usubjid) %>% \r\n  mutate(\r\n    seq = row_number(),\r\n    first_score = if_else(seq==1,score,NA)\r\n  ) %>% \r\n  fill(first_score,.direction=\"down\")","language":"r","html":"<pre><code class=\"language-r\">#==============================================================================;\r\n#Retain first score;\r\n#==============================================================================;\r\n\r\nfs &lt;- tribble(\r\n  ~usubjid,~visitnum,~score,\r\n  101,1,20,\r\n  101,2,10,\r\n  101,3,30,\r\n  102,1,90,\r\n  102,2,10,\r\n  102,3,100\r\n)\r\n\r\nfs01 &lt;- fs %&gt;% \r\n  arrange(usubjid,visitnum) %&gt;% \r\n  group_by(usubjid) %&gt;% \r\n  mutate(\r\n    seq = row_number(),\r\n    first_score = if_else(seq==1,score,NA)\r\n  ) %&gt;% \r\n  fill(first_score,.direction=&quot;down&quot;)<\/code><\/pre>"},{"label":"R (base)","code":"fs <- data.frame(\r\n  usubjid = c(101, 101, 101, 102, 102, 102),\r\n  visitnum = c(1, 2, 3, 1, 2, 3),\r\n  score = c(20, 10, 30, 90, 10, 100)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nfs01 <- fs[order(fs$usubjid, fs$visitnum), ]\r\n\r\nfs01$first_score <- ave(fs01$score, fs01$usubjid, FUN = function(x) x[1])","language":"r","html":"<pre><code class=\"language-r\">fs &lt;- data.frame(\r\n  usubjid = c(101, 101, 101, 102, 102, 102),\r\n  visitnum = c(1, 2, 3, 1, 2, 3),\r\n  score = c(20, 10, 30, 90, 10, 100)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nfs01 &lt;- fs[order(fs$usubjid, fs$visitnum), ]\r\n\r\nfs01$first_score &lt;- ave(fs01$score, fs01$usubjid, FUN = function(x) x[1])<\/code><\/pre>"}] || [];
