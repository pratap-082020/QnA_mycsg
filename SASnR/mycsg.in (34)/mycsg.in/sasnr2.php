...
<title>mycsg SASnR2 2701_Retain_last_non_missing_value</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Retain last non-missing value</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Last Observation Carried Forward (LOCF) is a common imputation: when a visit value is missing, substitute the most recent non-missing value from the same subject.</li>
<li>LOCF lets downstream summaries treat the visit as if it had the carried value, and keeps the full row count of the original dataset.</li>
<li>SAS does this with a DATA step that <code>RETAIN</code>s a helper variable and updates it only on non-missing rows.</li>
<li>Tidyverse uses <code>group_by(usubjid) %&gt;% fill(score, .direction = "down")</code> to propagate non-NA values forward within each group.</li>
<li>Base R uses <code>ave()</code> with a small loop that walks each subject's vector and replaces NAs with the last seen non-NA value.</li>
<li>This lesson keeps an <code>orig_score</code> column as a snapshot so we can see where the fill actually happened.</li>
...
<li>To carry the most recent non-missing score forward until a new observed value appears, we use <code>RETAIN</code> together with a <code>missing()</code> check.</li>
<li>We start by sorting the dataset by subject and visit &mdash; SAS needs the BY order so that forward-fill means the same thing on every row.</li>
<li><code>retain last_score;</code> keeps <code>last_score</code> alive across rows instead of resetting to missing at each iteration.</li>
<li>We snapshot the incoming value into <code>orig_score</code> first so we can always see what was there before the fill.</li>
<li>When <code>score</code> is non-missing, we update <code>last_score</code> with it. When it is missing, we replace <code>score</code> with the retained <code>last_score</code> &mdash; the classic "LOCF" (Last Observation Carried Forward) step.</li>
<li>After running, we confirm that missing scores have been replaced with the most recent prior non-missing score for the same subject, and <code>orig_score</code> still shows the original NAs.</li>
...
lnms01 &lt;- lnms %&gt;% 
  ...
  group_by(usubjid) %&gt;% 
  fill(score,.direction=&quot;down&quot;)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To do LOCF in tidyverse, we group by subject, sort by visit, and use <code>fill()</code> to push values downward over missing positions.</li>
<li>We snapshot the incoming score into <code>orig_score</code> before the fill so the original NAs remain visible for audit.</li>
<li><code>arrange(usubjid, visitnum)</code> is the tidyverse analogue of <code>PROC SORT BY usubjid visitnum</code>.</li>
<li><code>group_by(usubjid)</code> scopes the fill to each subject &mdash; we never carry a value from one subject onto another.</li>
<li><code>fill(score, .direction = "down")</code> walks each group top-to-bottom and replaces every NA in <code>score</code> with the most recent prior non-NA value. Same shape as the SAS retain + conditional assignment.</li>
<li>After running, we confirm that <code>lnms01$score</code> has no NAs within a subject where a prior value existed, and that <code>orig_score</code> still marks where the fills happened.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">lnms &lt;- data.frame(
...
<li>In base R, LOCF is a small custom loop tucked inside <code>ave()</code>, so the fill happens per subject rather than across the whole vector.</li>
<li>We sort first with <code>order(lnms$usubjid, lnms$visitnum)</code> so "previous row" has a clear meaning.</li>
<li>We snapshot the original column into <code>orig_score</code> before the fill so the audit trail is preserved.</li>
<li>The helper walks the per-subject vector <code>x</code>: it tracks the last non-NA value in <code>last</code>, and whenever it sees an NA it overwrites it with <code>last</code>.</li>
<li>If a subject's first row is already NA, <code>last</code> stays NA and the row is left as-is &mdash; we cannot carry forward something we have not seen yet.</li>
<li>Passing this helper to <code>ave()</code> runs it once per subject, then lines the filled vector back up row-for-row in the original data frame.</li>
<li>We run and compare <code>lnms01$score</code> against <code>orig_score</code> &mdash; every NA that had a prior non-NA value should now be filled; subjects with all-missing starts remain NA.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"*=============================================================================;\n* Create the input dataset;\n*=============================================================================;\n\ndata lnms;\n    input usubjid visitnum score;\n    datalines;\n101 1 20\n101 2 .\n101 3 30\n102 1 90\n102 2 10\n102 3 .\n102 4 .\n;\nrun;\n\n*=============================================================================;\n* Sort the dataset by usubjid and visitnum;\n*=============================================================================;\nproc sort data=lnms;\n    by usubjid visitnum;\nrun;\n\n*=============================================================================;\n* Retain last non-missing score;\n*=============================================================================;\ndata lnms01;\n    set lnms;\n    by usubjid visitnum;\n\n    retain last_score;\n    * Store the original score;\n    orig_score = score;\n\n    * Update the last non-missing score if present;\n    if not missing(score) then last_score = score;\n\n    * If missing, retain the previous value;\n    if missing(score) then score = last_score;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">*=============================================================================;\n* Create the input dataset;\n*=============================================================================;\n\ndata lnms;\n    input usubjid visitnum score;\n    datalines;\n101 1 20\n101 2 .\n101 3 30\n102 1 90\n102 2 10\n102 3 .\n102 4 .\n;\nrun;\n\n*=============================================================================;\n* Sort the dataset by usubjid and visitnum;\n*=============================================================================;\nproc sort data=lnms;\n    by usubjid visitnum;\nrun;\n\n*=============================================================================;\n* Retain last non-missing score;\n*=============================================================================;\ndata lnms01;\n    set lnms;\n    by usubjid visitnum;\n\n    retain last_score;\n    * Store the original score;\n    orig_score = score;\n\n    * Update the last non-missing score if present;\n    if not missing(score) then last_score = score;\n\n    * If missing, retain the previous value;\n    if missing(score) then score = last_score;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"#==============================================================================;\r\n#Retain last non-missing score;\r\n#==============================================================================;\r\n\r\nlnms <- tribble(\r\n  ~usubjid,~visitnum,~score,\r\n  101,1,20,\r\n  101,2,NA,\r\n  101,3,30,\r\n  102,1,90,\r\n  102,2,10,\r\n  102,3,NA,\r\n  102,4,NA\r\n)\r\n\r\nlnms01 <- lnms %>% \r\n  mutate(orig_score=score) %>% \r\n  arrange(usubjid,visitnum) %>%\r\n  group_by(usubjid) %>% \r\n  fill(score,.direction=\"down\")","language":"r","html":"<pre><code class=\"language-r\">#==============================================================================;\r\n#Retain last non-missing score;\r\n#==============================================================================;\r\n\r\nlnms &lt;- tribble(\r\n  ~usubjid,~visitnum,~score,\r\n  101,1,20,\r\n  101,2,NA,\r\n  101,3,30,\r\n  102,1,90,\r\n  102,2,10,\r\n  102,3,NA,\r\n  102,4,NA\r\n)\r\n\r\nlnms01 &lt;- lnms %&gt;% \r\n  mutate(orig_score=score) %&gt;% \r\n  arrange(usubjid,visitnum) %&gt;%\r\n  group_by(usubjid) %&gt;% \r\n  fill(score,.direction=&quot;down&quot;)<\/code><\/pre>"},{"label":"R (base)","code":"lnms <- data.frame(\r\n  usubjid = c(101, 101, 101, 102, 102, 102, 102),\r\n  visitnum = c(1, 2, 3, 1, 2, 3, 4),\r\n  score = c(20, NA, 30, 90, 10, NA, NA)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nlnms01 <- lnms[order(lnms$usubjid, lnms$visitnum), ]\r\nlnms01$orig_score <- lnms01$score\r\n\r\nlnms01$score <- ave(lnms01$score, lnms01$usubjid, FUN = function(x) {\r\n  last <- NA\r\n  for (i in seq_along(x)) {\r\n    if (!is.na(x[i])) {\r\n      last <- x[i]\r\n    } else if (!is.na(last)) {\r\n      x[i] <- last\r\n    }\r\n  }\r\n  x\r\n})","language":"r","html":"<pre><code class=\"language-r\">lnms &lt;- data.frame(\r\n  usubjid = c(101, 101, 101, 102, 102, 102, 102),\r\n  visitnum = c(1, 2, 3, 1, 2, 3, 4),\r\n  score = c(20, NA, 30, 90, 10, NA, NA)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nlnms01 &lt;- lnms[order(lnms$usubjid, lnms$visitnum), ]\r\nlnms01$orig_score &lt;- lnms01$score\r\n\r\nlnms01$score &lt;- ave(lnms01$score, lnms01$usubjid, FUN = function(x) {\r\n  last &lt;- NA\r\n  for (i in seq_along(x)) {\r\n    if (!is.na(x[i])) {\r\n      last &lt;- x[i]\r\n    } else if (!is.na(last)) {\r\n      x[i] &lt;- last\r\n    }\r\n  }\r\n  x\r\n})<\/code><\/pre>"}] || [];
