...
<title>mycsg SASnR2 2703_Retain_maximum_score_upto_current_observation</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Retain maximum score upto the current observation</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>The mirror of the running-minimum lesson: carry forward the largest score seen so far within each subject.</li>
<li>Running maximums show up in worst-observed-value summaries on the high side &mdash; highest lab value observed to date, peak pain score, and similar clinical derivations.</li>
<li>As with the minimum, the running maximum resets at the start of each subject so no values leak across subjects.</li>
<li>SAS uses <code>RETAIN</code> with <code>max(score, maxscore)</code>, resetting on <code>first.usubjid</code>.</li>
<li>Tidyverse uses <code>group_by()</code> + <code>cummax()</code>, with <code>-Inf</code> as the NA placeholder so missing values never win the maximum.</li>
<li>Base R uses <code>ave()</code> with a helper that masks NAs as <code>-Inf</code> and calls <code>cummax()</code> within each subject.</li>
...
<li>This is the mirror image of the running-minimum pattern &mdash; we carry forward the largest score seen so far within each subject.</li>
<li>After sorting by subject and visit, we reset <code>maxscore</code> at the start of each subject with <code>if first.usubjid then maxscore = .;</code>.</li>
<li>On every non-missing row, we update <code>maxscore = max(score, maxscore)</code>. SAS treats missing <code>maxscore</code> as smaller than any real number, so the first non-missing score becomes the starting maximum naturally.</li>
<li>On missing-score rows we substitute <code>score = maxscore</code>, which fills the NA with the running high.</li>
<li>We keep <code>orig_score</code> so the original NAs remain visible after the fill.</li>
<li>After running, we verify <code>maxscore</code> never decreases within a subject and missing rows now carry the running maximum.</li>
...
<li>The tidyverse version mirrors the minimum case &mdash; same structure, opposite comparison.</li>
<li>We use <code>cummax()</code> to get the running maximum within each group.</li>
<li><code>cummax()</code> also does not handle NAs the way SAS does, so we substitute them with <code>-Inf</code> in a temporary column first. <code>-Inf</code> is smaller than every real score, so it never wins the maximum.</li>
<li><code>group_by(usubjid)</code> resets the cumulative maximum at each subject boundary &mdash; our analogue of <code>if first.usubjid then maxscore = .;</code>.</li>
<li>Then we assign <code>maxscore</code> into <code>score</code> only where the original value was NA, preserving real observations.</li>
<li>After running, we confirm that within each subject <code>maxscore</code> is monotonically non-decreasing and the filled rows show the running high.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">pmax &lt;- data.frame(
...
pmax01$score &lt;- ifelse(is.na(pmax01$orig_score), pmax01$maxscore, pmax01$score)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, the running maximum mirrors the minimum pattern &mdash; same structure, opposite comparison and opposite NA mask.</li>
<li><code>order(pmax$usubjid, pmax$visitnum)</code> gives us a stable visit-by-visit sequence within each subject.</li>
<li>We snapshot <code>orig_score</code> before overwriting, so we can tell which rows were filled.</li>
<li><code>ave(pmax01$score, pmax01$usubjid, FUN = function(x) { temp &lt;- ifelse(is.na(x), -Inf, x); cummax(temp) })</code> computes the running maximum within each subject.</li>
<li><code>-Inf</code> is smaller than every real score, so missing values never win the running maximum.</li>
<li><code>cummax()</code> returns the running maximum of the adjusted vector; NAs are effectively ignored by the mask.</li>
<li>Finally, <code>pmax01$score &lt;- ifelse(is.na(pmax01$orig_score), pmax01$maxscore, pmax01$score)</code> fills only the rows whose original score was NA.</li>
<li>We run and verify the per-subject <code>maxscore</code> is monotonically non-decreasing and the filled rows reflect the running high.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"*==============================================================================;\n* Create the input dataset;\n*==============================================================================;\n\ndata pmax;\n    input usubjid visitnum score;\n    datalines;\n101 1 20\n101 2 .\n101 3 30\n102 1 90\n102 2 10\n102 3 .\n102 4 91\n102 5 .\n;\nrun;\n\n*==============================================================================;\n* Sort the dataset by usubjid and visitnum;\n*==============================================================================;\n\nproc sort data=pmax;\n    by usubjid visitnum;\nrun;\n\n*==============================================================================;\n* Retain previous non-missing low score;\n*==============================================================================;\n\ndata pmax01;\n    set pmax;\n    by usubjid visitnum;\n\n    retain maxscore;\n\n    * Store the original score;\n    orig_score = score;\n\n    if first.usubjid then maxscore = .;\n\n    *Store maximum score and assign maxscore when score is missing;\n    if not missing(score) then maxscore = max(score,maxscore);\n    else score=maxscore;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">*==============================================================================;\n* Create the input dataset;\n*==============================================================================;\n\ndata pmax;\n    input usubjid visitnum score;\n    datalines;\n101 1 20\n101 2 .\n101 3 30\n102 1 90\n102 2 10\n102 3 .\n102 4 91\n102 5 .\n;\nrun;\n\n*==============================================================================;\n* Sort the dataset by usubjid and visitnum;\n*==============================================================================;\n\nproc sort data=pmax;\n    by usubjid visitnum;\nrun;\n\n*==============================================================================;\n* Retain previous non-missing low score;\n*==============================================================================;\n\ndata pmax01;\n    set pmax;\n    by usubjid visitnum;\n\n    retain maxscore;\n\n    * Store the original score;\n    orig_score = score;\n\n    if first.usubjid then maxscore = .;\n\n    *Store maximum score and assign maxscore when score is missing;\n    if not missing(score) then maxscore = max(score,maxscore);\n    else score=maxscore;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"#==============================================================================;\r\n#Retain previous non-missing high score;\r\n#==============================================================================;\r\n\r\npmax <- tribble(\r\n  ~usubjid,~visitnum,~score,\r\n  101,1,20,\r\n  101,2,NA,\r\n  101,3,30,\r\n  102,1,90,\r\n  102,2,10,\r\n  102,3,NA,\r\n  102,4,91,\r\n  102,5,NA\r\n)\r\n\r\n\r\npmax01 <- pmax %>%\r\n  arrange(usubjid,visitnum) %>% \r\n  group_by(usubjid) %>% \r\n  mutate(\r\n    orig_score = score,\r\n    temp_score = if_else(is.na(score),-Inf,score),\r\n    maxscore=cummax(temp_score),\r\n    score = if_else(is.na(score),maxscore,score)\r\n  )","language":"r","html":"<pre><code class=\"language-r\">#==============================================================================;\r\n#Retain previous non-missing high score;\r\n#==============================================================================;\r\n\r\npmax &lt;- tribble(\r\n  ~usubjid,~visitnum,~score,\r\n  101,1,20,\r\n  101,2,NA,\r\n  101,3,30,\r\n  102,1,90,\r\n  102,2,10,\r\n  102,3,NA,\r\n  102,4,91,\r\n  102,5,NA\r\n)\r\n\r\n\r\npmax01 &lt;- pmax %&gt;%\r\n  arrange(usubjid,visitnum) %&gt;% \r\n  group_by(usubjid) %&gt;% \r\n  mutate(\r\n    orig_score = score,\r\n    temp_score = if_else(is.na(score),-Inf,score),\r\n    maxscore=cummax(temp_score),\r\n    score = if_else(is.na(score),maxscore,score)\r\n  )<\/code><\/pre>"},{"label":"R (base)","code":"pmax <- data.frame(\r\n  usubjid = c(101, 101, 101, 102, 102, 102, 102, 102),\r\n  visitnum = c(1, 2, 3, 1, 2, 3, 4, 5),\r\n  score = c(20, NA, 30, 90, 10, NA, 91, NA)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\npmax01 <- pmax[order(pmax$usubjid, pmax$visitnum), ]\r\n\r\npmax01$orig_score <- pmax01$score\r\n\r\npmax01$maxscore <- ave(pmax01$score, pmax01$usubjid, FUN = function(x) {\r\n  temp <- ifelse(is.na(x), -Inf, x)\r\n  cummax(temp)\r\n})\r\n\r\npmax01$score <- ifelse(is.na(pmax01$orig_score), pmax01$maxscore, pmax01$score)","language":"r","html":"<pre><code class=\"language-r\">pmax &lt;- data.frame(\r\n  usubjid = c(101, 101, 101, 102, 102, 102, 102, 102),\r\n  visitnum = c(1, 2, 3, 1, 2, 3, 4, 5),\r\n  score = c(20, NA, 30, 90, 10, NA, 91, NA)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\npmax01 &lt;- pmax[order(pmax$usubjid, pmax$visitnum), ]\r\n\r\npmax01$orig_score &lt;- pmax01$score\r\n\r\npmax01$maxscore &lt;- ave(pmax01$score, pmax01$usubjid, FUN = function(x) {\r\n  temp &lt;- ifelse(is.na(x), -Inf, x)\r\n  cummax(temp)\r\n})\r\n\r\npmax01$score &lt;- ifelse(is.na(pmax01$orig_score), pmax01$maxscore, pmax01$score)<\/code><\/pre>"}] || [];
