...
<title>mycsg SASnR2 2705_Retain_Concatenate_character_results_into_a_single_value</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Retain - Concatenate character results into a single value in a group</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Some reports ask for all of a subject's results joined into a single string &mdash; for example, "Y;N;Y" for a subject with three visits &mdash; collapsing many rows into one.</li>
<li>The output has one row per subject, with a long character column holding the joined values.</li>
<li>SAS does this with <code>RETAIN</code> + <code>CATX</code> in a DATA step, then keeps only the last row of each subject.</li>
<li>Tidyverse uses <code>group_by() %&gt;% summarize(str_flatten(...))</code> to build the concatenated string in one step.</li>
<li>Base R uses <code>aggregate()</code> with a <code>paste(..., collapse = ";")</code> helper to achieve the same effect.</li>
<li>This lesson joins each subject's non-missing results into one semicolon-separated string; missing values are dropped from the join automatically.</li>
...
<li>To collapse a character column across a subject into a single semicolon-separated string, we use <code>RETAIN</code> together with <code>CATX</code> and keep only the last row per subject.</li>
<li>We give <code>all_results</code> a generous length (<code>$32767</code>) up front so the catx build has room to grow &mdash; the default length would truncate long concatenations.</li>
<li><code>if first.usubjid then all_results = result;</code> primes the string with the first value of the subject.</li>
<li>On every subsequent row, <code>catx(";", all_results, result)</code> appends the current result with a semicolon separator, ignoring empty values automatically.</li>
<li><code>if last.usubjid;</code> is a subsetting <code>if</code> &mdash; it keeps only the final row per subject, which by now holds the fully-built string.</li>
<li>We <code>keep usubjid all_results;</code> to drop the per-row columns and return one row per subject with the concatenated results.</li>
<li>After running, we check that each subject appears once in <code>concat01</code> and <code>all_results</code> is a single string joining all non-missing results with semicolons.</li>
...
concat01 &lt;- concat %&gt;%
  ...
  ) %&gt;% ungroup()</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>For a tidyverse equivalent, we group by subject and use <code>summarize()</code> with <code>str_flatten()</code> to collapse the character column in one step.</li>
<li><code>arrange(usubjid, visitnum)</code> ensures the values are concatenated in the intended order, the same way <code>PROC SORT</code> does for the SAS version.</li>
<li><code>group_by(usubjid)</code> scopes the summary so we get one output row per subject.</li>
<li><code>str_flatten(result, collapse = ";", na.rm = TRUE)</code> is the workhorse &mdash; it joins every value with the <code>;</code> separator and drops NAs from the join.</li>
<li><code>na.rm = TRUE</code> is what gives us the SAS <code>catx</code> behaviour of skipping missing values automatically.</li>
<li>We finish with <code>ungroup()</code> to remove the grouping metadata so downstream code is not surprised.</li>
<li>After running, we verify <code>concat01</code> has one row per subject and <code>all_results</code> is the semicolon-joined string of non-NA results.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">concat &lt;- data.frame(
...
names(concat01)[2] &lt;- &quot;all_results&quot;</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, <code>aggregate()</code> groups the data by subject and applies a custom function that returns one concatenated string per group.</li>
<li>We sort first with <code>order(concat$usubjid, concat$visitnum)</code> so the joined order matches the intended visit order.</li>
<li><code>aggregate(result ~ usubjid, data = concat, FUN = function(x) paste(x[!is.na(x)], collapse = ";"))</code> is the one-line version.</li>
<li>Inside the helper, <code>x[!is.na(x)]</code> drops NAs before the join &mdash; mirroring the SAS <code>catx</code> behaviour of skipping missing values.</li>
<li><code>paste(..., collapse = ";")</code> joins the remaining values with a semicolon separator.</li>
<li><code>names(concat01)[2] &lt;- "all_results"</code> renames the aggregated column to match the report convention.</li>
<li>We run and confirm <code>concat01</code> has one row per subject with the semicolon-joined result string, lining up with the SAS and tidyverse outputs.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"*==============================================================================;\n* Create the input dataset;\n*==============================================================================;\ndata concat;\n    input usubjid visitnum result$;\n    datalines;\n101 1 Y\n101 2 N\n101 3 Y\n102 1 Y\n102 2 N\n102 3 .\n102 4 N\n102 5 Y\n;\nrun;\n\n*==============================================================================;\n*Concatenate non-missing results into a single string;\n*==============================================================================;\n\nproc sort data=concat;\n   by usubjid visitnum;\nrun;\n\ndata concat01;\n   set concat;\n   by usubjid visitnum;\n\n   length all_results $32767;\n\n   retain all_results;\n\n   if first.usubjid then all_results=result;\n   else if not missing(result) then all_results=catx(\";\",all_results,result);\n\n   if last.usubjid;\n\n   keep usubjid all_results;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">*==============================================================================;\n* Create the input dataset;\n*==============================================================================;\ndata concat;\n    input usubjid visitnum result$;\n    datalines;\n101 1 Y\n101 2 N\n101 3 Y\n102 1 Y\n102 2 N\n102 3 .\n102 4 N\n102 5 Y\n;\nrun;\n\n*==============================================================================;\n*Concatenate non-missing results into a single string;\n*==============================================================================;\n\nproc sort data=concat;\n   by usubjid visitnum;\nrun;\n\ndata concat01;\n   set concat;\n   by usubjid visitnum;\n\n   length all_results $32767;\n\n   retain all_results;\n\n   if first.usubjid then all_results=result;\n   else if not missing(result) then all_results=catx(&quot;;&quot;,all_results,result);\n\n   if last.usubjid;\n\n   keep usubjid all_results;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\r\nlibrary(stringr)\r\n\r\n#==============================================================================\r\n# Create the input dataset using tribble\r\n#==============================================================================\r\n\r\nconcat <- tribble(\r\n  ~usubjid, ~visitnum, ~result,\r\n  101, 1, \"Y\",\r\n  101, 2, \"N\",\r\n  101, 3, \"Y\",\r\n  102, 1, \"Y\",\r\n  102, 2, \"N\",\r\n  102, 3, NA,\r\n  102, 4, \"N\",\r\n  102, 5, \"Y\"\r\n)\r\n\r\n#==============================================================================\r\n# Concatenate non-missing results into a single string\r\n#==============================================================================\r\n\r\nconcat01 <- concat %>%\r\n  arrange(usubjid, visitnum) %>%\r\n  group_by(usubjid) %>%\r\n  summarize(\r\n    all_results = str_flatten(result, collapse=\";\",na.rm = TRUE)\r\n  ) %>% ungroup()","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\r\nlibrary(stringr)\r\n\r\n#==============================================================================\r\n# Create the input dataset using tribble\r\n#==============================================================================\r\n\r\nconcat &lt;- tribble(\r\n  ~usubjid, ~visitnum, ~result,\r\n  101, 1, &quot;Y&quot;,\r\n  101, 2, &quot;N&quot;,\r\n  101, 3, &quot;Y&quot;,\r\n  102, 1, &quot;Y&quot;,\r\n  102, 2, &quot;N&quot;,\r\n  102, 3, NA,\r\n  102, 4, &quot;N&quot;,\r\n  102, 5, &quot;Y&quot;\r\n)\r\n\r\n#==============================================================================\r\n# Concatenate non-missing results into a single string\r\n#==============================================================================\r\n\r\nconcat01 &lt;- concat %&gt;%\r\n  arrange(usubjid, visitnum) %&gt;%\r\n  group_by(usubjid) %&gt;%\r\n  summarize(\r\n    all_results = str_flatten(result, collapse=&quot;;&quot;,na.rm = TRUE)\r\n  ) %&gt;% ungroup()<\/code><\/pre>"},{"label":"R (base)","code":"concat <- data.frame(\r\n  usubjid = c(101, 101, 101, 102, 102, 102, 102, 102),\r\n  visitnum = c(1, 2, 3, 1, 2, 3, 4, 5),\r\n  result = c(\"Y\", \"N\", \"Y\", \"Y\", \"N\", NA, \"N\", \"Y\")\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nconcat <- concat[order(concat$usubjid, concat$visitnum), ]\r\n\r\nconcat01 <- aggregate(result ~ usubjid, data = concat, FUN = function(x) {\r\n  paste(x[!is.na(x)], collapse = \";\")\r\n})\r\n\r\nnames(concat01)[2] <- \"all_results\"","language":"r","html":"<pre><code class=\"language-r\">concat &lt;- data.frame(\r\n  usubjid = c(101, 101, 101, 102, 102, 102, 102, 102),\r\n  visitnum = c(1, 2, 3, 1, 2, 3, 4, 5),\r\n  result = c(&quot;Y&quot;, &quot;N&quot;, &quot;Y&quot;, &quot;Y&quot;, &quot;N&quot;, NA, &quot;N&quot;, &quot;Y&quot;)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nconcat &lt;- concat[order(concat$usubjid, concat$visitnum), ]\r\n\r\nconcat01 &lt;- aggregate(result ~ usubjid, data = concat, FUN = function(x) {\r\n  paste(x[!is.na(x)], collapse = &quot;;&quot;)\r\n})\r\n\r\nnames(concat01)[2] &lt;- &quot;all_results&quot;<\/code><\/pre>"}] || [];
