...
<title>mycsg SASnR2 2810_Replicating_usage_of_loops_for_dummy_creation</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Replicating the concept of usage of loops for dummy data creation</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>A recurring need in early prototyping: build a skeleton dataset covering every combination of a few values &mdash; treatments x visits, groups x timepoints, subjects x parameters.</li>
<li>We want every pair to appear exactly once, so we can attach real data later without worrying about missing cells.</li>
<li>SAS builds this with nested <code>DO</code> loops and an <code>OUTPUT</code> statement, writing one row per iteration of the inner loop.</li>
<li>Tidyverse uses <code>crossing()</code> from tidyr, a one-liner that returns every combination of the supplied vectors.</li>
<li>Base R uses <code>expand.grid()</code>, the classic base-R way of producing a full-factorial data frame.</li>
<li>This lesson builds a 3 x 4 grid of treatments (1, 3, 5) and visits (0, 1, 2, 8) &mdash; 12 rows with every pair present once.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data loop;
    do treatment=1,3,5;
        do avisitn=0,1,2,8;
            output;
        end;
...
<li>To build a dummy dataset with every combination of a set of values, we reach for a nested <code>DO</code> loop with an <code>OUTPUT</code> statement in between.</li>
<li>The outer loop iterates over treatments (1, 3, 5) and the inner loop over visit numbers (0, 1, 2, 8).</li>
<li>The <code>output;</code> statement inside the inner loop writes one observation for every combination &mdash; 3 x 4 = 12 rows.</li>
<li>Without an explicit <code>output</code>, SAS would only write a row at the end of each DATA step iteration; with it, we get a full cross-product.</li>
<li>This is the quickest way to prototype a skeleton dataset for testing &mdash; for example, a subject-by-visit grid before actual data is available.</li>
<li>After running, we check that <code>loop</code> has 12 rows with every <code>treatment</code>-<code>avisitn</code> pair appearing exactly once.</li>
...
loop &lt;- crossing(treatment = c(1, 3, 5), avisitn = c(0, 1, 2, 8))</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>For the same cross-product in tidyverse, we call <code>crossing()</code> with one named vector per dimension.</li>
<li><code>crossing()</code> returns a tibble of all unique combinations of the supplied vectors &mdash; the tidyverse equivalent of the SAS nested <code>DO</code> loops + <code>output</code>.</li>
<li>It automatically deduplicates the inputs and orders the output; no explicit loop, <code>output</code>, or row counter needed.</li>
<li>This is far more readable than a for-loop plus <code>rbind()</code> when we want a grid of values for prototyping or full-factorial testing.</li>
<li>A related function, <code>expand_grid()</code>, does the same without deduping &mdash; useful when the inputs intentionally contain repeats.</li>
<li>After running, we confirm <code>loop</code> has 12 rows (3 treatments x 4 visits) with every combination present exactly once.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">loop &lt;- expand.grid(
  treatment = c(1, 3, 5),
  avisitn = c(0, 1, 2, 8)
)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>Base R offers <code>expand.grid()</code> for a full cross-product data frame &mdash; the simplest one-liner for a prototype grid.</li>
<li><code>expand.grid(treatment = c(1, 3, 5), avisitn = c(0, 1, 2, 8))</code> produces every combination of the two vectors &mdash; 12 rows here.</li>
<li>The arguments become column names in the output; first-argument values vary fastest by default (opposite of SAS DO-loop convention).</li>
<li>To match SAS's iteration order (outer varies slowest), either reverse the arguments or call <code>expand.grid(..., KEEP.OUT.ATTRS = FALSE, stringsAsFactors = FALSE)</code> and sort explicitly.</li>
<li>For more than two vectors, just keep passing named arguments: <code>expand.grid(a = 1:2, b = 1:3, c = 1:4)</code> gives 24 rows.</li>
<li>We run and confirm <code>loop</code> has 12 rows with every <code>treatment</code>-<code>avisitn</code> pair appearing exactly once.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data loop;\n    do treatment=1,3,5;\n        do avisitn=0,1,2,8;\n            output;\n        end;\n    end;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data loop;\n    do treatment=1,3,5;\n        do avisitn=0,1,2,8;\n            output;\n        end;\n    end;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\r\n\r\n\u00a0\r\n\r\nloop <- crossing(treatment = c(1, 3, 5), avisitn = c(0, 1, 2, 8))","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\r\n\r\n\u00a0\r\n\r\nloop &lt;- crossing(treatment = c(1, 3, 5), avisitn = c(0, 1, 2, 8))<\/code><\/pre>"},{"label":"R (base)","code":"loop <- expand.grid(\r\n  treatment = c(1, 3, 5),\r\n  avisitn = c(0, 1, 2, 8)\r\n)","language":"r","html":"<pre><code class=\"language-r\">loop &lt;- expand.grid(\r\n  treatment = c(1, 3, 5),\r\n  avisitn = c(0, 1, 2, 8)\r\n)<\/code><\/pre>"}] || [];
