...
<title>mycsg SASnR2 2900_proc_format_named_vector</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Replicate PROC FORMAT Using Named Vectors in R</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Clinical data often travels as numeric codes &mdash; sex = 1/2, race = 1/2/3/4 &mdash; but reports and SDTM want the human-readable labels.</li>
<li>The mapping from code to label is reusable: define it once and apply it wherever the coded column appears.</li>
<li>SAS defines the mapping with <code>proc format</code> and applies it with <code>put(code, fmt.)</code> to derive a labelled column.</li>
<li>Tidyverse and base R both use the same trick: a named character vector, indexed by the string form of the code.</li>
<li>Named-vector lookups are fast, easy to read, and need no extra package &mdash; the closest R idiom to SAS formats.</li>
<li>This lesson maps Sex codes 1 and 2 to "Male" and "Female" and adds the label as a new column <code>sexc</code>.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">* Create format using PROC FORMAT;
proc format;
    value sexfmt
    1 = &quot;Male&quot;
    2 = &quot;Female&quot;
...
<li>In SAS, the code-to-label mapping is declared with <code>proc format</code> and then applied with <code>put()</code>.</li>
<li><code>proc format; value sexfmt 1 = "Male" 2 = "Female"; run;</code> defines a reusable format called <code>sexfmt</code>.</li>
<li>The format is stored in the current format catalogue and is available to any DATA step or PROC until the session ends.</li>
<li><code>sexc = put(sex, sexfmt.);</code> translates the numeric code into its label &mdash; the trailing dot on <code>sexfmt.</code> tells SAS it is a user-defined format.</li>
<li>Unlike a merge-lookup, the format costs almost nothing to evaluate row by row and stays attached to the column as metadata.</li>
<li>We run and open DM_LABELED; the new <code>sexc</code> column should read "Male" for sex = 1 and "Female" for sex = 2.</li>
...
mutate( sexc = sex_fmt[as.character(sex)] )</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In R, the closest thing to a SAS format is a named character vector: the names are the codes and the values are the labels.</li>
<li><code>sex_fmt &lt;- c("1" = "Male", "2" = "Female")</code> defines the mapping; both names and values are strings.</li>
<li><code>mutate(sexc = sex_fmt[as.character(sex)])</code> looks up each code in the named vector and pulls out the matching label.</li>
<li><code>as.character(sex)</code> is important because the names of the vector are strings &mdash; indexing a named vector with numeric values would be by position, not by name.</li>
<li>Codes not present in the mapping return <code>NA</code>, so unexpected values are flagged rather than silently mislabelled.</li>
<li>Named vectors cover most use cases; <code>dplyr::recode()</code> or <code>dplyr::case_when()</code> are alternatives when the mapping logic is more complex than a flat lookup.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">dm &lt;- data.frame(
...
dm_labeled$sexc &lt;- sex_fmt[as.character(dm_labeled$sex)]</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, the same named-vector pattern applies &mdash; no tidyverse required.</li>
<li><code>sex_fmt &lt;- c("1" = "Male", "2" = "Female")</code> defines the code-to-label map.</li>
<li><code>dm_labeled$sexc &lt;- sex_fmt[as.character(dm_labeled$sex)]</code> does the lookup and assigns the result back as a new column.</li>
<li>We always wrap the code in <code>as.character()</code> so the lookup hits the vector by name rather than by numeric position.</li>
<li>For a multi-column lookup, build the map as a data frame and use <code>merge()</code>; for a simple one-column decode, the named-vector approach is the lightest solution.</li>
<li>We run and confirm <code>dm_labeled$sexc</code> has "Male" / "Female" matching the numeric <code>sex</code> column row for row.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"* Create format using PROC FORMAT;\nproc format;\n    value sexfmt\n    1 = \"Male\"\n    2 = \"Female\"\n    ;\nrun;\n\ndata dm;\n  input usubjid $ sex;\n  datalines;\n01 1\n02 2\n03 1\n04 2\n;\nrun;\n\n* Apply the format and create labeled variable;\ndata dm_labeled;\n    set dm;\n    sexc = put(sex, sexfmt.);\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">* Create format using PROC FORMAT;\nproc format;\n    value sexfmt\n    1 = &quot;Male&quot;\n    2 = &quot;Female&quot;\n    ;\nrun;\n\ndata dm;\n  input usubjid $ sex;\n  datalines;\n01 1\n02 2\n03 1\n04 2\n;\nrun;\n\n* Apply the format and create labeled variable;\ndata dm_labeled;\n    set dm;\n    sexc = put(sex, sexfmt.);\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\r\n\r\n# Sample tibble with coded values\r\n\r\ndm <- tibble( usubjid = c(\"01\", \"02\", \"03\", \"04\"), sex = c(1, 2, 1, 2) )\r\n\r\n# Define named vector format\r\n\r\nsex_fmt <- c(\"1\" = \"Male\", \"2\" = \"Female\")\r\n\r\n# Apply the format using mutate\r\n\r\ndm_labeled <- dm %>%\r\n\r\nmutate( sexc = sex_fmt[as.character(sex)] )","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\r\n\r\n# Sample tibble with coded values\r\n\r\ndm &lt;- tibble( usubjid = c(&quot;01&quot;, &quot;02&quot;, &quot;03&quot;, &quot;04&quot;), sex = c(1, 2, 1, 2) )\r\n\r\n# Define named vector format\r\n\r\nsex_fmt &lt;- c(&quot;1&quot; = &quot;Male&quot;, &quot;2&quot; = &quot;Female&quot;)\r\n\r\n# Apply the format using mutate\r\n\r\ndm_labeled &lt;- dm %&gt;%\r\n\r\nmutate( sexc = sex_fmt[as.character(sex)] )<\/code><\/pre>"},{"label":"R (base)","code":"dm <- data.frame(\r\n  usubjid = c(\"01\", \"02\", \"03\", \"04\"),\r\n  sex = c(1, 2, 1, 2)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nsex_fmt <- c(\"1\" = \"Male\", \"2\" = \"Female\")\r\n\r\ndm_labeled <- dm\r\n\r\ndm_labeled$sexc <- sex_fmt[as.character(dm_labeled$sex)]","language":"r","html":"<pre><code class=\"language-r\">dm &lt;- data.frame(\r\n  usubjid = c(&quot;01&quot;, &quot;02&quot;, &quot;03&quot;, &quot;04&quot;),\r\n  sex = c(1, 2, 1, 2)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nsex_fmt &lt;- c(&quot;1&quot; = &quot;Male&quot;, &quot;2&quot; = &quot;Female&quot;)\r\n\r\ndm_labeled &lt;- dm\r\n\r\ndm_labeled$sexc &lt;- sex_fmt[as.character(dm_labeled$sex)]<\/code><\/pre>"}] || [];
