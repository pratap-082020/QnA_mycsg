...
<title>mycsg SASnR2 1100_Create_a_copy_of_an_existing_dataset</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Create a copy of an existing dataset</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Sometimes we need a fresh copy of a dataset to experiment on &mdash; so that our changes do not touch the original.</li>
<li>In SAS, the idiomatic way is a <code>DATA</code> step with a <code>SET</code> statement. In R, we simply assign the existing object to a new name.</li>
<li>This lesson shows the same &ldquo;copy CLASS to a new dataset called new_class&rdquo; task in each environment.</li>
<li>After running, we should have two independent objects in memory: the original <code>class</code> and a copy called <code>new_class</code> with identical contents.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><div style="font-family: Courier New; font-size: 12pt;"><strong><span style="color: #000080; background-color: #ffffff;">data</span></strong><span style="color: #000000; background-color: #ffffff;"> CLASS;<br></span> <span style="color: #0000ff; background-color: #ffffff;">infile</span> <span style="color: #0000ff; background-color: #ffffff;">datalines</span> <span style="color: #0000ff; background-color: #ffffff;">dlm</span><span style="color: #000000; background-color: #ffffff;">=</span><span style="color: #800080; background-color: #ffffff;">'|'</span> <span style="color: #0000ff; background-color: #ffffff;">dsd</span> <span style="color: #0000ff; background-color: #ffffff;">missover</span>; <span style="color: #000000; background-color: #ffffff;"><br></span><span style="color: #0000ff; background-color: #ffffff;">input</span><span style="color: #000000; background-color: #ffffff;"> Name : </span><span style="color: #08726d; background-color: #ffffff;">$8.</span><span style="color: #000000; background-color: #ffffff;"> Sex : </span><span style="color: #08726d; background-color: #ffffff;">$1.</span><span style="color: #000000; background-color: #ffffff;"> Age : </span><span style="color: #08726d; background-color: #ffffff;">best32.</span><span style="color: #000000; background-color: #ffffff;"> Height : </span><span style="color: #08726d; background-color: #ffffff;">best32.</span><span style="color: #000000; background-color: #ffffff;"> Weight : </span><span style="color: #08726d; background-color: #ffffff;">best32.</span>; <span style="color: #000000; background-color: #ffffff;"><br></span><span style="color: #0000ff; background-color: #ffffff;">datalines</span>; <span style="color: #000000; background-color: #ffffff;"><br></span><span style="color: #89632b; background-color: #ffffc0;">Alfred|M|14|69|112.5<br>Alice|F|13|56.5|84<br>Barbara|F|13|65.3|98<br></span>; <span style="color: #000000; background-color: #ffffff;"><br></span><strong><span style="color: #000080; background-color: #ffffff;">run</span></strong>; <span style="color: #000000; background-color: #ffffff;"><br><br></span><strong><span style="color: #000080; background-color: #ffffff;">data</span></strong><span style="color: #000000; background-color: #ffffff;"> new_class;<br></span> <span style="color: #000000; background-color: #ffffff;">    </span><span style="color: #0000ff; background-color: #ffffff;">set</span><span style="color: #000000; background-color: #ffffff;"> class;<br></span> <strong><span style="color: #000080; background-color: #ffffff;">run</span></strong><span style="color: #000000; background-color: #ffffff;">;</span></div>      </div><div class="variant-description mt-2"><ul>
<li>To copy a SAS dataset, we write a <code>DATA</code> step that names the new dataset, reads from the old one with <code>SET</code>, and runs.</li>
<li><code>data new_class;</code> declares the output. <code>set class;</code> reads every observation of CLASS into new_class one at a time.</li>
<li>No explicit <code>keep</code> or <code>drop</code> statement means SAS copies every variable unchanged.</li>
<li>The result is a brand-new dataset in the WORK library. Any later edits we make to <code>new_class</code> do not affect the original <code>class</code>.</li>
<li>This pattern also serves as the starting point for in-place transformations &mdash; we copy first, then add derived columns, filter rows, or change formats on the copy.</li>
...
new_class&lt;-class</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To copy a tibble in R, we simply assign it to a new name: <code>new_class &lt;- class</code>.</li>
<li>R&rsquo;s copy-on-modify semantics mean the new name and the old name start out pointing at the same data, but as soon as we modify either one, R makes a real copy behind the scenes.</li>
<li>So even though it looks like a reference assignment, we never have to worry about edits leaking from one object to the other.</li>
<li>There is no tidyverse-specific function for copying a tibble &mdash; plain assignment is the idiomatic approach.</li>
<li>After running, <code>new_class</code> has the same rows and columns as <code>class</code>, and the two are independent for all future edits.</li>
...
new_class &lt;- class</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, the copy is done the same way as in tidyverse &mdash; with a regular assignment.</li>
<li><code>new_class &lt;- class</code> creates a second reference, which R silently promotes to a real copy the first time either object is modified.</li>
<li>This behaviour is identical whether we are working with a tibble or a plain data frame. There is no separate <code>copy()</code> function in base R for this, and we do not need one.</li>
<li>Run <code>identical(class, new_class)</code> right after the assignment to confirm the two are element-wise equal.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data\u00a0CLASS; infile\u00a0datalines\u00a0dlm='|'\u00a0dsd\u00a0missover; input\u00a0Name\u00a0:\u00a0$8.\u00a0Sex\u00a0:\u00a0$1.\u00a0Age\u00a0:\u00a0best32.\u00a0Height\u00a0:\u00a0best32.\u00a0Weight\u00a0:\u00a0best32.; datalines; Alfred|M|14|69|112.5Alice|F|13|56.5|84Barbara|F|13|65.3|98; run; data\u00a0new_class; \u00a0\u00a0\u00a0\u00a0set\u00a0class; run;","language":"sas","html":"<div style=\"font-family: Courier New; font-size: 12pt;\"><strong><span style=\"color: #000080; background-color: #ffffff;\">data<\/span><\/strong><span style=\"color: #000000; background-color: #ffffff;\">\u00a0CLASS;<br><\/span> <span style=\"color: #0000ff; background-color: #ffffff;\">infile<\/span>\u00a0<span style=\"color: #0000ff; background-color: #ffffff;\">datalines<\/span>\u00a0<span style=\"color: #0000ff; background-color: #ffffff;\">dlm<\/span><span style=\"color: #000000; background-color: #ffffff;\">=<\/span><span style=\"color: #800080; background-color: #ffffff;\">'|'<\/span>\u00a0<span style=\"color: #0000ff; background-color: #ffffff;\">dsd<\/span>\u00a0<span style=\"color: #0000ff; background-color: #ffffff;\">missover<\/span>; <span style=\"color: #000000; background-color: #ffffff;\"><br><\/span><span style=\"color: #0000ff; background-color: #ffffff;\">input<\/span><span style=\"color: #000000; background-color: #ffffff;\">\u00a0Name\u00a0:\u00a0<\/span><span style=\"color: #08726d; background-color: #ffffff;\">$8.<\/span><span style=\"color: #000000; background-color: #ffffff;\">\u00a0Sex\u00a0:\u00a0<\/span><span style=\"color: #08726d; background-color: #ffffff;\">$1.<\/span><span style=\"color: #000000; background-color: #ffffff;\">\u00a0Age\u00a0:\u00a0<\/span><span style=\"color: #08726d; background-color: #ffffff;\">best32.<\/span><span style=\"color: #000000; background-color: #ffffff;\">\u00a0Height\u00a0:\u00a0<\/span><span style=\"color: #08726d; background-color: #ffffff;\">best32.<\/span><span style=\"color: #000000; background-color: #ffffff;\">\u00a0Weight\u00a0:\u00a0<\/span><span style=\"color: #08726d; background-color: #ffffff;\">best32.<\/span>; <span style=\"color: #000000; background-color: #ffffff;\"><br><\/span><span style=\"color: #0000ff; background-color: #ffffff;\">datalines<\/span>; <span style=\"color: #000000; background-color: #ffffff;\"><br><\/span><span style=\"color: #89632b; background-color: #ffffc0;\">Alfred|M|14|69|112.5<br>Alice|F|13|56.5|84<br>Barbara|F|13|65.3|98<br><\/span>; <span style=\"color: #000000; background-color: #ffffff;\"><br><\/span><strong><span style=\"color: #000080; background-color: #ffffff;\">run<\/span><\/strong>; <span style=\"color: #000000; background-color: #ffffff;\"><br><br><\/span><strong><span style=\"color: #000080; background-color: #ffffff;\">data<\/span><\/strong><span style=\"color: #000000; background-color: #ffffff;\">\u00a0new_class;<br><\/span> <span style=\"color: #000000; background-color: #ffffff;\">\u00a0\u00a0\u00a0\u00a0<\/span><span style=\"color: #0000ff; background-color: #ffffff;\">set<\/span><span style=\"color: #000000; background-color: #ffffff;\">\u00a0class;<br><\/span> <strong><span style=\"color: #000080; background-color: #ffffff;\">run<\/span><\/strong><span style=\"color: #000000; background-color: #ffffff;\">;<\/span><\/div>"},{"label":"R (tidyverse)","code":"library(tidyverse)\n\nclass<-tribble(\n~Name,~Sex,~Age,~Height,~Weight,\n\"Alfred\",\"M\",14,69,112.5,\n\"Alice\",\"F\",13,56.5,84,\n\"Barbara\",\"F\",13,65.3,98,\n)\n\n\nnew_class<-class","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\n\nclass&lt;-tribble(\n~Name,~Sex,~Age,~Height,~Weight,\n&quot;Alfred&quot;,&quot;M&quot;,14,69,112.5,\n&quot;Alice&quot;,&quot;F&quot;,13,56.5,84,\n&quot;Barbara&quot;,&quot;F&quot;,13,65.3,98,\n)\n\n\nnew_class&lt;-class<\/code><\/pre>"},{"label":"R (base)","code":"class <- data.frame(\n  Name = c(\"Alfred\", \"Alice\", \"Barbara\"),\n  Sex = c(\"M\", \"F\", \"F\"),\n  Age = c(14, 13, 13),\n  Height = c(69, 56.5, 65.3),\n  Weight = c(112.5, 84, 98)\n  , stringsAsFactors = FALSE\n)\n\nnew_class <- class","language":"r","html":"<pre><code class=\"language-r\">class &lt;- data.frame(\n  Name = c(&quot;Alfred&quot;, &quot;Alice&quot;, &quot;Barbara&quot;),\n  Sex = c(&quot;M&quot;, &quot;F&quot;, &quot;F&quot;),\n  Age = c(14, 13, 13),\n  Height = c(69, 56.5, 65.3),\n  Weight = c(112.5, 84, 98)\n  , stringsAsFactors = FALSE\n)\n\nnew_class &lt;- class<\/code><\/pre>"}] || [];
