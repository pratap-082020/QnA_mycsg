...
<title>mycsg SASnR2 3001_read_sas_files_using_path</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Reading Files Using Path References</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Sometimes, we want to work with the concept of "Reading Files Using Path References" in a clear, repeatable way.</li>
<li>This lesson walks through a simple example and shows the key steps.</li>
<li>We will see one approach on how we can do it in SAS and R.</li>
...
<div style="font-family: Courier New; font-size: 14pt;"><br><span style="color: #0000ff; background-color: #ffffff;">libname</span><span style="color: #000000; background-color: #ffffff;"> mydata </span><span style="color: #800080; background-color: #ffffff;">"path"</span><span style="color: #000000; background-color: #ffffff;">; <br><br></span><strong><span style="color: #000080; background-color: #ffffff;">data</span></strong><span style="color: #000000; background-color: #ffffff;"> class; <br>    </span><span style="color: #0000ff; background-color: #ffffff;">set</span><span style="color: #000000; background-color: #ffffff;"> mydata.class; <br></span><strong><span style="color: #000080; background-color: #ffffff;">run</span></strong><span style="color: #000000; background-color: #ffffff;">;</span></div>
<p> </p>      </div><div class="variant-description mt-2"><ul>
<li>In SAS, `libname` assigns a shortcut (library) to a folder of datasets.</li>
<li>You can then refer to datasets in that folder using the library name, e.g., `mydata.class`.</li>
<li>This enables reading multiple datasets from a folder without repeating the full path.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r"># Direct path reading
library(haven)
...
library(glue)
folder_path &lt;- &quot;D:/SAS/Home/dev/&quot;
class &lt;- read_sas(glue(&quot;{folder_path}/class.sas7bdat&quot;))</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To read a .sas7bdat file into R, we use <code>haven::read_sas()</code>.</li>
<li>The simplest form takes a full path: <code>read_sas("path/class.sas7bdat")</code>.</li>
<li>For portability across machines, we build the path dynamically with <code>file.path()</code> &mdash; it inserts the correct separator for the operating system.</li>
<li><code>glue()</code> offers a string-interpolation alternative that many find easier to read: <code>glue("{folder_path}/class.sas7bdat")</code>.</li>
<li>All three approaches end up in the same spot &mdash; a tibble holding the contents of the SAS dataset.</li>
<li>This lesson has no separate tidyverse variant because <code>read_sas()</code> from haven is the standard way in any R workflow &mdash; tidyverse users and base R users use the same function.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"\u00a0\r\nlibname\u00a0mydata\u00a0\"path\";\u00a0data\u00a0class;\u00a0\u00a0\u00a0\u00a0\u00a0set\u00a0mydata.class;\u00a0run;\r\n\u00a0","language":"sas","html":"<p>\u00a0<\/p>\r\n<div style=\"font-family: Courier New; font-size: 14pt;\"><br><span style=\"color: #0000ff; background-color: #ffffff;\">libname<\/span><span style=\"color: #000000; background-color: #ffffff;\">\u00a0mydata\u00a0<\/span><span style=\"color: #800080; background-color: #ffffff;\">\"path\"<\/span><span style=\"color: #000000; background-color: #ffffff;\">;\u00a0<br><br><\/span><strong><span style=\"color: #000080; background-color: #ffffff;\">data<\/span><\/strong><span style=\"color: #000000; background-color: #ffffff;\">\u00a0class;\u00a0<br>\u00a0\u00a0\u00a0\u00a0<\/span><span style=\"color: #0000ff; background-color: #ffffff;\">set<\/span><span style=\"color: #000000; background-color: #ffffff;\">\u00a0mydata.class;\u00a0<br><\/span><strong><span style=\"color: #000080; background-color: #ffffff;\">run<\/span><\/strong><span style=\"color: #000000; background-color: #ffffff;\">;<\/span><\/div>\r\n<p>\u00a0<\/p>"},{"label":"R (base)","code":"# Direct path reading\nlibrary(haven)\n\nclass <- read_sas(\"path\/class.sas7bdat\")\n\n# Using file.path for portability\nfolder_path <- \"D:\/SAS\/Home\/dev\/\"\nclass <- read_sas(file.path(folder_path, \"class.sas7bdat\"))\n\n# Using glue for portability\nlibrary(glue)\nfolder_path <- \"D:\/SAS\/Home\/dev\/\"\nclass <- read_sas(glue(\"{folder_path}\/class.sas7bdat\"))","language":"r","html":"<pre><code class=\"language-r\"># Direct path reading\nlibrary(haven)\n\nclass &lt;- read_sas(&quot;path\/class.sas7bdat&quot;)\n\n# Using file.path for portability\nfolder_path &lt;- &quot;D:\/SAS\/Home\/dev\/&quot;\nclass &lt;- read_sas(file.path(folder_path, &quot;class.sas7bdat&quot;))\n\n# Using glue for portability\nlibrary(glue)\nfolder_path &lt;- &quot;D:\/SAS\/Home\/dev\/&quot;\nclass &lt;- read_sas(glue(&quot;{folder_path}\/class.sas7bdat&quot;))<\/code><\/pre>"}] || [];
