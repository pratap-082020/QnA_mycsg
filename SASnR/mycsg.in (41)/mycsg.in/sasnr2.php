...
<title>mycsg SASnR2 3100_Character_functions</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Character Functions</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Character functions cover the everyday chores of clinical string handling: measure a value, count a separator, extract a piece, replace a token, check for missing.</li>
<li>Most SAS character functions have a direct analogue in R, usually in the stringr package plus a few base R helpers.</li>
<li>SAS offers <code>length</code>, <code>countw</code>, <code>scan</code>, <code>countc</code>, <code>find</code>, <code>substr</code>, <code>catx</code>, <code>translate</code>, <code>tranwrd</code>, and <code>missing</code> as the daily toolkit.</li>
<li>Tidyverse mirrors these with <code>str_length</code>, <code>str_count</code>, <code>word</code>, <code>str_locate</code>, <code>str_sub</code>, <code>str_c</code>, <code>chartr</code>, <code>str_replace_all</code>, and <code>is.na</code>.</li>
<li>Base R covers them with <code>nchar</code>, <code>strsplit</code>, <code>gregexpr</code>, <code>regexpr</code>, <code>substr</code>, <code>paste</code>, <code>chartr</code>, <code>gsub</code>, and <code>is.na</code>.</li>
<li>This lesson applies each function to a column of ISO-8601 partial dates so we can compare the outputs side by side across the three languages.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data alldata;
...
<li>This lesson demonstrates the most common SAS character functions by applying them to a column of ISO-8601 partial dates.</li>
<li><code>LENGTH(dtc)</code> returns the number of characters in the value &mdash; useful for spotting truncated or oversized strings.</li>
<li><code>COUNTW(dtc, 'T')</code> counts words when <code>T</code> is treated as a delimiter &mdash; so "2025-12-31T23:59" has two words.</li>
<li><code>SCAN(dtc, 1, 'T')</code> grabs the first word with the same delimiter &mdash; the date portion before the <code>T</code>.</li>
<li><code>COUNTC(dtc, '-')</code> counts occurrences of the dash character &mdash; handy for distinguishing year-only, year-month and full date formats.</li>
<li><code>FIND(dtc, 'T', 'i')</code> returns the character position of the <code>T</code> separator, or 0 if absent.</li>
<li><code>SUBSTR(dtc, 1, 10)</code> extracts the first 10 characters &mdash; the full date portion when present.</li>
<li><code>CATX(" ", "Event start:", dtc)</code> joins values with a space separator, quietly skipping empty pieces.</li>
<li><code>TRANSLATE(dtc, '-', ':')</code> swaps every colon with a dash, character-by-character.</li>
<li><code>TRANWRD(dtc, '2025', '20xx')</code> replaces a whole word or substring with another &mdash; the go-to for string redaction.</li>
<li><code>MISSING(dtc)</code> returns 1 if the value is missing and 0 otherwise &mdash; safer than testing <code>dtc = ""</code>.</li>
<li>After running, we inspect <code>dates01</code> and confirm each new column reflects the transformation its name suggests.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-tidyverse">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-tidyverse" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (tidyverse)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">alldata &lt;- tribble(
  ~dtc,
  &quot;2025&quot;,
  &quot;2025-01&quot;,
  &quot;2025-01-10&quot;,
...
<li>The tidyverse equivalents mostly live in <code>stringr</code>, with a couple of base R helpers mixed in.</li>
<li><code>str_length(dtc)</code> mirrors SAS <code>LENGTH()</code> &mdash; character count of a string, NA-safe.</li>
<li>For "count of words" with a separator, we use <code>str_count(dtc, "T") + 1</code>. <code>str_count()</code> counts the separator; adding one gives the word count SAS <code>COUNTW</code> returns.</li>
<li><code>word(dtc, 1, sep = "T")</code> extracts the first word by a separator &mdash; the match for SAS <code>SCAN(dtc, 1, 'T')</code>.</li>
<li><code>str_count(dtc, "-")</code> counts occurrences of a character or pattern &mdash; equivalent to SAS <code>COUNTC</code>.</li>
<li><code>str_locate(dtc, "T")[, 1]</code> returns the start position of the first match &mdash; the tidyverse analogue of SAS <code>FIND()</code>. We subscript <code>[, 1]</code> to get the start column only.</li>
<li><code>str_sub(dtc, 1, 10)</code> mirrors <code>SUBSTR(dtc, 1, 10)</code>.</li>
<li><code>str_c("Event start:", dtc, sep = " ")</code> concatenates with a separator &mdash; the equivalent of SAS <code>CATX</code>, though it does not skip NAs automatically (watch for that).</li>
<li>For character-by-character swap, we use base R <code>chartr(":", "-", dtc)</code> &mdash; matches SAS <code>TRANSLATE()</code>.</li>
<li><code>str_replace_all(dtc, "2025", "20xx")</code> replaces a substring everywhere it occurs, the same as SAS <code>TRANWRD()</code>.</li>
<li><code>is.na(dtc)</code> is the base R check for missing values &mdash; mirrors SAS <code>MISSING()</code> and returns a logical.</li>
<li>After running, we inspect <code>dates01</code> and confirm each column matches its SAS counterpart row-by-row.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">alldata &lt;- data.frame(
  dtc = c(
    &quot;2025&quot;,
    &quot;2025-01&quot;,
    &quot;2025-01-10&quot;,
...
dates01$missing &lt;- is.na(dates01$dtc)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>Base R covers the same character operations as stringr, with slightly older names and a few sharp edges around NAs.</li>
<li><code>nchar(dtc)</code> is the base R analogue of SAS <code>LENGTH()</code>; it returns the character count and is NA-safe.</li>
<li>For word count with a separator, <code>lengths(strsplit(dtc, "T", fixed = TRUE))</code> returns the number of pieces per string &mdash; handling NAs explicitly through a logical index.</li>
<li>To grab the first piece, <code>sapply(strsplit(dtc, "T", fixed = TRUE), `[`, 1)</code> extracts element 1 of each split result &mdash; the base-R analogue of SAS <code>SCAN()</code>.</li>
<li>For a character count, <code>gregexpr("-", dtc, fixed = TRUE)</code> returns every match position; wrap it in a helper to convert "no match" (-1) to 0, matching SAS <code>COUNTC()</code>.</li>
<li><code>regexpr("T", dtc, fixed = TRUE)</code> returns the position of the first match (-1 if none) &mdash; replace -1 with NA to mirror SAS <code>FIND()</code>.</li>
<li><code>substr(dtc, 1, 10)</code> is identical in name and behaviour to SAS <code>SUBSTR()</code>.</li>
<li><code>paste("Event start:", dtc)</code> concatenates with a space separator, but propagates NA unless we guard with <code>ifelse(is.na(dtc), NA, paste(...))</code>.</li>
<li><code>chartr(":", "-", dtc)</code> swaps characters one-for-one &mdash; the mirror of SAS <code>TRANSLATE()</code>.</li>
<li><code>gsub("2025", "20xx", dtc)</code> replaces a whole substring, matching SAS <code>TRANWRD()</code>; use <code>fixed = TRUE</code> if the pattern contains regex metacharacters.</li>
<li><code>is.na(dtc)</code> covers SAS <code>MISSING()</code>.</li>
<li>We run and compare each new column against the tidyverse and SAS outputs &mdash; they should match value for value.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data alldata;\n    infile cards dsd dlm='|' truncover;\n    input dtc : $20.;\ncards;\n2025\n2025-01\n2025-01-10\n2025-02-28\n2025-03\n2025-12-31\n2025-12-31T23\n2025-12-31T23:59\n2025-12-31T23:59:59\n.\n;\nrun;\n\ndata dates01;\n    set alldata;\n\n    length=length(dtc);\n    countw=countw(dtc,'T');\n    scan_fword=scan(dtc,1,'T');\n    countc=countc(dtc,'-');\n    findt=find(dtc,'T','i');\n    substr_date=substr(dtc,1,10);\n    concat=catx(\" \", \"Event start:\",dtc);\n    translate=translate(dtc,'-',':');\n    tranwrd=tranwrd(dtc,'2025','20xx');\n    missing=missing(dtc);\n\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data alldata;\n    infile cards dsd dlm=&#039;|&#039; truncover;\n    input dtc : $20.;\ncards;\n2025\n2025-01\n2025-01-10\n2025-02-28\n2025-03\n2025-12-31\n2025-12-31T23\n2025-12-31T23:59\n2025-12-31T23:59:59\n.\n;\nrun;\n\ndata dates01;\n    set alldata;\n\n    length=length(dtc);\n    countw=countw(dtc,&#039;T&#039;);\n    scan_fword=scan(dtc,1,&#039;T&#039;);\n    countc=countc(dtc,&#039;-&#039;);\n    findt=find(dtc,&#039;T&#039;,&#039;i&#039;);\n    substr_date=substr(dtc,1,10);\n    concat=catx(&quot; &quot;, &quot;Event start:&quot;,dtc);\n    translate=translate(dtc,&#039;-&#039;,&#039;:&#039;);\n    tranwrd=tranwrd(dtc,&#039;2025&#039;,&#039;20xx&#039;);\n    missing=missing(dtc);\n\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"alldata <- tribble(\r\n  ~dtc,\r\n  \"2025\",\r\n  \"2025-01\",\r\n  \"2025-01-10\",\r\n  \"2025-02-28\",\r\n  \"2025-03\",\r\n  \"2025-12-31\",\r\n  \"2025-12-31T23\",\r\n  \"2025-12-31T23:59\",\r\n  \"2025-12-31T23:59:59\",\r\n  NA\r\n)\r\n\r\n\r\ndates01 <- alldata %>%\r\n  mutate(\r\n    length = str_length(dtc),                         # LENGTH(dtc)\r\n    countw = str_count(dtc, \"T\") + 1,                  # COUNTW(dtc, 'T')\r\n    scan_fword = word(dtc, 1, sep = \"T\"),             # SCAN(dtc, 1, 'T')\r\n    countc = str_count(dtc, \"-\"),                     # COUNTC(dtc, '-')\r\n    findt = str_locate(dtc, \"T\")[, 1],                # FIND(dtc, 'T')\r\n    substr_date = str_sub(dtc, 1, 10),                # SUBSTR(dtc, 1, 10)\r\n    concat = str_c(\"Event start:\", dtc, sep = \" \"),   # CATX(\" \", ...)\r\n    translate = chartr(\":\",\"-\",  dtc),                # TRANSLATE(dtc, '-', ':')\r\n    tranwrd = str_replace_all(dtc, \"2025\", \"20xx\"),   # TRANWRD(dtc, '2025', '20xx')\r\n    missing = is.na(dtc)                              # MISSING(dtc)\r\n  )","language":"r","html":"<pre><code class=\"language-r\">alldata &lt;- tribble(\r\n  ~dtc,\r\n  &quot;2025&quot;,\r\n  &quot;2025-01&quot;,\r\n  &quot;2025-01-10&quot;,\r\n  &quot;2025-02-28&quot;,\r\n  &quot;2025-03&quot;,\r\n  &quot;2025-12-31&quot;,\r\n  &quot;2025-12-31T23&quot;,\r\n  &quot;2025-12-31T23:59&quot;,\r\n  &quot;2025-12-31T23:59:59&quot;,\r\n  NA\r\n)\r\n\r\n\r\ndates01 &lt;- alldata %&gt;%\r\n  mutate(\r\n    length = str_length(dtc),                         # LENGTH(dtc)\r\n    countw = str_count(dtc, &quot;T&quot;) + 1,                  # COUNTW(dtc, &#039;T&#039;)\r\n    scan_fword = word(dtc, 1, sep = &quot;T&quot;),             # SCAN(dtc, 1, &#039;T&#039;)\r\n    countc = str_count(dtc, &quot;-&quot;),                     # COUNTC(dtc, &#039;-&#039;)\r\n    findt = str_locate(dtc, &quot;T&quot;)[, 1],                # FIND(dtc, &#039;T&#039;)\r\n    substr_date = str_sub(dtc, 1, 10),                # SUBSTR(dtc, 1, 10)\r\n    concat = str_c(&quot;Event start:&quot;, dtc, sep = &quot; &quot;),   # CATX(&quot; &quot;, ...)\r\n    translate = chartr(&quot;:&quot;,&quot;-&quot;,  dtc),                # TRANSLATE(dtc, &#039;-&#039;, &#039;:&#039;)\r\n    tranwrd = str_replace_all(dtc, &quot;2025&quot;, &quot;20xx&quot;),   # TRANWRD(dtc, &#039;2025&#039;, &#039;20xx&#039;)\r\n    missing = is.na(dtc)                              # MISSING(dtc)\r\n  )<\/code><\/pre>"},{"label":"R (base)","code":"alldata <- data.frame(\r\n  dtc = c(\r\n    \"2025\",\r\n    \"2025-01\",\r\n    \"2025-01-10\",\r\n    \"2025-02-28\",\r\n    \"2025-03\",\r\n    \"2025-12-31\",\r\n    \"2025-12-31T23\",\r\n    \"2025-12-31T23:59\",\r\n    \"2025-12-31T23:59:59\",\r\n    NA\r\n  ),\r\n  stringsAsFactors = FALSE\r\n)\r\n\r\ndates01 <- alldata\r\n\r\n# length of the full string (NA stays NA)\r\ndates01$length <- nchar(dates01$dtc)\r\n\r\n# initialize index for non-missing dtc\r\nidx <- !is.na(dates01$dtc)\r\n\r\n# count of parts when split by \"T\"\r\ndates01$countw <- NA_integer_\r\ndates01$countw[idx] <- lengths(\r\n  strsplit(dates01$dtc[idx], \"T\", fixed = TRUE)\r\n)\r\n\r\n# first part before \"T\" (date portion)\r\ndates01$scan_fword <- NA_character_\r\ndates01$scan_fword[idx] <- sapply(\r\n  strsplit(dates01$dtc[idx], \"T\", fixed = TRUE),\r\n  `[`,\r\n  1\r\n)\r\n\r\n# count of \"-\" in the full string (corrected section)\r\ndates01$countc <- NA_integer_\r\ndates01$countc[idx] <- sapply(\r\n  gregexpr(\"-\", dates01$dtc[idx], fixed = TRUE),\r\n  function(pos) {\r\n    if (pos[1] == -1) 0L else length(pos)\r\n  }\r\n)\r\n\r\nfindt_pos <- regexpr(\"T\", dates01$dtc, fixed = TRUE)\r\n\r\ndates01$findt <- ifelse(findt_pos == -1, NA, findt_pos)\r\n\r\ndates01$substr_date <- substr(dates01$dtc, 1, 10)\r\n\r\ndates01$concat <- ifelse(is.na(dates01$dtc), NA, paste(\"Event start:\", dates01$dtc))\r\n\r\ndates01$translate <- chartr(\":\", \"-\", dates01$dtc)\r\n\r\ndates01$tranwrd <- gsub(\"2025\", \"20xx\", dates01$dtc)\r\n\r\ndates01$missing <- is.na(dates01$dtc)","language":"r","html":"<pre><code class=\"language-r\">alldata &lt;- data.frame(\r\n  dtc = c(\r\n    &quot;2025&quot;,\r\n    &quot;2025-01&quot;,\r\n    &quot;2025-01-10&quot;,\r\n    &quot;2025-02-28&quot;,\r\n    &quot;2025-03&quot;,\r\n    &quot;2025-12-31&quot;,\r\n    &quot;2025-12-31T23&quot;,\r\n    &quot;2025-12-31T23:59&quot;,\r\n    &quot;2025-12-31T23:59:59&quot;,\r\n    NA\r\n  ),\r\n  stringsAsFactors = FALSE\r\n)\r\n\r\ndates01 &lt;- alldata\r\n\r\n# length of the full string (NA stays NA)\r\ndates01$length &lt;- nchar(dates01$dtc)\r\n\r\n# initialize index for non-missing dtc\r\nidx &lt;- !is.na(dates01$dtc)\r\n\r\n# count of parts when split by &quot;T&quot;\r\ndates01$countw &lt;- NA_integer_\r\ndates01$countw[idx] &lt;- lengths(\r\n  strsplit(dates01$dtc[idx], &quot;T&quot;, fixed = TRUE)\r\n)\r\n\r\n# first part before &quot;T&quot; (date portion)\r\ndates01$scan_fword &lt;- NA_character_\r\ndates01$scan_fword[idx] &lt;- sapply(\r\n  strsplit(dates01$dtc[idx], &quot;T&quot;, fixed = TRUE),\r\n  `[`,\r\n  1\r\n)\r\n\r\n# count of &quot;-&quot; in the full string (corrected section)\r\ndates01$countc &lt;- NA_integer_\r\ndates01$countc[idx] &lt;- sapply(\r\n  gregexpr(&quot;-&quot;, dates01$dtc[idx], fixed = TRUE),\r\n  function(pos) {\r\n    if (pos[1] == -1) 0L else length(pos)\r\n  }\r\n)\r\n\r\nfindt_pos &lt;- regexpr(&quot;T&quot;, dates01$dtc, fixed = TRUE)\r\n\r\ndates01$findt &lt;- ifelse(findt_pos == -1, NA, findt_pos)\r\n\r\ndates01$substr_date &lt;- substr(dates01$dtc, 1, 10)\r\n\r\ndates01$concat &lt;- ifelse(is.na(dates01$dtc), NA, paste(&quot;Event start:&quot;, dates01$dtc))\r\n\r\ndates01$translate &lt;- chartr(&quot;:&quot;, &quot;-&quot;, dates01$dtc)\r\n\r\ndates01$tranwrd &lt;- gsub(&quot;2025&quot;, &quot;20xx&quot;, dates01$dtc)\r\n\r\ndates01$missing &lt;- is.na(dates01$dtc)<\/code><\/pre>"}] || [];
