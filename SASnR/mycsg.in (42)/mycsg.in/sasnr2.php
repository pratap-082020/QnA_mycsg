...
<title>mycsg SASnR2 3105_compress-or-remove-specific-characters</title>
...
<li class="nav-item dropdown d-flex align-items-center">
  ...
  <div class="dropdown-menu dropdown-menu-end dropdown-menu-right p-2"
       ...
       style="min-width: 320px; max-width: min(92vw, 420px); overflow-wrap:anywhere;">
        ...
        <div class="px-1 text-muted" style="font-size:.75rem;">As of 17 May 2026, 23:52 IST</div>
      </div>
    </li>  
      <li class="nav-item">
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Compress or remove specific characters</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Raw text values coming from EDC, external vendors, lab files or manual entry are often messy &mdash; extra spaces, stray punctuation, mixed separators like slashes and dashes, or special characters that we do not want in the analysis.</li>
<li>Before we can match, group, merge or summarise such values reliably, we typically clean them up by dropping unwanted characters.</li>
<li>SAS calls this operation <em>compressing</em> a string &mdash; the <code>COMPRESS()</code> function either strips a given set of characters or, with the <code>"k"</code> (keep) modifier, strips everything <em>except</em> a given set.</li>
<li>R does not have a dedicated "compress" function. We use <code>str_remove_all()</code> (tidyverse <code>stringr</code>) or <code>gsub()</code> (base R) with a pattern. A plain string removes literal characters; a regular expression removes a class of characters.</li>
<li>What to expect from the examples below:
<ul>
<li>A small input dataset with six deliberately messy text values.</li>
<li>Five cleaned columns showing common tasks: drop spaces only, drop any whitespace, drop slashes and dashes, keep only digits, keep only letters and digits.</li>
<li>The SAS, tidyverse and base R versions produce the same five columns so we can compare the syntax side by side.</li>
</ul>
</li>
<li>After running, inspect <code>compress02</code> (or the R equivalent) and confirm each new column reflects the intended rule. For example, <code>digits_only</code> on row 4 should contain only <code>015</code>.</li>
...
* Step 1: create a small dataset with messy text values                       ;
*==============================================================================;
data compress01;
...
*==============================================================================;
* Step 2: apply several COMPRESS variations                                   ;
*==============================================================================;
data compress02;
  set compress01;

  no_spaces     = compress(raw_text);             /* default: drops blanks only       */
  no_whitespace = compress(raw_text, , &quot;s&quot;);      /* &quot;s&quot; modifier: any whitespace     */
  no_dash_slash = compress(raw_text, &quot;-/&quot;);       /* drop listed characters           */
  digits_only   = compress(raw_text, , &quot;kd&quot;);     /* k=keep, d=digits                 */
  alnum_only    = compress(raw_text, , &quot;kad&quot;);    /* k=keep, a=alpha, d=digits        */
run;</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To drop only blank spaces, we call <code>compress</code> with just the first argument &mdash; for example, <code>compress(string)</code>.</li>
<li>This is the default behaviour and is often enough when the only clutter in our text is ordinary spaces.</li>
<li>If we want to remove a specific list of characters instead, we pass them as the second argument.</li>
<li>For example, <code>compress(string, "-/")</code> drops every hyphen and slash it finds.</li>
<li>The second argument is plain character matching, not a regular expression.</li>
<li>When we need something broader &mdash; like &ldquo;drop all whitespace&rdquo; or &ldquo;drop all punctuation&rdquo; &mdash; SAS offers single-letter modifiers in the third argument.</li>
<li>The useful ones are <code>d</code> (digits), <code>a</code> (alphabetic), <code>s</code> (whitespace), <code>p</code> (punctuation).</li>
<li>We add the <code>k</code> modifier to flip the meaning from &ldquo;drop these&rdquo; to &ldquo;keep these&rdquo;.</li>
<li>So <code>"kd"</code> keeps digits and drops everything else.</li>
<li>And <code>"kad"</code> keeps letters and digits, drops everything else.</li>
<li>One gotcha: bare <code>compress()</code> strips ordinary blanks but not tabs.</li>
<li>If the upstream file might contain tabs or newlines, we add the <code>s</code> modifier.</li>
...
#==============================================================================
# Step 2: apply several str_remove_all variations
#==============================================================================
compress02 &lt;- compress01 %&gt;%
  mutate(
    no_spaces     = str_remove_all(raw_text, &quot; &quot;),        # literal space only
    no_whitespace = str_remove_all(raw_text, &quot;\\s+&quot;),     # any whitespace run
    no_dash_slash = str_remove_all(raw_text, &quot;[-/]&quot;),     # character class
    digits_only   = str_remove_all(raw_text, &quot;[^0-9]&quot;),   # drop non-digits  -&gt; keep digits
    alnum_only    = str_remove_all(raw_text, &quot;[^A-Za-z0-9]&quot;) # drop non-alnum -&gt; keep alnum
  )</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To drop any run of characters from a string in tidyverse, we call <code>str_remove_all(string, pattern)</code>.</li>
<li>It returns the cleaned value and is vectorised, so we can drop it straight inside <code>mutate()</code> and let it run across a whole column.</li>
<li>The second argument is a regular expression, not a plain character list.</li>
<li>That is the big shift from SAS &mdash; instead of single-letter modifiers like <code>"kd"</code>, we describe the characters we want to drop with regex syntax.</li>
<li>Here is how each of the five cleaned columns maps back to its SAS <code>compress()</code> equivalent:
<ul>
<li>For dropping just literal spaces, pattern <code>" "</code> matches only the ordinary space character. Same result as SAS <code>compress(raw_text)</code>.</li>
<li>For any run of whitespace (spaces, tabs, newlines), we use <code>"\\s+"</code>. Same result as SAS <code>compress(raw_text, , "s")</code>.</li>
<li>To drop a specific set of characters, we put them in a character class like <code>"[-/]"</code>, which means &ldquo;any one of <code>-</code> or <code>/</code>&rdquo;. Same result as SAS <code>compress(raw_text, "-/")</code>.</li>
<li>To keep only digits, we flip the logic &mdash; <code>"[^0-9]"</code> means &ldquo;any character that is <em>not</em> a digit&rdquo;, so removing those leaves the digits behind. Same result as SAS <code>compress(raw_text, , "kd")</code>.</li>
<li>To keep only letters and digits, same trick &mdash; <code>"[^A-Za-z0-9]"</code> removes everything that is not alphanumeric. Same result as SAS <code>compress(raw_text, , "kad")</code>.</li>
</ul>
</li>
<li>The <code>^</code> inside square brackets is the critical trick.</li>
<li>It flips the class from &ldquo;match these&rdquo; to &ldquo;match everything except these&rdquo;.</li>
<li>That is how we emulate the SAS <code>k</code> (keep) modifier in R.</li>
...
# Step 1: create a small data frame with messy text values
#==============================================================================
compress01 &lt;- data.frame(
  id = 1:6,
  raw_text = c(
    &quot;SUBJ 001&quot;,
    &quot;SUBJ-002/A&quot;,
...
#==============================================================================
# Step 2: apply several gsub variations
#==============================================================================
compress02 &lt;- compress01
compress02$no_spaces     &lt;- gsub(&quot; &quot;,           &quot;&quot;, compress02$raw_text, fixed = TRUE)
compress02$no_whitespace &lt;- gsub(&quot;\\s+&quot;,        &quot;&quot;, compress02$raw_text)
compress02$no_dash_slash &lt;- gsub(&quot;[-/]&quot;,        &quot;&quot;, compress02$raw_text)
compress02$digits_only   &lt;- gsub(&quot;[^0-9]&quot;,      &quot;&quot;, compress02$raw_text)
compress02$alnum_only    &lt;- gsub(&quot;[^A-Za-z0-9]&quot;, &quot;&quot;, compress02$raw_text)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>When we need find-and-replace on a character vector in base R, we reach for <code>gsub(pattern, replacement, x)</code>.</li>
<li>To turn it into a compress-style operation, we set <code>replacement = ""</code> &mdash; matches are simply deleted.</li>
<li>The regex patterns here are exactly the same as the tidyverse version, because <code>str_remove_all()</code> is essentially a friendly wrapper over <code>gsub()</code>.</li>
<li>Notice that we use <code>fixed = TRUE</code> on the first call. That switches off regex interpretation so <code>" "</code> is treated as a literal space character.</li>
<li>For plain literal drops like that, <code>fixed = TRUE</code> is both clearer to read and faster to execute than running the regex engine.</li>
<li>Instead of <code>mutate()</code>, we assign one column at a time with <code>compress02$column &lt;- ...</code>. More typing, but zero external dependencies.</li>
<li>That matters when we are working in a locked-down or validated R environment where installing tidyverse (and its many transitive dependencies) is not an option.</li>
<li>After running, we print <code>compress02</code> in the console and spot-check each new column.</li>
<li>On row 4, <code>digits_only</code> should be <code>"015"</code>.</li>
<li>On row 5, <code>alnum_only</code> should be <code>"ABC123XYZ"</code>.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"*==============================================================================;\n* Step 1: create a small dataset with messy text values                       ;\n*==============================================================================;\ndata compress01;\n  infile datalines dlm='|' dsd truncover;\n  length raw_text $40;\n  input id raw_text $;\n  datalines;\n1|SUBJ 001\n2|SUBJ-002\/A\n3|  AE-TERM 123 \n4|VISIT 01 \/ DAY 5\n5|ABC@123#XYZ\n6| 2024-01\/15 \n;\nrun;\n\n*==============================================================================;\n* Step 2: apply several COMPRESS variations                                   ;\n*==============================================================================;\ndata compress02;\n  set compress01;\n\n  no_spaces     = compress(raw_text);             \/* default: drops blanks only       *\/\n  no_whitespace = compress(raw_text, , \"s\");      \/* \"s\" modifier: any whitespace     *\/\n  no_dash_slash = compress(raw_text, \"-\/\");       \/* drop listed characters           *\/\n  digits_only   = compress(raw_text, , \"kd\");     \/* k=keep, d=digits                 *\/\n  alnum_only    = compress(raw_text, , \"kad\");    \/* k=keep, a=alpha, d=digits        *\/\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">*==============================================================================;\n* Step 1: create a small dataset with messy text values                       ;\n*==============================================================================;\ndata compress01;\n  infile datalines dlm=&#039;|&#039; dsd truncover;\n  length raw_text $40;\n  input id raw_text $;\n  datalines;\n1|SUBJ 001\n2|SUBJ-002\/A\n3|  AE-TERM 123 \n4|VISIT 01 \/ DAY 5\n5|ABC@123#XYZ\n6| 2024-01\/15 \n;\nrun;\n\n*==============================================================================;\n* Step 2: apply several COMPRESS variations                                   ;\n*==============================================================================;\ndata compress02;\n  set compress01;\n\n  no_spaces     = compress(raw_text);             \/* default: drops blanks only       *\/\n  no_whitespace = compress(raw_text, , &quot;s&quot;);      \/* &quot;s&quot; modifier: any whitespace     *\/\n  no_dash_slash = compress(raw_text, &quot;-\/&quot;);       \/* drop listed characters           *\/\n  digits_only   = compress(raw_text, , &quot;kd&quot;);     \/* k=keep, d=digits                 *\/\n  alnum_only    = compress(raw_text, , &quot;kad&quot;);    \/* k=keep, a=alpha, d=digits        *\/\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\n\n#==============================================================================\n# Step 1: create a small dataset with messy text values\n#==============================================================================\ncompress01 <- tribble(\n  ~id, ~raw_text,\n  1, \"SUBJ 001\",\n  2, \"SUBJ-002\/A\",\n  3, \"  AE-TERM 123 \",\n  4, \"VISIT 01 \/ DAY 5\",\n  5, \"ABC@123#XYZ\",\n  6, \" 2024-01\/15 \"\n)\n\n#==============================================================================\n# Step 2: apply several str_remove_all variations\n#==============================================================================\ncompress02 <- compress01 %>%\n  mutate(\n    no_spaces     = str_remove_all(raw_text, \" \"),        # literal space only\n    no_whitespace = str_remove_all(raw_text, \"\\\\s+\"),     # any whitespace run\n    no_dash_slash = str_remove_all(raw_text, \"[-\/]\"),     # character class\n    digits_only   = str_remove_all(raw_text, \"[^0-9]\"),   # drop non-digits  -> keep digits\n    alnum_only    = str_remove_all(raw_text, \"[^A-Za-z0-9]\") # drop non-alnum -> keep alnum\n  )","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\n\n#==============================================================================\n# Step 1: create a small dataset with messy text values\n#==============================================================================\ncompress01 &lt;- tribble(\n  ~id, ~raw_text,\n  1, &quot;SUBJ 001&quot;,\n  2, &quot;SUBJ-002\/A&quot;,\n  3, &quot;  AE-TERM 123 &quot;,\n  4, &quot;VISIT 01 \/ DAY 5&quot;,\n  5, &quot;ABC@123#XYZ&quot;,\n  6, &quot; 2024-01\/15 &quot;\n)\n\n#==============================================================================\n# Step 2: apply several str_remove_all variations\n#==============================================================================\ncompress02 &lt;- compress01 %&gt;%\n  mutate(\n    no_spaces     = str_remove_all(raw_text, &quot; &quot;),        # literal space only\n    no_whitespace = str_remove_all(raw_text, &quot;\\\\s+&quot;),     # any whitespace run\n    no_dash_slash = str_remove_all(raw_text, &quot;[-\/]&quot;),     # character class\n    digits_only   = str_remove_all(raw_text, &quot;[^0-9]&quot;),   # drop non-digits  -&gt; keep digits\n    alnum_only    = str_remove_all(raw_text, &quot;[^A-Za-z0-9]&quot;) # drop non-alnum -&gt; keep alnum\n  )<\/code><\/pre>"},{"label":"R (base)","code":"#==============================================================================\n# Step 1: create a small data frame with messy text values\n#==============================================================================\ncompress01 <- data.frame(\n  id = 1:6,\n  raw_text = c(\n    \"SUBJ 001\",\n    \"SUBJ-002\/A\",\n    \"  AE-TERM 123 \",\n    \"VISIT 01 \/ DAY 5\",\n    \"ABC@123#XYZ\",\n    \" 2024-01\/15 \"\n  ),\n  stringsAsFactors = FALSE\n)\n\n#==============================================================================\n# Step 2: apply several gsub variations\n#==============================================================================\ncompress02 <- compress01\ncompress02$no_spaces     <- gsub(\" \",           \"\", compress02$raw_text, fixed = TRUE)\ncompress02$no_whitespace <- gsub(\"\\\\s+\",        \"\", compress02$raw_text)\ncompress02$no_dash_slash <- gsub(\"[-\/]\",        \"\", compress02$raw_text)\ncompress02$digits_only   <- gsub(\"[^0-9]\",      \"\", compress02$raw_text)\ncompress02$alnum_only    <- gsub(\"[^A-Za-z0-9]\", \"\", compress02$raw_text)","language":"r","html":"<pre><code class=\"language-r\">#==============================================================================\n# Step 1: create a small data frame with messy text values\n#==============================================================================\ncompress01 &lt;- data.frame(\n  id = 1:6,\n  raw_text = c(\n    &quot;SUBJ 001&quot;,\n    &quot;SUBJ-002\/A&quot;,\n    &quot;  AE-TERM 123 &quot;,\n    &quot;VISIT 01 \/ DAY 5&quot;,\n    &quot;ABC@123#XYZ&quot;,\n    &quot; 2024-01\/15 &quot;\n  ),\n  stringsAsFactors = FALSE\n)\n\n#==============================================================================\n# Step 2: apply several gsub variations\n#==============================================================================\ncompress02 &lt;- compress01\ncompress02$no_spaces     &lt;- gsub(&quot; &quot;,           &quot;&quot;, compress02$raw_text, fixed = TRUE)\ncompress02$no_whitespace &lt;- gsub(&quot;\\\\s+&quot;,        &quot;&quot;, compress02$raw_text)\ncompress02$no_dash_slash &lt;- gsub(&quot;[-\/]&quot;,        &quot;&quot;, compress02$raw_text)\ncompress02$digits_only   &lt;- gsub(&quot;[^0-9]&quot;,      &quot;&quot;, compress02$raw_text)\ncompress02$alnum_only    &lt;- gsub(&quot;[^A-Za-z0-9]&quot;, &quot;&quot;, compress02$raw_text)<\/code><\/pre>"}] || [];
