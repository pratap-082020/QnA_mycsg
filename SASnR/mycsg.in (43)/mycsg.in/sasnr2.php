...
<title>mycsg SASnR2 3190_count_missing_selected_vars</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Count Missing Values in Selected Variables</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>A common audit question: how many of a given set of columns are missing on each row? &mdash; useful for completeness checks, data-quality flags, and reviewer queries.</li>
<li>We want a count per row, looking only at the columns we care about, not every column in the dataset.</li>
<li>SAS offers <code>CMISS()</code>, which works on both character and numeric inputs &mdash; just list the columns to check.</li>
<li>Tidyverse uses <code>across()</code> with <code>is.na()</code> and <code>rowSums()</code> to count NAs across selected columns.</li>
<li>Base R uses <code>rowSums(is.na(df[, cols]))</code> &mdash; the bracket subset picks the columns, <code>is.na()</code> builds a logical matrix, and <code>rowSums()</code> sums each row.</li>
<li>This lesson counts missingness across <code>name</code> and <code>score</code> on a small four-row dataset so each value is easy to inspect.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data example;
...
<li><code>CMISS()</code> is the SAS function that counts the missing values among its arguments &mdash; it accepts both numeric and character variables in the same call.</li>
<li><code>cmiss_subset = cmiss(name, score);</code> counts how many of <code>name</code> and <code>score</code> are missing on each row.</li>
<li>On a row where both are missing the result is 2; where only one is missing it is 1; where both are present it is 0.</li>
<li><code>CMISS()</code> ignores the type difference between <code>name</code> (character) and <code>score</code> (numeric) &mdash; no need to split the call.</li>
<li>For the count of non-missing values, <code>N(of var1 var2 ...)</code> is the complementary function &mdash; useful for "complete n" summaries.</li>
<li>We run and check <code>example2</code>: the <code>cmiss_subset</code> value on each row should equal the number of missing cells among <code>name</code> and <code>score</code>.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-tidyverse">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-tidyverse" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (tidyverse)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">df &lt;- tibble(
id = 1:4,
...
df2 &lt;- df %&gt;% mutate(cmiss_subset = rowSums(is.na(across(c(name, score)))))</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In tidyverse, we combine <code>across()</code> with <code>is.na()</code> and <code>rowSums()</code> to reproduce the SAS <code>CMISS()</code> row-wise count.</li>
<li><code>across(c(name, score))</code> selects the target columns; using <code>across()</code> keeps the call compact and supports any tidyselect helper.</li>
<li><code>is.na(across(...))</code> returns a logical data frame where TRUE marks missing cells.</li>
<li><code>rowSums(...)</code> sums those TRUE/FALSE values row by row, giving the missing-count per row.</li>
<li>We can swap <code>c(name, score)</code> for <code>where(is.character)</code> or <code>starts_with("lb")</code> to count missingness in any column family without listing names.</li>
<li>We run and confirm <code>df2$cmiss_subset</code> matches the SAS <code>example2.cmiss_subset</code> row for row.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">df &lt;- data.frame(
  id = 1:4,
  name = c(&quot;Alice&quot;, NA, &quot;Charlie&quot;, NA),
  age = c(25, 30, NA, NA),
  score = c(85, 90, 88, NA)
...
df2$cmiss_subset &lt;- rowSums(is.na(df2[, c(&quot;name&quot;, &quot;score&quot;)]))</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, the same idea is a one-liner: <code>rowSums(is.na(df[, c("name", "score")]))</code>.</li>
<li><code>df[, c("name", "score")]</code> picks the columns of interest; the comma keeps all rows.</li>
<li><code>is.na(...)</code> returns a logical matrix over those columns &mdash; TRUE where the value is missing.</li>
<li><code>rowSums(...)</code> sums each row, and because TRUE counts as 1 and FALSE as 0, the total is the missing-count per row.</li>
<li>For the complement, <code>rowSums(!is.na(...))</code> gives the count of non-missing values per row.</li>
<li>We run and confirm <code>df2$cmiss_subset</code> matches the SAS and tidyverse outputs; the code has no package dependencies.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data example;\n    input id name $ age score;\n    datalines;\n1 Alice 25 85\n2 . 30 90\n3 Charlie . 88\n4 . . .\n;\nrun;\n\ndata example2;\n    set example;\n    cmiss_subset = cmiss(name, score);\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data example;\n    input id name $ age score;\n    datalines;\n1 Alice 25 85\n2 . 30 90\n3 Charlie . 88\n4 . . .\n;\nrun;\n\ndata example2;\n    set example;\n    cmiss_subset = cmiss(name, score);\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"df <- tibble(\r\nid = 1:4,\r\nname = c(\"Alice\", NA, \"Charlie\", NA),\r\nage = c(25, 30, NA, NA),\r\nscore = c(85, 90, 88, NA) \r\n)\r\n \r\n\r\ndf2 <- df %>% mutate(cmiss_subset = rowSums(is.na(across(c(name, score)))))","language":"r","html":"<pre><code class=\"language-r\">df &lt;- tibble(\r\nid = 1:4,\r\nname = c(&quot;Alice&quot;, NA, &quot;Charlie&quot;, NA),\r\nage = c(25, 30, NA, NA),\r\nscore = c(85, 90, 88, NA) \r\n)\r\n \r\n\r\ndf2 &lt;- df %&gt;% mutate(cmiss_subset = rowSums(is.na(across(c(name, score)))))<\/code><\/pre>"},{"label":"R (base)","code":"df <- data.frame(\r\n  id = 1:4,\r\n  name = c(\"Alice\", NA, \"Charlie\", NA),\r\n  age = c(25, 30, NA, NA),\r\n  score = c(85, 90, 88, NA)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\ndf2 <- df\r\n\r\ndf2$cmiss_subset <- rowSums(is.na(df2[, c(\"name\", \"score\")]))","language":"r","html":"<pre><code class=\"language-r\">df &lt;- data.frame(\r\n  id = 1:4,\r\n  name = c(&quot;Alice&quot;, NA, &quot;Charlie&quot;, NA),\r\n  age = c(25, 30, NA, NA),\r\n  score = c(85, 90, 88, NA)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\ndf2 &lt;- df\r\n\r\ndf2$cmiss_subset &lt;- rowSums(is.na(df2[, c(&quot;name&quot;, &quot;score&quot;)]))<\/code><\/pre>"}] || [];
