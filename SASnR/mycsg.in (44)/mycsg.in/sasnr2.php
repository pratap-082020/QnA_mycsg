...
<title>mycsg SASnR2 3191_character_numeric_conversion</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Convert Between Character and Numeric Variables</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Type conversion between character and numeric is a constant need in clinical data &mdash; strings come in from CDISC specs, numbers come back out for analysis.</li>
<li>Two directions to know: character to numeric for analysis, numeric to character for report layout.</li>
<li>In both directions, invalid values (e.g. "ABC" in a numeric column) need to become missing rather than crashing the program.</li>
<li>SAS uses <code>input(char, informat.)</code> for character-to-numeric and <code>put(num, format.)</code> for numeric-to-character.</li>
<li>R uses <code>as.numeric()</code> and <code>as.character()</code> directly in both tidyverse and base dialects.</li>
<li>This lesson demonstrates both conversions on a mixed dataset that includes one invalid string ("ABC") so we can see how each language handles the edge case.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data convert;
...
data convert2;
    ...
    * Convert character to numeric;
    char_to_num = input(char_num, best.);

    * Convert numeric to character;
    num_to_char = put(num_val, best.);
run;</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>SAS splits the two conversion directions across two functions: <code>INPUT()</code> goes character-to-numeric, <code>PUT()</code> goes numeric-to-character.</li>
<li><code>char_to_num = input(char_num, best.);</code> reads the character value through a numeric informat and returns a numeric result.</li>
<li>The <code>best.</code> informat picks the most appropriate numeric reading automatically &mdash; good enough for most general-purpose conversions.</li>
<li>Invalid strings like <code>"ABC"</code> cannot be parsed as a number, so SAS returns missing and writes a note to the log.</li>
<li><code>num_to_char = put(num_val, best.);</code> writes the numeric value through a format and returns a character string.</li>
<li>The <code>best.</code> format gives SAS latitude over width and decimal placement; use something tighter like <code>8.2</code> when the report needs a fixed layout.</li>
<li>We run and confirm <code>convert2</code>: the ABC row should have <code>char_to_num = .</code>, and the missing-<code>num_val</code> row should show an empty <code>num_to_char</code>.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-tidyverse">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-tidyverse" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (tidyverse)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">convert &lt;- tribble(
~id, ~char_num, ~num_val,
&quot;1&quot;, &quot;100&quot;, 55,
&quot;2&quot;, &quot;200&quot;, 60,
&quot;3&quot;, &quot;ABC&quot;, 65,
...
<li>In tidyverse, the two base-R conversion functions do the whole job &mdash; no stringr or forcats needed for this simple case.</li>
<li><code>as.numeric(char_num)</code> reads each character value as a number; values that cannot be parsed become NA with a warning.</li>
<li><code>as.character(num_val)</code> writes each numeric value as a string &mdash; NA stays NA.</li>
<li>Both calls are wrapped in a single <code>mutate()</code>, so the result keeps the original columns plus the two derived ones.</li>
<li>For stricter parsing, <code>readr::parse_number()</code> is an alternative that handles thousand separators, currency symbols, and similar dressings.</li>
<li>We run and confirm <code>convert2</code>: the ABC row has <code>char_to_num = NA</code> and the NA-<code>num_val</code> row has <code>num_to_char = NA</code>.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">convert &lt;- data.frame(
  id = c(&quot;1&quot;, &quot;2&quot;, &quot;3&quot;, &quot;4&quot;, &quot;5&quot;),
...
convert2$num_to_char &lt;- as.character(convert2$num_val)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, the two conversions are one-liners using <code>as.numeric()</code> and <code>as.character()</code> directly on data frame columns.</li>
<li><code>convert2$char_to_num &lt;- as.numeric(convert2$char_num)</code> converts the character column; unparseable strings become NA with a warning.</li>
<li><code>convert2$num_to_char &lt;- as.character(convert2$num_val)</code> converts the numeric column; NA stays NA.</li>
<li>If we need to pre-clean the string (strip commas, percent signs, whitespace), do so with <code>gsub()</code> or <code>trimws()</code> before the <code>as.numeric()</code> call.</li>
<li>For formatted numeric-to-character output, <code>sprintf("%.2f", num_val)</code> or <code>format(num_val, nsmall = 2)</code> gives finer control.</li>
<li>We run and compare <code>convert2</code> against the SAS and tidyverse outputs; the ABC row should convert to NA in all three.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data convert;\n    input id $ char_num $ num_val;\n    datalines;\n1  100    55\n2  200    60\n3  ABC    65\n4  300    .\n5  .      75\n;\nrun;\n\ndata convert2;\n    set convert;\n\n    * Convert character to numeric;\n    char_to_num = input(char_num, best.);\n\n    * Convert numeric to character;\n    num_to_char = put(num_val, best.);\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data convert;\n    input id $ char_num $ num_val;\n    datalines;\n1  100    55\n2  200    60\n3  ABC    65\n4  300    .\n5  .      75\n;\nrun;\n\ndata convert2;\n    set convert;\n\n    * Convert character to numeric;\n    char_to_num = input(char_num, best.);\n\n    * Convert numeric to character;\n    num_to_char = put(num_val, best.);\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"convert <- tribble(\r\n~id, ~char_num, ~num_val,\r\n\"1\", \"100\", 55,\r\n\"2\", \"200\", 60,\r\n\"3\", \"ABC\", 65,\r\n\"4\", \"300\", NA,\r\n\"5\", NA, 75 )\r\n\r\n \r\nconvert2 <- convert %>%\r\n   mutate( char_to_num = as.numeric(char_num),\r\n     num_to_char = as.character(num_val)\r\n)","language":"r","html":"<pre><code class=\"language-r\">convert &lt;- tribble(\r\n~id, ~char_num, ~num_val,\r\n&quot;1&quot;, &quot;100&quot;, 55,\r\n&quot;2&quot;, &quot;200&quot;, 60,\r\n&quot;3&quot;, &quot;ABC&quot;, 65,\r\n&quot;4&quot;, &quot;300&quot;, NA,\r\n&quot;5&quot;, NA, 75 )\r\n\r\n \r\nconvert2 &lt;- convert %&gt;%\r\n   mutate( char_to_num = as.numeric(char_num),\r\n     num_to_char = as.character(num_val)\r\n)<\/code><\/pre>"},{"label":"R (base)","code":"convert <- data.frame(\r\n  id = c(\"1\", \"2\", \"3\", \"4\", \"5\"),\r\n  char_num = c(\"100\", \"200\", \"ABC\", \"300\", NA),\r\n  num_val = c(55, 60, 65, NA, 75)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nconvert2 <- convert\r\n\r\nconvert2$char_to_num <- as.numeric(convert2$char_num)\r\n\r\nconvert2$num_to_char <- as.character(convert2$num_val)","language":"r","html":"<pre><code class=\"language-r\">convert &lt;- data.frame(\r\n  id = c(&quot;1&quot;, &quot;2&quot;, &quot;3&quot;, &quot;4&quot;, &quot;5&quot;),\r\n  char_num = c(&quot;100&quot;, &quot;200&quot;, &quot;ABC&quot;, &quot;300&quot;, NA),\r\n  num_val = c(55, 60, 65, NA, 75)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nconvert2 &lt;- convert\r\n\r\nconvert2$char_to_num &lt;- as.numeric(convert2$char_num)\r\n\r\nconvert2$num_to_char &lt;- as.character(convert2$num_val)<\/code><\/pre>"}] || [];
