...
<title>mycsg SASnR2 3193_keep_nonmissing_columns</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Keep Only Variables with Non-missing Values Across Dataset</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>In real clinical data, some expected columns are never collected &mdash; every row is missing for them.</li>
<li>A useful clean-up pass: drop any column whose values are entirely missing across the dataset, keeping only columns that carry at least one real observation.</li>
<li>SAS does this by walking the <code>_character_</code> and <code>_numeric_</code> arrays, collecting variable names with at least one non-missing value, and passing that list to a <code>KEEP=</code> option via <code>call symputx</code>.</li>
<li>Tidyverse uses <code>select(where(...))</code> with a predicate that returns TRUE when any value in the column is non-missing.</li>
<li>Base R uses <code>sapply()</code> to build a column-level logical mask, then subsets the data frame with that mask.</li>
<li>This lesson cleans up a <code>lab</code> dataset where <code>ldh</code> and <code>crp</code> are fully missing, dropping those two columns from the result.</li>
...
data _null_;
    ...
    do i = 1 to dim(charvars);
        if charvars{i} ne &#039;&#039; then
            keep_chars = catx(&#039; &#039;, keep_chars, vname(charvars{i}));
    end;

    do j = 1 to dim(numvars);
        if numvars{j} ne . then
            keep_nums = catx(&#039; &#039;, keep_nums, vname(numvars{j}));
    end;
...
<li>In SAS, the trick is to inspect every variable, collect the names of the ones with at least one non-missing value, and rebuild the dataset using a <code>KEEP=</code> list.</li>
<li>A <code>data _null_;</code> step walks the rows without producing output, using two arrays: <code>array charvars {*} _character_;</code> and <code>array numvars {*} _numeric_;</code>.</li>
<li>Two <code>RETAIN</code>ed accumulators (<code>keep_chars</code>, <code>keep_nums</code>) grow across rows; we use <code>catx(' ', ...)</code> to append names as we discover non-missing values.</li>
<li><code>vname(charvars{i})</code> returns the original variable name associated with the i-th array element &mdash; exactly what a KEEP list needs.</li>
<li>On the last row (<code>set lab end=last;</code>), <code>call symputx('keep_vars', catx(' ', keep_chars, keep_nums));</code> stores the final list into a macro variable.</li>
<li>A second DATA step then rebuilds the output: <code>data lab_clean; set lab(keep=&amp;keep_vars.); run;</code>.</li>
<li>We run and confirm LAB_CLEAN has the same rows as LAB but drops the fully-missing columns <code>ldh</code> and <code>crp</code>.</li>
...
lab_clean &lt;- lab %&gt;%
select(where(~ any(!is.na(.) &amp; . != &quot;&quot;)))</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In tidyverse, <code>select(where(...))</code> is the one-line version: keep only columns that pass a predicate.</li>
<li><code>select(where(~ any(!is.na(.) &amp; . != "")))</code> keeps columns that have at least one non-missing and non-blank value.</li>
<li>The <code>~</code> introduces an anonymous predicate; <code>.</code> stands for the current column vector.</li>
<li><code>any(!is.na(.) &amp; . != "")</code> returns TRUE if any value in the column is both non-NA and not an empty string &mdash; useful when a column has an NA alongside blanks.</li>
<li>The resulting <code>lab_clean</code> keeps the same row count as <code>lab</code> but sheds the fully-missing columns.</li>
<li>We run and compare column counts before and after: the drop should match whichever columns are entirely missing in the source data.</li>
...
lab_clean_tmp</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, the same pattern uses <code>sapply()</code> to build a column-level logical mask, then subsets with bracket notation.</li>
<li><code>keep_cols &lt;- sapply(df, function(x) any(!is.na(x) &amp; x != "" &amp; x != "NA"))</code> returns a named logical vector &mdash; TRUE for columns worth keeping.</li>
<li>Inside the helper, <code>any(!is.na(x) &amp; x != "" &amp; x != "NA")</code> returns TRUE when at least one value is neither NA, an empty string, nor the literal string "NA".</li>
<li><code>df[, keep_cols]</code> subsets the data frame to the columns whose mask entry is TRUE; the comma keeps every row.</li>
<li>For strictly-NA columns (no blank or "NA" strings to worry about), the predicate simplifies to <code>any(!is.na(x))</code>.</li>
<li>We run and confirm the output drops any column that is fully missing; real data columns pass through unchanged.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data lab;\n    input usubjid $ hgb glu chol$ alb alt$ ast ldh$ crp;\n    datalines;\n101 12.5 110 POS 4.2 NEG 21 . .\n102 13.0 105 NEG 4.1 NEG 25 . .\n103 . . . . . . . .\n104 14.1 108 POS 4.5 NEG 20 . .\n;\nrun;\n\ndata _null_;\n    set lab end=last;\n\n    array charvars {*} _character_;\n    array numvars {*} _numeric_;\n\n    length keep_chars keep_nums $32767;\n\n    retain keep_chars keep_nums;\n\n    do i = 1 to dim(charvars);\n        if charvars{i} ne '' then\n            keep_chars = catx(' ', keep_chars, vname(charvars{i}));\n    end;\n\n    do j = 1 to dim(numvars);\n        if numvars{j} ne . then\n            keep_nums = catx(' ', keep_nums, vname(numvars{j}));\n    end;\n\n    if last then\n        call symputx('keep_vars', catx(' ', keep_chars, keep_nums));\nrun;\n\ndata lab_clean;\n    set lab(keep=&keep_vars.);\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data lab;\n    input usubjid $ hgb glu chol$ alb alt$ ast ldh$ crp;\n    datalines;\n101 12.5 110 POS 4.2 NEG 21 . .\n102 13.0 105 NEG 4.1 NEG 25 . .\n103 . . . . . . . .\n104 14.1 108 POS 4.5 NEG 20 . .\n;\nrun;\n\ndata _null_;\n    set lab end=last;\n\n    array charvars {*} _character_;\n    array numvars {*} _numeric_;\n\n    length keep_chars keep_nums $32767;\n\n    retain keep_chars keep_nums;\n\n    do i = 1 to dim(charvars);\n        if charvars{i} ne &#039;&#039; then\n            keep_chars = catx(&#039; &#039;, keep_chars, vname(charvars{i}));\n    end;\n\n    do j = 1 to dim(numvars);\n        if numvars{j} ne . then\n            keep_nums = catx(&#039; &#039;, keep_nums, vname(numvars{j}));\n    end;\n\n    if last then\n        call symputx(&#039;keep_vars&#039;, catx(&#039; &#039;, keep_chars, keep_nums));\nrun;\n\ndata lab_clean;\n    set lab(keep=&amp;keep_vars.);\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\r\n\r\nlab <- tribble(\r\n~usubjid, ~hgb, ~glu, ~chol, ~alb, ~alt, ~ast, ~ldh, ~crp,\r\n\"101\", 12.5, 110, \"POS\", 4.2, \"NEG\", 21, NA, NA,\r\n\"102\", 13.0, 105, \"NEG\", 4.1, \"NEG\", 25, NA, NA,\r\n\"103\", NA, NA, NA, NA, NA, NA, NA, NA,\r\n\"104\", 14.1, 108, \"POS\", 4.5, \"NEG\", 20, NA, NA )\r\n \r\n\r\nlab_clean <- lab %>%\r\nselect(where(~ any(!is.na(.) & . != \"\")))","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\r\n\r\nlab &lt;- tribble(\r\n~usubjid, ~hgb, ~glu, ~chol, ~alb, ~alt, ~ast, ~ldh, ~crp,\r\n&quot;101&quot;, 12.5, 110, &quot;POS&quot;, 4.2, &quot;NEG&quot;, 21, NA, NA,\r\n&quot;102&quot;, 13.0, 105, &quot;NEG&quot;, 4.1, &quot;NEG&quot;, 25, NA, NA,\r\n&quot;103&quot;, NA, NA, NA, NA, NA, NA, NA, NA,\r\n&quot;104&quot;, 14.1, 108, &quot;POS&quot;, 4.5, &quot;NEG&quot;, 20, NA, NA )\r\n \r\n\r\nlab_clean &lt;- lab %&gt;%\r\nselect(where(~ any(!is.na(.) &amp; . != &quot;&quot;)))<\/code><\/pre>"},{"label":"R (base)","code":"lab <- data.frame(\r\n  usubjid = c(101, 102, 103, 104),\r\n  hgb  = c(\"12.5\", \"13.0\", \"NA\", \"14.1\"),\r\n  glu  = c(\"110\", \"105\", \"NA\", \"108\"),\r\n  chol = c(\"POS\", \"NEG\", \"NA\", \"POS\"),\r\n  alb  = c(\"4.2\", \"4.1\", \"NA\", \"4.5\"),\r\n  alt  = c(\"NEG\", \"NEG\", \"NA\", \"NEG\"),\r\n  ast  = c(\"21\", \"25\", \"NA\", \"20\"),\r\n  ldh  = c(\"NA\", \"NA\", \"NA\", \"NA\"),\r\n  crp  = c(\"NA\", \"NA\", \"NA\", \"NA\"),\r\n  stringsAsFactors = FALSE\r\n)\r\n\r\nlab_clean_tmp <- lab\r\n\r\n# identify columns that have at least one non-\"NA\" and non-blank value\r\nkeep_cols <- sapply(lab_clean_tmp, function(x) {\r\n  any(!is.na(x) & x != \"\" & x != \"NA\")\r\n})\r\n\r\n# subset dataset\r\nlab_clean_tmp <- lab_clean_tmp[, keep_cols]\r\n\r\nlab_clean_tmp","language":"r","html":"<pre><code class=\"language-r\">lab &lt;- data.frame(\r\n  usubjid = c(101, 102, 103, 104),\r\n  hgb  = c(&quot;12.5&quot;, &quot;13.0&quot;, &quot;NA&quot;, &quot;14.1&quot;),\r\n  glu  = c(&quot;110&quot;, &quot;105&quot;, &quot;NA&quot;, &quot;108&quot;),\r\n  chol = c(&quot;POS&quot;, &quot;NEG&quot;, &quot;NA&quot;, &quot;POS&quot;),\r\n  alb  = c(&quot;4.2&quot;, &quot;4.1&quot;, &quot;NA&quot;, &quot;4.5&quot;),\r\n  alt  = c(&quot;NEG&quot;, &quot;NEG&quot;, &quot;NA&quot;, &quot;NEG&quot;),\r\n  ast  = c(&quot;21&quot;, &quot;25&quot;, &quot;NA&quot;, &quot;20&quot;),\r\n  ldh  = c(&quot;NA&quot;, &quot;NA&quot;, &quot;NA&quot;, &quot;NA&quot;),\r\n  crp  = c(&quot;NA&quot;, &quot;NA&quot;, &quot;NA&quot;, &quot;NA&quot;),\r\n  stringsAsFactors = FALSE\r\n)\r\n\r\nlab_clean_tmp &lt;- lab\r\n\r\n# identify columns that have at least one non-&quot;NA&quot; and non-blank value\r\nkeep_cols &lt;- sapply(lab_clean_tmp, function(x) {\r\n  any(!is.na(x) &amp; x != &quot;&quot; &amp; x != &quot;NA&quot;)\r\n})\r\n\r\n# subset dataset\r\nlab_clean_tmp &lt;- lab_clean_tmp[, keep_cols]\r\n\r\nlab_clean_tmp<\/code><\/pre>"}] || [];
