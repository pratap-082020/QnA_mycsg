...
<title>mycsg SASnR2 3201_Apply_Basic_Numeric_Functions</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Apply Basic Numeric Functions</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Basic numeric functions &mdash; round, ceil, floor, truncate, absolute value, modulo &mdash; show up everywhere in data manipulation, from banding continuous scores to building derived flags.</li>
<li>Most SAS numeric functions have direct analogues in R, with one notable exception: the sign convention for the modulo operator differs between SAS and R.</li>
<li>SAS offers <code>ROUND</code>, <code>CEIL</code>, <code>FLOOR</code>, <code>INT</code>, <code>ABS</code>, and <code>MOD</code>.</li>
<li>R offers <code>round</code>, <code>ceiling</code>, <code>floor</code>, <code>trunc</code>, <code>abs</code>, and <code>%%</code> (with a small helper needed to match SAS <code>MOD</code>).</li>
<li>This lesson applies each function to a vector that includes negative values, a zero, and positive fractions, so edge cases are easy to see.</li>
<li>Pay particular attention to the two modulo columns &mdash; they only diverge on negative numerators, which is exactly where the two conventions disagree.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data numbers;
input num;
round_val = round(num);
ceil_val = ceil(num);
floor_val = floor(num);
...
<li>This lesson walks through the most common SAS numeric functions applied to a small vector of floats.</li>
<li><code>ROUND(num)</code> rounds to the nearest whole number using half-up banker rounding.</li>
<li><code>CEIL(num)</code> rounds up towards positive infinity &mdash; always gives the next integer at or above the value.</li>
<li><code>FLOOR(num)</code> rounds down towards negative infinity &mdash; always gives the next integer at or below the value.</li>
<li><code>INT(num)</code> truncates towards zero &mdash; drops the fractional part without rounding.</li>
<li><code>ABS(num)</code> returns the magnitude, always non-negative.</li>
<li><code>MOD(num, 3)</code> returns the remainder after integer division by 3, with the same sign as the numerator.</li>
<li>After running, we inspect the <code>numbers</code> dataset and confirm each derived column reflects the expected rounding or remainder rule.</li>
<li>Useful for building derived flags (for example, rounding a continuous age to a whole number) or for arithmetic grouping.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-tidyverse">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-tidyverse" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (tidyverse)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">mod_sas_style &lt;- function(x, y) {
  x - y * trunc(x / y)
}
...
<li>Most of the SAS numeric functions have direct base R equivalents; tidyverse pipelines them inside <code>mutate()</code>.</li>
<li><code>round(num)</code> matches SAS <code>ROUND()</code> &mdash; although R uses banker's rounding (round-half-to-even), which differs subtly from SAS for .5 values.</li>
<li><code>ceiling(num)</code> is the R equivalent of SAS <code>CEIL()</code>.</li>
<li><code>floor(num)</code> matches SAS <code>FLOOR()</code> exactly.</li>
<li><code>trunc(num)</code> truncates towards zero, matching SAS <code>INT()</code>.</li>
<li><code>abs(num)</code> is identical to SAS <code>ABS()</code>.</li>
<li>For modulo, R has two options. <code>num %% 3</code> is R's built-in modulo, but it follows the mathematical convention (result has the sign of the divisor). SAS <code>MOD()</code> uses the truncation convention (sign of the numerator).</li>
<li>We define a small helper <code>mod_sas_style(x, y) = x - y * trunc(x / y)</code> to reproduce the SAS behaviour exactly, and include both for comparison.</li>
<li>After running, we confirm <code>mod_val1</code> (R convention) and <code>mod_val2</code> (SAS convention) only disagree on negative inputs &mdash; exactly where the two conventions diverge.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">mod_sas_style &lt;- function(x, y) {
  x - y * trunc(x / y)
}
...
df$mod_val2 &lt;- mod_sas_style(df$num, 3)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>Base R covers the same numeric operations one-for-one, with <code>round</code>, <code>ceiling</code>, <code>floor</code>, <code>trunc</code>, <code>abs</code>, and the <code>%%</code> operator.</li>
<li><code>round(x)</code> uses banker's rounding (round-half-to-even), which differs from SAS <code>ROUND()</code> for values ending in .5; <code>round(1.5)</code> returns 2 in both, but <code>round(2.5)</code> returns 2 in R and 3 in SAS.</li>
<li><code>ceiling(x)</code>, <code>floor(x)</code>, and <code>trunc(x)</code> behave identically to SAS <code>CEIL()</code>, <code>FLOOR()</code>, and <code>INT()</code>.</li>
<li><code>abs(x)</code> matches SAS <code>ABS()</code>.</li>
<li>For modulo, <code>x %% y</code> follows the mathematical convention (result takes the sign of the divisor); SAS <code>MOD()</code> takes the sign of the numerator.</li>
<li>The helper <code>mod_sas_style(x, y) &lt;- function(x, y) x - y * trunc(x / y)</code> reproduces the SAS convention exactly, giving the same answer on every row.</li>
<li>We run and compare <code>mod_val1</code> (R convention) with <code>mod_val2</code> (SAS convention) &mdash; they agree for non-negative inputs and disagree for negative inputs.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data numbers;\ninput num;\nround_val = round(num);\nceil_val = ceil(num);\nfloor_val = floor(num);\nint_val = int(num);\nabs_val = abs(num);\nmod_val = mod(num, 3);\ndatalines;\n-2.51\n-1.2\n0\n1.3\n2.7\n;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data numbers;\ninput num;\nround_val = round(num);\nceil_val = ceil(num);\nfloor_val = floor(num);\nint_val = int(num);\nabs_val = abs(num);\nmod_val = mod(num, 3);\ndatalines;\n-2.51\n-1.2\n0\n1.3\n2.7\n;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"mod_sas_style <- function(x, y) {\r\n  x - y * trunc(x \/ y)\r\n}\r\n\r\ndf <- tibble(\r\n  num = c(-2.51, -1.2, 0, 1.3, 2.7)\r\n) %>%\r\n  mutate(\r\n    round_val = round(num),\r\n    ceil_val  = ceiling(num),\r\n    floor_val = floor(num),\r\n    int_val   = trunc(num),\r\n    abs_val   = abs(num),\r\n\r\nmod_val1   = num%%3, \r\n    mod_val2   = mod_sas_style(num, 3)  \r\n  )","language":"r","html":"<pre><code class=\"language-r\">mod_sas_style &lt;- function(x, y) {\r\n  x - y * trunc(x \/ y)\r\n}\r\n\r\ndf &lt;- tibble(\r\n  num = c(-2.51, -1.2, 0, 1.3, 2.7)\r\n) %&gt;%\r\n  mutate(\r\n    round_val = round(num),\r\n    ceil_val  = ceiling(num),\r\n    floor_val = floor(num),\r\n    int_val   = trunc(num),\r\n    abs_val   = abs(num),\r\n\r\nmod_val1   = num%%3, \r\n    mod_val2   = mod_sas_style(num, 3)  \r\n  )<\/code><\/pre>"},{"label":"R (base)","code":"mod_sas_style <- function(x, y) {\r\n  x - y * trunc(x \/ y)\r\n}\r\n\r\ndf <- data.frame(\r\n  num = c(-2.51, -1.2, 0, 1.3, 2.7)\r\n)\r\n\r\ndf$round_val <- round(df$num)\r\n\r\ndf$ceil_val <- ceiling(df$num)\r\n\r\ndf$floor_val <- floor(df$num)\r\n\r\ndf$int_val <- trunc(df$num)\r\n\r\ndf$abs_val <- abs(df$num)\r\n\r\ndf$mod_val1 <- df$num %% 3\r\n\r\ndf$mod_val2 <- mod_sas_style(df$num, 3)","language":"r","html":"<pre><code class=\"language-r\">mod_sas_style &lt;- function(x, y) {\r\n  x - y * trunc(x \/ y)\r\n}\r\n\r\ndf &lt;- data.frame(\r\n  num = c(-2.51, -1.2, 0, 1.3, 2.7)\r\n)\r\n\r\ndf$round_val &lt;- round(df$num)\r\n\r\ndf$ceil_val &lt;- ceiling(df$num)\r\n\r\ndf$floor_val &lt;- floor(df$num)\r\n\r\ndf$int_val &lt;- trunc(df$num)\r\n\r\ndf$abs_val &lt;- abs(df$num)\r\n\r\ndf$mod_val1 &lt;- df$num %% 3\r\n\r\ndf$mod_val2 &lt;- mod_sas_style(df$num, 3)<\/code><\/pre>"}] || [];
