...
<title>mycsg SASnR2 3210_rounding_values</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Round Numeric Values</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Rounding is a daily need in report layout and derived-variable building &mdash; to an integer, to a decimal place, up to the next integer, down to the previous one.</li>
<li>Four common asks: round to nearest integer, round to a decimal place, ceiling, and floor.</li>
<li>SAS offers <code>round(value)</code> for nearest integer, <code>round(value, 0.01)</code> for a decimal place, and <code>ceil</code> / <code>floor</code> for directional rounding.</li>
<li>R uses <code>round()</code> with an optional digits argument, plus <code>ceiling()</code> and <code>floor()</code>.</li>
<li>A subtle difference: R's <code>round()</code> uses banker's rounding (round-half-to-even), whereas SAS rounds half up.</li>
<li>This lesson rounds a short vector that includes 1.5 and -4.75 so the half-case difference becomes visible.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data round_demo;
    input value;
    datalines;
3.1416
2.718
...
data round_results;
    ...
    round_dec = round(value,0.01);
    ceil_val = ceil(value);
    floor_val = floor(value);
run;</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In SAS, <code>round(value)</code> rounds to the nearest integer using the round-half-up rule.</li>
<li><code>round(value, 0.01)</code> rounds to the nearest two decimal places; the second argument is the rounding unit, not a digit count.</li>
<li><code>ceil(value)</code> returns the smallest integer greater than or equal to the value &mdash; always steps upward.</li>
<li><code>floor(value)</code> returns the largest integer less than or equal to the value &mdash; always steps downward.</li>
<li>On 1.5, SAS <code>round()</code> returns 2; on -4.75 rounded to two decimals, it returns -4.75 (already aligned).</li>
<li>We run and confirm each derived column reflects the intended rounding rule; pay attention to the 1.5 row as the half-case test.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-tidyverse">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-tidyverse" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (tidyverse)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">round_demo &lt;- tribble( ~value, 
                     3.1416, 
                     2.718, 
                     9.99, 
                     1.5, 
...
<li>In R, <code>round(value)</code> rounds to the nearest integer, but uses banker's rounding (round-half-to-even).</li>
<li><code>round(value, 2)</code> rounds to two decimal places; the second argument is a digit count, not a rounding unit &mdash; a small naming difference from SAS.</li>
<li><code>ceiling(value)</code> matches SAS <code>CEIL()</code>; <code>floor(value)</code> matches SAS <code>FLOOR()</code>.</li>
<li>For exact parity with SAS on half-cases, a custom helper <code>round_sas &lt;- function(x, n = 0) trunc(x * 10^n + sign(x) * 0.5) / 10^n</code> replicates round-half-away-from-zero.</li>
<li>The tidyverse wrapper here is just <code>mutate()</code> with the base R functions &mdash; no extra packages needed for simple rounding.</li>
<li>We run and compare the <code>round_val</code> column across SAS and R: 1.5 rounds to 2 in both, but 2.5 would round to 2 in R and 3 in SAS.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">round_demo &lt;- data.frame(
...
round_results$floor_val &lt;- floor(round_results$value)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>Base R uses the same four functions: <code>round()</code>, <code>round(x, digits)</code>, <code>ceiling()</code>, and <code>floor()</code>.</li>
<li>Assignments go column by column: <code>round_results$round_val &lt;- round(round_results$value)</code> and so on.</li>
<li>The digits argument of <code>round()</code> accepts any non-negative integer; negative values round to tens, hundreds, thousands.</li>
<li><code>ceiling()</code> and <code>floor()</code> are direct equivalents of SAS <code>CEIL()</code> and <code>FLOOR()</code>.</li>
<li>As with the tidyverse version, R's banker's rounding differs from SAS for the .5 case &mdash; the helper pattern above can be used to bridge the gap when exact parity is required.</li>
<li>We run and verify the four derived columns; the only systematic difference from SAS will be on half-case rows.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data round_demo;\n    input value;\n    datalines;\n3.1416\n2.718\n9.99\n1.5\n-4.75\n;\nrun;\n\ndata round_results;\n    set round_demo;\n    round_val = round(value);\n    round_dec = round(value,0.01);\n    ceil_val = ceil(value);\n    floor_val = floor(value);\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data round_demo;\n    input value;\n    datalines;\n3.1416\n2.718\n9.99\n1.5\n-4.75\n;\nrun;\n\ndata round_results;\n    set round_demo;\n    round_val = round(value);\n    round_dec = round(value,0.01);\n    ceil_val = ceil(value);\n    floor_val = floor(value);\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"round_demo <- tribble( ~value, \r\n                     3.1416, \r\n                     2.718, \r\n                     9.99, \r\n                     1.5, \r\n                     -4.75 ) \r\n\r\nround_results <- round_demo %>% \r\n  mutate( \r\n    round_val = round(value), \r\n    round_dec = round(value,2), \r\n    ceil_val = ceiling(value), \r\n    floor_val = floor(value) \r\n    )","language":"r","html":"<pre><code class=\"language-r\">round_demo &lt;- tribble( ~value, \r\n                     3.1416, \r\n                     2.718, \r\n                     9.99, \r\n                     1.5, \r\n                     -4.75 ) \r\n\r\nround_results &lt;- round_demo %&gt;% \r\n  mutate( \r\n    round_val = round(value), \r\n    round_dec = round(value,2), \r\n    ceil_val = ceiling(value), \r\n    floor_val = floor(value) \r\n    )<\/code><\/pre>"},{"label":"R (base)","code":"round_demo <- data.frame(\r\n  value = c(3.1416, 2.718, 9.99, 1.5, -4.75)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nround_results <- round_demo\r\n\r\nround_results$round_val <- round(round_results$value)\r\n\r\nround_results$round_dec <- round(round_results$value, 2)\r\n\r\nround_results$ceil_val <- ceiling(round_results$value)\r\n\r\nround_results$floor_val <- floor(round_results$value)","language":"r","html":"<pre><code class=\"language-r\">round_demo &lt;- data.frame(\r\n  value = c(3.1416, 2.718, 9.99, 1.5, -4.75)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nround_results &lt;- round_demo\r\n\r\nround_results$round_val &lt;- round(round_results$value)\r\n\r\nround_results$round_dec &lt;- round(round_results$value, 2)\r\n\r\nround_results$ceil_val &lt;- ceiling(round_results$value)\r\n\r\nround_results$floor_val &lt;- floor(round_results$value)<\/code><\/pre>"}] || [];
