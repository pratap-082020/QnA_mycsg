...
<title>mycsg SASnR2 3214_cumulative_operations</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Cumulative Sum, Mean, and Product</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Running totals show up everywhere &mdash; cumulative exposure, cumulative score, running product for compounding, rolling average for trends.</li>
<li>A cumulative function takes a vector and returns a vector of the same length where each element is the accumulated result up to that point.</li>
<li>SAS reaches for <code>RETAIN</code> inside a DATA step, together with sum statements (<code>cum_sum + score;</code>) for addition and explicit multiplication for products.</li>
<li>R has built-ins: <code>cumsum()</code> for running sum, <code>cumprod()</code> for running product, and derived forms for running average.</li>
<li>Running average is a quotient of the running sum and the running row count &mdash; both SAS and R build it the same way.</li>
<li>This lesson computes running sum, product, and average on a five-row score vector so every step is easy to verify.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data scores;
    input id score;
    datalines;
1 10
2 15
...
<li>In SAS, cumulative operations live inside a DATA step with a <code>RETAIN</code> statement holding the running totals.</li>
<li>We initialise all accumulators on the first row: <code>if _n_ = 1 then do; cum_sum = score; cum_prod = score; count = 1; cum_avg = score; end;</code>.</li>
<li>On every subsequent row we update each accumulator: <code>cum_sum + score;</code> adds to the running sum using the sum-statement shortcut.</li>
<li><code>cum_prod = cum_prod * score;</code> multiplies the running product by the new value &mdash; no shortcut operator exists for multiplication.</li>
<li><code>count + 1;</code> counts how many rows have been processed; <code>cum_avg = cum_sum / count;</code> derives the running average.</li>
<li>The sum-statement form (<code>var + expr;</code>) retains the variable and treats missing values as zero &mdash; a quiet convenience built into the SAS language.</li>
<li>We run and verify the CUMULATIVE dataset: <code>cum_sum</code> grows monotonically, <code>cum_prod</code> multiplies through, and <code>cum_avg</code> is the running mean.</li>
...
<li>In tidyverse, the three cumulative forms fit in a single <code>mutate()</code> call, using built-in functions <code>cumsum</code>, <code>cumprod</code>, and <code>row_number</code>.</li>
<li><code>cumsum(score)</code> returns a vector of running totals &mdash; position i is the sum of the first i values.</li>
<li><code>cumprod(score)</code> returns a vector of running products &mdash; position i is the product of the first i values.</li>
<li>For the running average, we divide the cumulative sum by the running row count: <code>cum_avg = cum_sum / row_number()</code>.</li>
<li><code>row_number()</code> inside <code>mutate()</code> gives 1, 2, 3, ... &mdash; the running count we need as the denominator.</li>
<li>If the data is grouped, wrap these in <code>group_by()</code> so the cumulatives reset at each group boundary.</li>
<li>We run and compare CUMULATIVE against the SAS output &mdash; the three derived columns should match row for row.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">scores &lt;- data.frame(
  id = c(1, 2, 3, 4, 5),
...
cumulative$cum_avg &lt;- cumulative$cum_sum / seq_along(cumulative$score)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, the same three cumulative functions exist: <code>cumsum()</code>, <code>cumprod()</code>, and a row-count helper.</li>
<li><code>cumulative$cum_sum &lt;- cumsum(cumulative$score)</code> adds the running total as a new column.</li>
<li><code>cumulative$cum_prod &lt;- cumprod(cumulative$score)</code> adds the running product.</li>
<li>For the running average, <code>seq_along(cumulative$score)</code> returns the row index vector 1, 2, ..., n &mdash; the denominator for the mean.</li>
<li><code>cumulative$cum_avg &lt;- cumulative$cum_sum / seq_along(cumulative$score)</code> gives the running mean row by row.</li>
<li>For grouped cumulatives in base R, <code>ave(score, group, FUN = cumsum)</code> applies <code>cumsum</code> separately within each group.</li>
<li>We run and confirm the CUMULATIVE data frame matches the tidyverse and SAS outputs.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data scores;\n    input id score;\n    datalines;\n1 10\n2 15\n3 20\n4 25\n5 30\n;\nrun;\n\ndata cumulative;\n    set scores;\n    retain cum_sum cum_prod cum_avg count;\n\n    if _n_ = 1 then do;\n        cum_sum = score;\n        cum_prod = score;\n        count = 1;\n        cum_avg = score;\n    end;\n    else do;\n        cum_sum + score;\n        cum_prod = cum_prod * score;\n        count + 1;\n        cum_avg = cum_sum \/ count;\n    end;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data scores;\n    input id score;\n    datalines;\n1 10\n2 15\n3 20\n4 25\n5 30\n;\nrun;\n\ndata cumulative;\n    set scores;\n    retain cum_sum cum_prod cum_avg count;\n\n    if _n_ = 1 then do;\n        cum_sum = score;\n        cum_prod = score;\n        count = 1;\n        cum_avg = score;\n    end;\n    else do;\n        cum_sum + score;\n        cum_prod = cum_prod * score;\n        count + 1;\n        cum_avg = cum_sum \/ count;\n    end;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\r\n\r\nscores <- tribble(\r\n  ~id, ~score,\r\n  1, 10,\r\n  2, 15,\r\n  3, 20,\r\n  4, 25,\r\n  5, 30\r\n)\r\n\r\ncumulative <- scores %>%\r\n  mutate(\r\n    cum_sum = cumsum(score),\r\n    cum_prod = cumprod(score),\r\n    cum_avg = cum_sum \/ row_number()\r\n  )","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\r\n\r\nscores &lt;- tribble(\r\n  ~id, ~score,\r\n  1, 10,\r\n  2, 15,\r\n  3, 20,\r\n  4, 25,\r\n  5, 30\r\n)\r\n\r\ncumulative &lt;- scores %&gt;%\r\n  mutate(\r\n    cum_sum = cumsum(score),\r\n    cum_prod = cumprod(score),\r\n    cum_avg = cum_sum \/ row_number()\r\n  )<\/code><\/pre>"},{"label":"R (base)","code":"scores <- data.frame(\r\n  id = c(1, 2, 3, 4, 5),\r\n  score = c(10, 15, 20, 25, 30)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\ncumulative <- scores\r\n\r\ncumulative$cum_sum <- cumsum(cumulative$score)\r\n\r\ncumulative$cum_prod <- cumprod(cumulative$score)\r\n\r\ncumulative$cum_avg <- cumulative$cum_sum \/ seq_along(cumulative$score)","language":"r","html":"<pre><code class=\"language-r\">scores &lt;- data.frame(\r\n  id = c(1, 2, 3, 4, 5),\r\n  score = c(10, 15, 20, 25, 30)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\ncumulative &lt;- scores\r\n\r\ncumulative$cum_sum &lt;- cumsum(cumulative$score)\r\n\r\ncumulative$cum_prod &lt;- cumprod(cumulative$score)\r\n\r\ncumulative$cum_avg &lt;- cumulative$cum_sum \/ seq_along(cumulative$score)<\/code><\/pre>"}] || [];
