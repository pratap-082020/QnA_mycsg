...
<title>mycsg SASnR2 3407_rowwise_max_from_multiple_columns</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Find Maximum Value Across Multiple Variables in Each Row</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Clinical data often spreads repeated measures across columns &mdash; <code>hgb_v1</code>, <code>hgb_v2</code>, <code>hgb_v3</code> &mdash; and a derivation needs the per-row maximum across those columns.</li>
<li>The answer is a new column whose value on each row is the largest of the listed columns, ignoring missings.</li>
<li>SAS handles this in a DATA step with the function <code>max(of var1--varN)</code>, where the <code>of</code> keyword signals a variable list.</li>
<li>Tidyverse uses <code>rowwise()</code> + <code>mutate(max = max(...))</code> to evaluate the max one row at a time.</li>
<li>Base R uses <code>pmax()</code> (parallel max across vectors) or <code>apply(..., 1, max)</code> on selected columns.</li>
<li>This lesson picks the maximum hemoglobin across three visits, so each subject ends up with a single maximum value.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">data lab;
...
<li>In SAS, row-wise aggregation across columns uses functions with the <code>of</code> keyword followed by a variable list.</li>
<li><code>maxval = max(of hgb_v1--hgb_v3);</code> computes the maximum of the three columns on each row.</li>
<li>The double-dash (<code>--</code>) expands to every variable between <code>hgb_v1</code> and <code>hgb_v3</code> in column order &mdash; a concise list when the columns sit next to each other.</li>
<li>If any column is missing, SAS silently skips it in the <code>max()</code> call; a row with all three missing returns missing.</li>
<li>We can swap <code>max</code> for <code>min</code>, <code>mean</code>, <code>sum</code>, or <code>n</code> to compute any of the common row-wise summaries the same way.</li>
<li>We run and check the <code>maxval</code> column matches the largest of the three input columns on each row.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-tidyverse">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-tidyverse" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (tidyverse)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">lab &lt;- tribble(
...
<li>In tidyverse, <code>mutate()</code> works column-wise by default, so a naive <code>max(HGB_V1, HGB_V2, HGB_V3)</code> returns one value across the whole tibble, not one value per row.</li>
<li>The row-wise form uses <code>rowwise()</code> before <code>mutate()</code> &mdash; dplyr then evaluates the call one row at a time.</li>
<li><code>lab %&gt;% rowwise() %&gt;% mutate(maxval = max(HGB_V1, HGB_V2, HGB_V3))</code> gives the correct per-row maximum.</li>
<li>We remember to call <code>ungroup()</code> afterwards so the tibble goes back to ordinary (column-wise) behaviour for downstream steps.</li>
<li>An alternative that avoids <code>rowwise()</code>: <code>mutate(maxval = pmax(HGB_V1, HGB_V2, HGB_V3))</code> &mdash; <code>pmax()</code> is vectorised across the inputs.</li>
<li>For many columns, use <code>mutate(maxval = pmax(!!!across(starts_with("HGB_"))))</code> or the modern <code>across()</code> with <code>.fns = list(max = max)</code>.</li>
<li>We run and compare <code>lab01$maxval</code> (wrong, single value) against <code>lab02$maxval</code> (correct per-row) to see the difference.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">lab &lt;- data.frame(
...
lab02$maxval &lt;- apply(lab02[, c(&quot;HGB_V1&quot;, &quot;HGB_V2&quot;, &quot;HGB_V3&quot;)], 1, max)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>Base R offers two clean options for row-wise maxima: <code>pmax()</code> and <code>apply()</code>.</li>
<li><code>pmax(lab$HGB_V1, lab$HGB_V2, lab$HGB_V3)</code> returns the parallel maximum across the vectors &mdash; one value per row.</li>
<li>The "p" in <code>pmax()</code> means "parallel"; <code>max()</code> alone would collapse every value to a single overall maximum.</li>
<li>For a larger number of columns, <code>apply(lab[, c("HGB_V1", "HGB_V2", "HGB_V3")], 1, max)</code> applies <code>max</code> along the rows of the sub-frame (the <code>1</code> means "by row").</li>
<li>Both forms return the same numeric vector, which we assign back into the data frame as a new column.</li>
<li>For missing values, use <code>pmax(..., na.rm = TRUE)</code> or <code>apply(..., 1, max, na.rm = TRUE)</code> &mdash; otherwise NAs propagate.</li>
<li>We run and confirm the two approaches give identical results, matching the SAS <code>maxval</code> column row for row.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data lab;\ninput usubjid hgb_v1 hgb_v2 hgb_v3;\ndatalines;\n1001 12.5 13.1 12.8\n1002 13.0 14.0 13.5\n1003 11.2 11.8 12.0\n;\nrun;\n\ndata lab02;\n   set lab;\n   maxval = max(of hgb_v1--hgb_v3);\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data lab;\ninput usubjid hgb_v1 hgb_v2 hgb_v3;\ndatalines;\n1001 12.5 13.1 12.8\n1002 13.0 14.0 13.5\n1003 11.2 11.8 12.0\n;\nrun;\n\ndata lab02;\n   set lab;\n   maxval = max(of hgb_v1--hgb_v3);\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"lab <- tribble(\r\n  ~USUBJID, ~HGB_V1, ~HGB_V2, ~HGB_V3,\r\n  1001,     12.5,     13.1,    12.8,\r\n  1002,     13.0,     14,      13.5,\r\n  1003,     11.2,     11.8,    12.0\r\n)\r\n\r\nlab01 <- lab %>% \r\n  mutate(\r\n    maxval=max(HGB_V1,HGB_V2,HGB_V3)\r\n  )\r\n\r\nlab02 <- lab %>% \r\n  rowwise() %>% \r\n  mutate(\r\n    maxval=max(HGB_V1,HGB_V2,HGB_V3)\r\n  )","language":"r","html":"<pre><code class=\"language-r\">lab &lt;- tribble(\r\n  ~USUBJID, ~HGB_V1, ~HGB_V2, ~HGB_V3,\r\n  1001,     12.5,     13.1,    12.8,\r\n  1002,     13.0,     14,      13.5,\r\n  1003,     11.2,     11.8,    12.0\r\n)\r\n\r\nlab01 &lt;- lab %&gt;% \r\n  mutate(\r\n    maxval=max(HGB_V1,HGB_V2,HGB_V3)\r\n  )\r\n\r\nlab02 &lt;- lab %&gt;% \r\n  rowwise() %&gt;% \r\n  mutate(\r\n    maxval=max(HGB_V1,HGB_V2,HGB_V3)\r\n  )<\/code><\/pre>"},{"label":"R (base)","code":"lab <- data.frame(\r\n  USUBJID = c(1001, 1002, 1003),\r\n  HGB_V1 = c(12.5, 13.0, 11.2),\r\n  HGB_V2 = c(13.1, 14, 11.8),\r\n  HGB_V3 = c(12.8, 13.5, 12.0)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nlab01 <- lab\r\n\r\nlab01$maxval <- pmax(lab01$HGB_V1, lab01$HGB_V2, lab01$HGB_V3)\r\n\r\nlab02 <- lab\r\n\r\nlab02$maxval <- apply(lab02[, c(\"HGB_V1\", \"HGB_V2\", \"HGB_V3\")], 1, max)","language":"r","html":"<pre><code class=\"language-r\">lab &lt;- data.frame(\r\n  USUBJID = c(1001, 1002, 1003),\r\n  HGB_V1 = c(12.5, 13.0, 11.2),\r\n  HGB_V2 = c(13.1, 14, 11.8),\r\n  HGB_V3 = c(12.8, 13.5, 12.0)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nlab01 &lt;- lab\r\n\r\nlab01$maxval &lt;- pmax(lab01$HGB_V1, lab01$HGB_V2, lab01$HGB_V3)\r\n\r\nlab02 &lt;- lab\r\n\r\nlab02$maxval &lt;- apply(lab02[, c(&quot;HGB_V1&quot;, &quot;HGB_V2&quot;, &quot;HGB_V3&quot;)], 1, max)<\/code><\/pre>"}] || [];
