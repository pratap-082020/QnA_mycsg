...
<title>mycsg SASnR2 3800_compare-datasets</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Compare datasets</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>In clinical programming, we routinely compare two versions of the same dataset &mdash; for example the production DM and the independently programmed QC DM &mdash; to prove they match.</li>
<li>A good comparison should answer three questions:
<ul>
<li>Do both datasets have the same variables (names, types, lengths, labels)?</li>
<li>For subjects common to both datasets, do the variable values match?</li>
<li>Which subjects are in only one dataset and not the other?</li>
</ul>
</li>
<li>SAS uses <code>PROC COMPARE</code> with <code>BASE=</code> and <code>COMPARE=</code>. The log and listing together answer all three questions, and the return code (<code>&amp;sysinfo</code>) can be interrogated downstream in automation.</li>
<li>R has no single built-in equivalent, so we reach for a helper package. <code>diffdf</code> and <code>arsenal::comparedf()</code> are the two popular choices in clinical work &mdash; both print a structured report very similar in spirit to PROC COMPARE output.</li>
<li>The examples below build two small DM-like datasets, then run the same comparison in SAS, tidyverse R (using <code>diffdf</code>) and base R (using <code>arsenal</code>).</li>
<li>Before running, note the differences we deliberately seeded:
<ul>
<li>Subject <code>1005</code> has <code>sex = F</code> in <code>dm</code> but <code>sex = M</code> in <code>qc_dm</code>.</li>
<li>Subjects <code>1006</code> and <code>1007</code> are in <code>dm</code> but missing from <code>qc_dm</code>.</li>
<li>Subject <code>1008</code> is in <code>qc_dm</code> but not in <code>dm</code>.</li>
</ul>
</li>
<li>After running, confirm the report flags exactly these three discrepancies. If the report shows more or fewer, revisit the code.</li>
...
* Step 1: production DM (7 subjects)                                          ;
*==============================================================================;
data dm;
  infile cards truncover dlm=&quot;|&quot;;
  length domain $2 usubjid $5 sex $3;
...
*==============================================================================;
* Step 2: QC DM (6 subjects) — one mismatch, two missing, one extra          ;
*==============================================================================;
data qc_dm;
  infile cards truncover dlm=&quot;|&quot;;
  length domain $2 usubjid $5 sex $3;
...
* Step 3: compare                                                             ;
*==============================================================================;
proc compare base = dm compare = qc_dm listall;
  id usubjid;
run;

*------------------------------------------------------------------------------;
* Step 4 (optional): inspect the return code                                  ;
*------------------------------------------------------------------------------;
%put NOTE: PROC COMPARE sysinfo = &amp;sysinfo;</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To compare two datasets in SAS, we run <code>PROC COMPARE</code> with <code>base=</code> and <code>compare=</code> arguments. The one named on <code>base=</code> is the reference.</li>
<li>Every section of the report then reads as &ldquo;how does COMPARE differ from BASE&rdquo;, so we need to keep that framing in mind when explaining the output to a reviewer.</li>
<li>We typically add an <code>ID</code> statement so SAS knows which set of variables identifies a row. Without it, PROC COMPARE lines up records by position, which is rarely what we want.</li>
<li>With <code>id usubjid</code>, SAS matches on subject and then reports value mismatches per variable for the matched rows.</li>
<li>For the most thorough listing, we add the <code>listall</code> option. That asks for every available report &mdash; variables only in one side, observations only in one side, and the value-by-value comparison.</li>
<li>Verbose, but it leaves nothing to assumptions.</li>
<li>Things we confirm in the listing:
<ul>
<li>Variables summary &mdash; are all expected variables present in both datasets with matching types and lengths?</li>
<li>Observation summary &mdash; how many rows are in BASE only, in COMPARE only, and in both?</li>
<li>Value comparison results &mdash; for matched rows, which variables differ and for which subjects?</li>
</ul>
</li>
<li>When PROC COMPARE sits inside a validation macro, we can also check the return code <code>&amp;sysinfo</code>.</li>
<li>A value of <code>0</code> means everything matched; any non-zero value means at least one discrepancy.</li>
<li>Useful when we want the program to fail automatically on any mismatch.</li>
...
library(diffdf)  # install.packages(&quot;diffdf&quot;) if needed

#==============================================================================
# Step 1: production DM
#==============================================================================
dm &lt;- tribble(
  ~usubjid, ~sex, ~age,
  &quot;1001&quot;,   &quot;M&quot;,  72,
  ...
  mutate(domain = &quot;DM&quot;) %&gt;%
  relocate(domain, .before = usubjid)

#==============================================================================
# Step 2: QC DM — one mismatch, two missing, one extra
#==============================================================================
qc_dm &lt;- tribble(
  ~usubjid, ~sex, ~age,
  &quot;1001&quot;,   &quot;M&quot;,  72,
...
#==============================================================================
# Step 3: compare with diffdf (clinical-programming friendly)
#==============================================================================
diff_report &lt;- diffdf(
  base    = dm,
  compare = qc_dm,
...
print(diff_report)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>For a SAS-to-R style comparison in tidyverse-friendly code, we can reach for <code>diffdf</code>.</li>
<li>It is a standalone R package from openpharma (not part of tidyverse itself), written specifically for clinical programming work.</li>
<li>Its output is deliberately modelled on PROC COMPARE, which makes it friendly for reviewers already used to reading SAS listings.</li>
<li>To tell <code>diffdf()</code> which set of columns identifies a row, we pass <code>keys = "usubjid"</code>. This is the direct analogue of the SAS <code>ID</code> statement.</li>
<li>Without <code>keys</code>, rows are matched positionally &mdash; the same caveat as PROC COMPARE.</li>
<li>When we <code>print(diff_report)</code>, the output is split into sections: variables only in one side, rows only in one side, and value-level mismatches per variable.</li>
<li>Exact section names appear in that console output; we note them down the first time we run the code so later runs are quick to scan.</li>
<li><code>diff_report</code> is also an object we can inspect programmatically with <code>str()</code> or subscripting.</li>
<li>For automated checks inside a validation script, we test whether any issues were found &mdash; the object&rsquo;s own print method says &ldquo;No issues were found&rdquo; when the two inputs match, and lists the issues otherwise.</li>
<li>This plays the same role as checking <code>&amp;sysinfo</code> after SAS PROC COMPARE.</li>
<li>After running, we confirm three things in the printed report:
<ul>
<li>exactly one value difference for <code>1005</code>,</li>
<li>exactly two rows in BASE only (<code>1006</code>, <code>1007</code>),</li>
<li>exactly one row in COMPARE only (<code>1008</code>).</li>
</ul>
</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse2" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse2">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse2">Copy</button>      <span class="toggle-indicator" data-target="collapse2">+</span>    </div>  </div>  <div id="collapse2" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse2"><pre><code class="language-r">library(arsenal)  # install.packages(&quot;arsenal&quot;) if needed
...
dm &lt;- data.frame(
  domain  = &quot;DM&quot;,
  usubjid = c(&quot;1001&quot;,&quot;1002&quot;,&quot;1003&quot;,&quot;1004&quot;,&quot;1005&quot;,&quot;1006&quot;,&quot;1007&quot;),
  sex     = c(&quot;M&quot;,&quot;F&quot;,&quot;F&quot;,&quot;M&quot;,&quot;F&quot;,&quot;F&quot;,&quot;F&quot;),
  age     = c(72, 65, 45, 40, 35, 6, 5),
...
qc_dm &lt;- data.frame(
  domain  = &quot;DM&quot;,
  usubjid = c(&quot;1001&quot;,&quot;1002&quot;,&quot;1003&quot;,&quot;1004&quot;,&quot;1005&quot;,&quot;1008&quot;),
  sex     = c(&quot;M&quot;,&quot;F&quot;,&quot;F&quot;,&quot;M&quot;,&quot;M&quot;,&quot;M&quot;),
  age     = c(72, 65, 45, 40, 35, 2),
  stringsAsFactors = FALSE
)

#==============================================================================
# Step 3: compare with arsenal::comparedf (PROC COMPARE equivalent)
#==============================================================================
cmp &lt;- comparedf(
  x  = dm,
  y  = qc_dm,
...
summary(cmp)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To compare two data frames in plain base R, we can reach for <code>arsenal::comparedf(x, y, by = ...)</code>.</li>
<li>It mirrors <code>PROC COMPARE BASE=x COMPARE=y ID=...</code>. The <code>by</code> argument plays the role of the SAS <code>ID</code> statement.</li>
<li>We call <code>summary(cmp)</code> to get a multi-section report &mdash; number of rows in each input, rows not shared, variables not shared, differences by variable, and a row-level difference table.</li>
<li>Each section is readable on its own or together as a whole audit trail.</li>
<li>For programmatic checks (CI pipelines, validation macros in R), we inspect the returned object directly.</li>
<li><code>cmp$comparison.control</code>, <code>cmp$vars.summary</code> and <code>cmp$frame.summary</code> give structured access to every slice of the report.</li>
<li>Notice that we built the inputs with <code>data.frame()</code> instead of <code>tribble()</code> and skipped the pipe. That keeps the only external dependency at <code>arsenal</code> itself, so the snippet stays runnable inside a stripped-down base R environment with no tidyverse installed.</li>
<li>After running, we confirm the three deliberately seeded discrepancies:
<ul>
<li><code>1005</code> sex mismatch,</li>
<li><code>1006</code>/<code>1007</code> present only in <code>dm</code>,</li>
<li><code>1008</code> present only in <code>qc_dm</code>.</li>
</ul>
</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"*==============================================================================;\n* Step 1: production DM (7 subjects)                                          ;\n*==============================================================================;\ndata dm;\n  infile cards truncover dlm=\"|\";\n  length domain $2 usubjid $5 sex $3;\n  input usubjid $ sex $ age;\n  domain = \"DM\";\n  label usubjid = \"Unique Subject Identifier\";\n  cards;\n1001|M|72\n1002|F|65\n1003|F|45\n1004|M|40\n1005|F|35\n1006|F|6\n1007|F|5\n;\nrun;\n\n*==============================================================================;\n* Step 2: QC DM (6 subjects) \u2014 one mismatch, two missing, one extra          ;\n*==============================================================================;\ndata qc_dm;\n  infile cards truncover dlm=\"|\";\n  length domain $2 usubjid $5 sex $3;\n  input usubjid $ sex $ age;\n  domain = \"DM\";\n  cards;\n1001|M|72\n1002|F|65\n1003|F|45\n1004|M|40\n1005|M|35\n1008|M|2\n;\nrun;\n\n*==============================================================================;\n* Step 3: compare                                                             ;\n*==============================================================================;\nproc compare base = dm compare = qc_dm listall;\n  id usubjid;\nrun;\n\n*------------------------------------------------------------------------------;\n* Step 4 (optional): inspect the return code                                  ;\n*------------------------------------------------------------------------------;\n%put NOTE: PROC COMPARE sysinfo = &sysinfo;","language":"sas","html":"<pre><code class=\"language-sas-mini\">*==============================================================================;\n* Step 1: production DM (7 subjects)                                          ;\n*==============================================================================;\ndata dm;\n  infile cards truncover dlm=&quot;|&quot;;\n  length domain $2 usubjid $5 sex $3;\n  input usubjid $ sex $ age;\n  domain = &quot;DM&quot;;\n  label usubjid = &quot;Unique Subject Identifier&quot;;\n  cards;\n1001|M|72\n1002|F|65\n1003|F|45\n1004|M|40\n1005|F|35\n1006|F|6\n1007|F|5\n;\nrun;\n\n*==============================================================================;\n* Step 2: QC DM (6 subjects) \u2014 one mismatch, two missing, one extra          ;\n*==============================================================================;\ndata qc_dm;\n  infile cards truncover dlm=&quot;|&quot;;\n  length domain $2 usubjid $5 sex $3;\n  input usubjid $ sex $ age;\n  domain = &quot;DM&quot;;\n  cards;\n1001|M|72\n1002|F|65\n1003|F|45\n1004|M|40\n1005|M|35\n1008|M|2\n;\nrun;\n\n*==============================================================================;\n* Step 3: compare                                                             ;\n*==============================================================================;\nproc compare base = dm compare = qc_dm listall;\n  id usubjid;\nrun;\n\n*------------------------------------------------------------------------------;\n* Step 4 (optional): inspect the return code                                  ;\n*------------------------------------------------------------------------------;\n%put NOTE: PROC COMPARE sysinfo = &amp;sysinfo;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\nlibrary(diffdf)  # install.packages(\"diffdf\") if needed\n\n#==============================================================================\n# Step 1: production DM\n#==============================================================================\ndm <- tribble(\n  ~usubjid, ~sex, ~age,\n  \"1001\",   \"M\",  72,\n  \"1002\",   \"F\",  65,\n  \"1003\",   \"F\",  45,\n  \"1004\",   \"M\",  40,\n  \"1005\",   \"F\",  35,\n  \"1006\",   \"F\",   6,\n  \"1007\",   \"F\",   5\n) %>%\n  mutate(domain = \"DM\") %>%\n  relocate(domain, .before = usubjid)\n\n#==============================================================================\n# Step 2: QC DM \u2014 one mismatch, two missing, one extra\n#==============================================================================\nqc_dm <- tribble(\n  ~usubjid, ~sex, ~age,\n  \"1001\",   \"M\",  72,\n  \"1002\",   \"F\",  65,\n  \"1003\",   \"F\",  45,\n  \"1004\",   \"M\",  40,\n  \"1005\",   \"M\",  35,\n  \"1008\",   \"M\",   2\n) %>%\n  mutate(domain = \"DM\") %>%\n  relocate(domain, .before = usubjid)\n\n#==============================================================================\n# Step 3: compare with diffdf (clinical-programming friendly)\n#==============================================================================\ndiff_report <- diffdf(\n  base    = dm,\n  compare = qc_dm,\n  keys    = \"usubjid\"\n)\n\nprint(diff_report)","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\nlibrary(diffdf)  # install.packages(&quot;diffdf&quot;) if needed\n\n#==============================================================================\n# Step 1: production DM\n#==============================================================================\ndm &lt;- tribble(\n  ~usubjid, ~sex, ~age,\n  &quot;1001&quot;,   &quot;M&quot;,  72,\n  &quot;1002&quot;,   &quot;F&quot;,  65,\n  &quot;1003&quot;,   &quot;F&quot;,  45,\n  &quot;1004&quot;,   &quot;M&quot;,  40,\n  &quot;1005&quot;,   &quot;F&quot;,  35,\n  &quot;1006&quot;,   &quot;F&quot;,   6,\n  &quot;1007&quot;,   &quot;F&quot;,   5\n) %&gt;%\n  mutate(domain = &quot;DM&quot;) %&gt;%\n  relocate(domain, .before = usubjid)\n\n#==============================================================================\n# Step 2: QC DM \u2014 one mismatch, two missing, one extra\n#==============================================================================\nqc_dm &lt;- tribble(\n  ~usubjid, ~sex, ~age,\n  &quot;1001&quot;,   &quot;M&quot;,  72,\n  &quot;1002&quot;,   &quot;F&quot;,  65,\n  &quot;1003&quot;,   &quot;F&quot;,  45,\n  &quot;1004&quot;,   &quot;M&quot;,  40,\n  &quot;1005&quot;,   &quot;M&quot;,  35,\n  &quot;1008&quot;,   &quot;M&quot;,   2\n) %&gt;%\n  mutate(domain = &quot;DM&quot;) %&gt;%\n  relocate(domain, .before = usubjid)\n\n#==============================================================================\n# Step 3: compare with diffdf (clinical-programming friendly)\n#==============================================================================\ndiff_report &lt;- diffdf(\n  base    = dm,\n  compare = qc_dm,\n  keys    = &quot;usubjid&quot;\n)\n\nprint(diff_report)<\/code><\/pre>"},{"label":"R (base)","code":"library(arsenal)  # install.packages(\"arsenal\") if needed\n\n#==============================================================================\n# Step 1: production DM\n#==============================================================================\ndm <- data.frame(\n  domain  = \"DM\",\n  usubjid = c(\"1001\",\"1002\",\"1003\",\"1004\",\"1005\",\"1006\",\"1007\"),\n  sex     = c(\"M\",\"F\",\"F\",\"M\",\"F\",\"F\",\"F\"),\n  age     = c(72, 65, 45, 40, 35, 6, 5),\n  stringsAsFactors = FALSE\n)\n\n#==============================================================================\n# Step 2: QC DM \u2014 one mismatch, two missing, one extra\n#==============================================================================\nqc_dm <- data.frame(\n  domain  = \"DM\",\n  usubjid = c(\"1001\",\"1002\",\"1003\",\"1004\",\"1005\",\"1008\"),\n  sex     = c(\"M\",\"F\",\"F\",\"M\",\"M\",\"M\"),\n  age     = c(72, 65, 45, 40, 35, 2),\n  stringsAsFactors = FALSE\n)\n\n#==============================================================================\n# Step 3: compare with arsenal::comparedf (PROC COMPARE equivalent)\n#==============================================================================\ncmp <- comparedf(\n  x  = dm,\n  y  = qc_dm,\n  by = \"usubjid\"\n)\n\nsummary(cmp)","language":"r","html":"<pre><code class=\"language-r\">library(arsenal)  # install.packages(&quot;arsenal&quot;) if needed\n\n#==============================================================================\n# Step 1: production DM\n#==============================================================================\ndm &lt;- data.frame(\n  domain  = &quot;DM&quot;,\n  usubjid = c(&quot;1001&quot;,&quot;1002&quot;,&quot;1003&quot;,&quot;1004&quot;,&quot;1005&quot;,&quot;1006&quot;,&quot;1007&quot;),\n  sex     = c(&quot;M&quot;,&quot;F&quot;,&quot;F&quot;,&quot;M&quot;,&quot;F&quot;,&quot;F&quot;,&quot;F&quot;),\n  age     = c(72, 65, 45, 40, 35, 6, 5),\n  stringsAsFactors = FALSE\n)\n\n#==============================================================================\n# Step 2: QC DM \u2014 one mismatch, two missing, one extra\n#==============================================================================\nqc_dm &lt;- data.frame(\n  domain  = &quot;DM&quot;,\n  usubjid = c(&quot;1001&quot;,&quot;1002&quot;,&quot;1003&quot;,&quot;1004&quot;,&quot;1005&quot;,&quot;1008&quot;),\n  sex     = c(&quot;M&quot;,&quot;F&quot;,&quot;F&quot;,&quot;M&quot;,&quot;M&quot;,&quot;M&quot;),\n  age     = c(72, 65, 45, 40, 35, 2),\n  stringsAsFactors = FALSE\n)\n\n#==============================================================================\n# Step 3: compare with arsenal::comparedf (PROC COMPARE equivalent)\n#==============================================================================\ncmp &lt;- comparedf(\n  x  = dm,\n  y  = qc_dm,\n  by = &quot;usubjid&quot;\n)\n\nsummary(cmp)<\/code><\/pre>"}] || [];
