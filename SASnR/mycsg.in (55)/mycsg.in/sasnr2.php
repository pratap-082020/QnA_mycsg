...
<title>mycsg SASnR2 4015_import_excel_workbooks_xlsx</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Import data from Excel workbooks - XLSX</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Use PROC IMPORT in SAS or readxl in R to read .xlsx files.</li>
<li>Specify the sheet name when needed.</li>
<li>Update file paths to your local machine.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">proc import datafile=&quot;path\\mf_data.xlsx&quot;
    out=males
    dbms=xlsx
    replace;
    sheet=&quot;Males&quot;;
run;</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>DBMS=XLSX reads the Excel workbook.</li>
<li>SHEET selects the worksheet to import.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">library(readxl)

males &lt;- read_excel(&quot;path/mf_data.xlsx&quot;, sheet = &quot;Males&quot;)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To read a <code>.xlsx</code> workbook into R, we use <code>readxl::read_excel()</code>.</li>
<li>We pass the file path as the first argument and name the sheet with <code>sheet =</code>.</li>
<li>The result is a tibble with one column per Excel column, with inferred types and automatic header detection.</li>
<li>readxl handles both .xlsx and .xls with the same function &mdash; no need for separate calls per format.</li>
<li>This lesson has no separate tidyverse variant because <code>read_excel()</code> is the idiomatic way in both base and tidyverse workflows &mdash; readxl is a separate package, not part of the core tidyverse.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"proc import datafile=\"path\\\\mf_data.xlsx\"\n    out=males\n    dbms=xlsx\n    replace;\n    sheet=\"Males\";\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">proc import datafile=&quot;path\\\\mf_data.xlsx&quot;\n    out=males\n    dbms=xlsx\n    replace;\n    sheet=&quot;Males&quot;;\nrun;<\/code><\/pre>"},{"label":"R (base)","code":"library(readxl)\n\nmales <- read_excel(\"path\/mf_data.xlsx\", sheet = \"Males\")","language":"r","html":"<pre><code class=\"language-r\">library(readxl)\n\nmales &lt;- read_excel(&quot;path\/mf_data.xlsx&quot;, sheet = &quot;Males&quot;)<\/code><\/pre>"}] || [];
