...
<title>mycsg SASnR2 4020_import_excel_workbooks_xls</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Import data from Excel workbooks - XLS</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Use PROC IMPORT in SAS or readxl in R to read .xls files.</li>
<li>Specify the sheet name when needed.</li>
<li>Update file paths to your local machine.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">proc import datafile=&quot;path\\mf_data.xls&quot;
    out=males
    dbms=excel
    replace;
    sheet=&quot;Males&quot;;
run;</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>DBMS=EXCEL reads the legacy .xls format.</li>
...
males &lt;- read_excel(&quot;path/mf_data.xls&quot;, sheet = &quot;Males&quot;)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To read an older <code>.xls</code> workbook, we use the same <code>readxl::read_excel()</code> we use for .xlsx files.</li>
<li>readxl detects the format automatically from the extension and dispatches to the right internal reader &mdash; we do not have to switch functions between .xls and .xlsx.</li>
<li>The <code>sheet =</code> argument accepts either a sheet name (a string) or a sheet number (1-indexed).</li>
<li>This lesson has no separate tidyverse variant because <code>read_excel()</code> is the same call in every R workflow &mdash; readxl sits alongside tidyverse, not inside it.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"proc import datafile=\"path\\\\mf_data.xls\"\n    out=males\n    dbms=excel\n    replace;\n    sheet=\"Males\";\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">proc import datafile=&quot;path\\\\mf_data.xls&quot;\n    out=males\n    dbms=excel\n    replace;\n    sheet=&quot;Males&quot;;\nrun;<\/code><\/pre>"},{"label":"R (base)","code":"library(readxl)\n\nmales <- read_excel(\"path\/mf_data.xls\", sheet = \"Males\")","language":"r","html":"<pre><code class=\"language-r\">library(readxl)\n\nmales &lt;- read_excel(&quot;path\/mf_data.xls&quot;, sheet = &quot;Males&quot;)<\/code><\/pre>"}] || [];
