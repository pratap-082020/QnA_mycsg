...
<title>mycsg SASnR2 5103_mimicking_macro_variable_creation_and_usage_list_of_values</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Create a macro variable to store list of values and use it in open code</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Many programs need to filter by a list of values rather than a single one &mdash; an IN-list of ages, a set of visit numbers, a handful of preferred terms.</li>
<li>The pattern extends the single-value case: define the list once, reference it in the filter clause.</li>
<li>SAS stores the list as a macro variable with <code>%let</code>, using <code>%str()</code> to protect commas from the macro processor.</li>
<li>R stores the list as an ordinary vector (<code>c(59, 72)</code>) and uses the <code>%in%</code> operator to test membership in either tidyverse or base dialects.</li>
<li>No macro quoting is needed in R because there is no macro processor to route characters through &mdash; a vector is just a vector.</li>
<li>This lesson filters an ADSL sample to rows whose age is 59 or 72, showing the same pattern in each language.</li>
...
<li>To store a list of values in a SAS macro variable, we use <code>%let</code> with <code>%str()</code> to protect the commas from being interpreted as parameters.</li>
<li><code>%let age_filter = %str(59,72);</code> creates a macro variable whose resolved value is the literal text "59,72".</li>
<li>We need <code>%str()</code> because bare commas in macro parameters have meaning to the macro processor &mdash; wrapping them in <code>%str()</code> tells SAS to treat them as ordinary characters.</li>
<li>In the DATA step, <code>if age in (&amp;age_filter.);</code> resolves to <code>if age in (59,72);</code>.</li>
<li>Note we are not using quotes here because <code>age</code> is numeric &mdash; for a character list we would need to build "A","B" and use <code>%bquote</code> or careful quoting.</li>
<li>This pattern is the SAS idiom for a parameterised IN-list: change the macro, rerun the step.</li>
<li>After running, we check <code>adsl_filtered</code> and confirm only the rows with age 59 or 72 appear.</li>
...
adsl_filtered &lt;- adsl %&gt;% 
  filter(age %in% age_filter)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In R, a list of values is a plain vector &mdash; no macro quoting needed.</li>
<li>We build the filter values with <code>age_filter &lt;- c(59, 72)</code>. For character values, we would use <code>c("A", "B")</code> with regular quotes.</li>
<li>Inside <code>filter()</code>, we use the <code>%in%</code> operator to test group membership: <code>filter(age %in% age_filter)</code>.</li>
<li><code>%in%</code> returns <code>TRUE</code> for each row whose <code>age</code> appears anywhere in <code>age_filter</code>, <code>FALSE</code> otherwise.</li>
<li>No special quoting is needed because R does not have a macro processor to route characters through &mdash; a vector is just a vector.</li>
<li>Scaling up is one line: to filter by three ages, we extend the vector to <code>c(59, 72, 85)</code> and the filter clause stays the same.</li>
<li>After running, we confirm <code>adsl_filtered</code> contains only rows with the listed ages.</li>
...
adsl_filtered &lt;- adsl[adsl$age %in% age_filter, ]</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, a list of filter values is just a vector &mdash; build it with <code>c()</code>, no macro quoting required.</li>
<li><code>age_filter &lt;- c(59, 72)</code> defines the list; for character values we would write <code>c("A", "B")</code>.</li>
<li><code>adsl_filtered &lt;- adsl[adsl$age %in% age_filter, ]</code> uses logical indexing with the <code>%in%</code> operator.</li>
<li><code>%in%</code> returns TRUE for each row whose value appears anywhere in the vector and FALSE otherwise.</li>
<li>The pattern scales straight up: to add a third age, extend the vector to <code>c(59, 72, 85)</code> and the filter expression stays the same.</li>
<li>We run and confirm <code>adsl_filtered</code> has only the rows matching the listed ages, lining up with the SAS and tidyverse outputs.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"*==============================================================================;\n*Macro variable to store a list of values;\n*==============================================================================;\n\ndata adsl;\n    input usubjid $ trtp $ age sex $;\n    datalines;\nSUBJ001 Placebo 67 M\nSUBJ002 DrugA   59 F\nSUBJ003 Placebo 72 F\nSUBJ004 DrugB   65 M\nSUBJ005 Placebo 60 F\n;\nrun;\n\n%let age_filter = %str(59,72);\n\ndata adsl_filtered;\n    set adsl;\n    if age in (&age_filter.);\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">*==============================================================================;\n*Macro variable to store a list of values;\n*==============================================================================;\n\ndata adsl;\n    input usubjid $ trtp $ age sex $;\n    datalines;\nSUBJ001 Placebo 67 M\nSUBJ002 DrugA   59 F\nSUBJ003 Placebo 72 F\nSUBJ004 DrugB   65 M\nSUBJ005 Placebo 60 F\n;\nrun;\n\n%let age_filter = %str(59,72);\n\ndata adsl_filtered;\n    set adsl;\n    if age in (&amp;age_filter.);\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"#==============================================================================;\r\n#List of values in macro variable;\r\n#==============================================================================;\r\nlibrary (tidyverse)\r\n\r\nadsl <- tribble(\r\n  ~usubjid, ~trt01p,     ~age, ~sex,\r\n  \"SUBJ001\", \"Placebo\", 67,   \"M\",\r\n  \"SUBJ002\", \"DrugA\",    59,   \"F\",\r\n  \"SUBJ003\", \"Placebo\", 72,   \"F\",\r\n  \"SUBJ004\", \"DrugB\",    65,   \"M\",\r\n  \"SUBJ005\", \"Placebo\", 60,   \"F\"\r\n)\r\n\r\nage_filter <- c(59,72)\r\n\r\nadsl_filtered <- adsl %>% \r\n  filter(age %in% age_filter)","language":"r","html":"<pre><code class=\"language-r\">#==============================================================================;\r\n#List of values in macro variable;\r\n#==============================================================================;\r\nlibrary (tidyverse)\r\n\r\nadsl &lt;- tribble(\r\n  ~usubjid, ~trt01p,     ~age, ~sex,\r\n  &quot;SUBJ001&quot;, &quot;Placebo&quot;, 67,   &quot;M&quot;,\r\n  &quot;SUBJ002&quot;, &quot;DrugA&quot;,    59,   &quot;F&quot;,\r\n  &quot;SUBJ003&quot;, &quot;Placebo&quot;, 72,   &quot;F&quot;,\r\n  &quot;SUBJ004&quot;, &quot;DrugB&quot;,    65,   &quot;M&quot;,\r\n  &quot;SUBJ005&quot;, &quot;Placebo&quot;, 60,   &quot;F&quot;\r\n)\r\n\r\nage_filter &lt;- c(59,72)\r\n\r\nadsl_filtered &lt;- adsl %&gt;% \r\n  filter(age %in% age_filter)<\/code><\/pre>"},{"label":"R (base)","code":"adsl <- data.frame(\r\n  usubjid = c(\"SUBJ001\", \"SUBJ002\", \"SUBJ003\", \"SUBJ004\", \"SUBJ005\"),\r\n  trt01p = c(\"Placebo\", \"DrugA\", \"Placebo\", \"DrugB\", \"Placebo\"),\r\n  age = c(67, 59, 72, 65, 60),\r\n  sex = c(\"M\", \"F\", \"F\", \"M\", \"F\")\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nage_filter <- c(59, 72)\r\n\r\nadsl_filtered <- adsl[adsl$age %in% age_filter, ]","language":"r","html":"<pre><code class=\"language-r\">adsl &lt;- data.frame(\r\n  usubjid = c(&quot;SUBJ001&quot;, &quot;SUBJ002&quot;, &quot;SUBJ003&quot;, &quot;SUBJ004&quot;, &quot;SUBJ005&quot;),\r\n  trt01p = c(&quot;Placebo&quot;, &quot;DrugA&quot;, &quot;Placebo&quot;, &quot;DrugB&quot;, &quot;Placebo&quot;),\r\n  age = c(67, 59, 72, 65, 60),\r\n  sex = c(&quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;F&quot;)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nage_filter &lt;- c(59, 72)\r\n\r\nadsl_filtered &lt;- adsl[adsl$age %in% age_filter, ]<\/code><\/pre>"}] || [];
