...
<title>mycsg SASnR2 8110_list_files_with_metadata</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>List Files with Metadata: Size, Created and Modified Time</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>The previous lesson (8100) listed files by name; this one adds the metadata columns we usually want alongside the name: size, creation time, modification time.</li>
<li>Typical uses: spot stale files that have not been updated in weeks, flag unexpectedly large outputs, audit the folder for completeness.</li>
<li>SAS needs a nested open &mdash; <code>fopen</code> on each file produced by <code>dopen</code>/<code>dread</code> &mdash; and reads metadata with <code>fsize()</code> and <code>finfo()</code>.</li>
<li>Tidyverse uses the <code>fs</code> package: <code>dir_ls()</code> for the path list and <code>file_info()</code> for the metadata in one tidy call.</li>
<li>Base R uses <code>list.files()</code> + <code>file.info()</code>; no extra package required.</li>
<li>This lesson produces a file-list data frame with size, modified time, and created time per file.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">filename mydir &quot;C:\Users\curio\Desktop\Rough&quot;;
...
<li>The SAS metadata version builds on the 8100 pattern and adds a nested <code>fopen()</code>/<code>fclose()</code> step per file.</li>
<li>We loop the directory with <code>dopen</code> / <code>dnum</code> / <code>dread</code> as before, storing the full path in <code>full_filename</code>.</li>
<li>For each file, <code>filename("fref", full_filename)</code> assigns a filename reference, and <code>fopen("fref")</code> opens the file handle for metadata queries.</li>
<li><code>fsize(fid)</code> returns the size in bytes; we wrap it in <code>put(..., best.)</code> plus <code>strip()</code> for a clean character string.</li>
<li><code>finfo(fid, "Last Modified")</code> and <code>finfo(fid, "Create Time")</code> pull the timestamps; the attribute names are operating-system dependent.</li>
<li><code>fclose(fid)</code> closes the file handle before we move to the next entry; <code>dclose(did)</code> closes the directory handle at the end.</li>
<li>The basename/extension split works exactly as in lesson 8100; the <code>drop</code> list removes the helper IDs and return codes.</li>
<li>We run and check FILE_LIST has file size plus creation and modified timestamps per row.</li>
...
print(file_list)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>The <code>fs</code> package turns this into a compact call by combining path listing and metadata retrieval.</li>
<li><code>dir_ls(path = folder_path, type = "file")</code> returns the full paths of every file in the folder &mdash; skipping sub-directories.</li>
<li><code>path_file(file_paths)</code> extracts just the file name portion from each path, filling the <code>filename</code> column.</li>
<li><code>tools::file_path_sans_ext()</code> and <code>tools::file_ext()</code> split basename from extension, just as in lesson 8100.</li>
<li><code>file_info(file_paths)</code> returns a tibble with size, modification time, change time, and permission bits for each file &mdash; we pull out the columns we need with <code>$size</code>, <code>$modification_time</code>, and <code>$change_time</code>.</li>
<li>Every piece goes into one <code>tibble()</code> call, giving a single data frame with name, folder, basename, extension, and metadata.</li>
<li>We run and verify each row shows the file name with its size and timestamps lined up correctly.</li>
...
print(file_list)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, the equivalent uses <code>list.files()</code> plus <code>file.info()</code> &mdash; both ship with the base distribution.</li>
<li><code>list.files(path = folder_path, full.names = TRUE)</code> gives the vector of full paths; storing it in <code>file_paths</code> lets us reuse it for both the name list and the metadata call.</li>
<li><code>file.info(file_paths)</code> returns a data frame with columns <code>size</code>, <code>mtime</code> (modification time), <code>ctime</code> (status change time), and a few permission fields.</li>
<li><code>basename(file_paths)</code> strips the directory portion of each path, leaving only the file name.</li>
<li>We combine all pieces in a single <code>data.frame()</code>, with <code>stringsAsFactors = FALSE</code> to keep character columns as strings.</li>
<li><code>tools::file_path_sans_ext()</code> and <code>tools::file_ext()</code> split basename from extension; they are happy with either full paths or just file names.</li>
<li>We run and inspect FILE_LIST &mdash; the shape should match the tidyverse version and the SAS FILE_LIST row for row.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"filename mydir \"C:\\Users\\curio\\Desktop\\Rough\";\n\ndata file_list;\n  length full_filename filename folder basename extension $256;\n  length file_size file_created file_modified $64;\n\n  folder = pathname(\"mydir\");\n  did = dopen(\"mydir\");\n\n  if did > 0 then do;\n    nfiles = dnum(did);\n    do i = 1 to nfiles;\n      filename = dread(did, i);\n      full_filename = cats(folder, \"\\\", filename);\n\n      rc1 = filename(\"fref\", full_filename);\n      if rc1 = 0 then do;\n        fid = fopen(\"fref\");\n        if fid > 0 then do;\n          file_size = strip(put(fsize(fid), best.));\n          file_modified = finfo(fid, \"Last Modified\");\n          file_created  = finfo(fid, \"Create Time\");\n          rc2 = fclose(fid);\n        end;\n      end;\n\n      if index(filename, \".\") then do;\n        extension = scan(filename, -1, \".\");\n        basename = substr(filename, 1, length(filename) - length(extension) - 1);\n      end;\n      else do;\n        extension = \"\";\n        basename = filename;\n      end;\n\n      output;\n    end;\n    rc = dclose(did);\n  end;\n\n  drop did nfiles i rc rc1 rc2 fid;\nrun;\n","language":"sas","html":"<pre><code class=\"language-sas-mini\">filename mydir &quot;C:\\Users\\curio\\Desktop\\Rough&quot;;\n\ndata file_list;\n  length full_filename filename folder basename extension $256;\n  length file_size file_created file_modified $64;\n\n  folder = pathname(&quot;mydir&quot;);\n  did = dopen(&quot;mydir&quot;);\n\n  if did &gt; 0 then do;\n    nfiles = dnum(did);\n    do i = 1 to nfiles;\n      filename = dread(did, i);\n      full_filename = cats(folder, &quot;\\&quot;, filename);\n\n      rc1 = filename(&quot;fref&quot;, full_filename);\n      if rc1 = 0 then do;\n        fid = fopen(&quot;fref&quot;);\n        if fid &gt; 0 then do;\n          file_size = strip(put(fsize(fid), best.));\n          file_modified = finfo(fid, &quot;Last Modified&quot;);\n          file_created  = finfo(fid, &quot;Create Time&quot;);\n          rc2 = fclose(fid);\n        end;\n      end;\n\n      if index(filename, &quot;.&quot;) then do;\n        extension = scan(filename, -1, &quot;.&quot;);\n        basename = substr(filename, 1, length(filename) - length(extension) - 1);\n      end;\n      else do;\n        extension = &quot;&quot;;\n        basename = filename;\n      end;\n\n      output;\n    end;\n    rc = dclose(did);\n  end;\n\n  drop did nfiles i rc rc1 rc2 fid;\nrun;\n<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\nlibrary(fs)\n\nfolder_path <- \"C:\/Users\/curio\/Desktop\/Rough\"\n\nfile_paths <- dir_ls(path = folder_path, type = \"file\")\n\nfile_list <- tibble(\n  full_filename = file_paths,\n  filename = path_file(file_paths),\n  folder = folder_path,\n  basename = tools::file_path_sans_ext(filename),\n  extension = tools::file_ext(filename),\n  file_size = file_info(file_paths)$size,\n  modified_time = file_info(file_paths)$modification_time,\n  created_time = file_info(file_paths)$change_time\n)\n\nprint(file_list)","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\nlibrary(fs)\n\nfolder_path &lt;- &quot;C:\/Users\/curio\/Desktop\/Rough&quot;\n\nfile_paths &lt;- dir_ls(path = folder_path, type = &quot;file&quot;)\n\nfile_list &lt;- tibble(\n  full_filename = file_paths,\n  filename = path_file(file_paths),\n  folder = folder_path,\n  basename = tools::file_path_sans_ext(filename),\n  extension = tools::file_ext(filename),\n  file_size = file_info(file_paths)$size,\n  modified_time = file_info(file_paths)$modification_time,\n  created_time = file_info(file_paths)$change_time\n)\n\nprint(file_list)<\/code><\/pre>"},{"label":"R (base)","code":"folder_path <- \"C:\/Users\/curio\/Desktop\/Rough\"\n\nfile_paths <- list.files(path = folder_path, full.names = TRUE)\n\nfile_info <- file.info(file_paths)\n\nfile_list <- data.frame(\n  full_filename = file_paths,\n  filename = basename(file_paths),\n  folder = folder_path,\n  basename = tools::file_path_sans_ext(basename(file_paths)),\n  extension = tools::file_ext(basename(file_paths)),\n  file_size = file_info$size,\n  modified_time = file_info$mtime,\n  created_time = file_info$ctime,\n  stringsAsFactors = FALSE\n)\n\nprint(file_list)","language":"r","html":"<pre><code class=\"language-r\">folder_path &lt;- &quot;C:\/Users\/curio\/Desktop\/Rough&quot;\n\nfile_paths &lt;- list.files(path = folder_path, full.names = TRUE)\n\nfile_info &lt;- file.info(file_paths)\n\nfile_list &lt;- data.frame(\n  full_filename = file_paths,\n  filename = basename(file_paths),\n  folder = folder_path,\n  basename = tools::file_path_sans_ext(basename(file_paths)),\n  extension = tools::file_ext(basename(file_paths)),\n  file_size = file_info$size,\n  modified_time = file_info$mtime,\n  created_time = file_info$ctime,\n  stringsAsFactors = FALSE\n)\n\nprint(file_list)<\/code><\/pre>"}] || [];
