...
<title>mycsg SASnR2 9105_Detach an R package</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Detach an R package (unload from session)</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Sometimes we want to remove a package from the current R session&mdash;similar to clearing a SAS LIBNAME with <code>LIBNAME mylib CLEAR;</code></li>
<li><code>detach("package:pkg", unload = TRUE)</code> removes an attached package from the search path and unloads its namespace</li>
<li>Check the current search path using <code>search()</code>; attached packages appear as <code>"package:pkg"</code></li>
<li>Meta-packages like <code>tidyverse</code> attach several packages; detach them individually (loop through the tidyverse core packages)</li>
<li>Detaching can break objects depending on those functions; prefer a fresh R session for a clean slate during analysis pipelines</li>
...
# Detach each explicitly; there is no single &quot;detach tidyverse&quot; command.
pkgs     &lt;- c(&quot;ggplot2&quot;, &quot;tibble&quot;, &quot;tidyr&quot;, &quot;readr&quot;, &quot;purrr&quot;, &quot;dplyr&quot;, &quot;stringr&quot;, &quot;forcats&quot;)
attached &lt;- paste0(&quot;package:&quot;, pkgs)
to_drop  &lt;- intersect(attached, search())

for (p in rev(to_drop)) {
  detach(p, unload = TRUE)
}</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>This lesson has no SAS variant and no separate tidyverse variant &mdash; SAS has no direct equivalent of unloading a package, and <code>detach()</code> is pure base R.</li>
<li>To remove a package from the current R session, we call <code>detach("package:name", unload = TRUE)</code>.</li>
<li>The <code>package:</code> prefix is required &mdash; it tells R to look up the package on its search path rather than treating the argument as a data frame.</li>
<li><code>unload = TRUE</code> tears down the package namespace too, freeing memory and preventing zombie references to its functions.</li>
<li>We use <code>search()</code> to see which packages are currently attached. It returns a character vector &mdash; each element is a position on the search path.</li>
<li>Tidyverse is a meta-package, so <code>library(tidyverse)</code> actually attaches eight packages (ggplot2, tibble, tidyr, readr, purrr, dplyr, stringr, forcats). There is no single command to detach them all.</li>
<li>The loop at the bottom handles that cleanly: we build the list of <code>package:name</code> strings, keep only the ones actually on the search path with <code>intersect()</code>, then detach each in reverse attachment order.</li>
<li>We detach in reverse order to avoid dependency problems &mdash; some tidyverse packages depend on others, and unloading a dependency before its consumers is asking for trouble.</li>
<li>After running, a fresh call to <code>search()</code> should no longer list any of the tidyverse packages.</li>
