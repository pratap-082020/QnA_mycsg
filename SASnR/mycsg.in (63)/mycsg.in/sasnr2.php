...
<title>mycsg SASnR2 9120_Remove objects from environment</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Remove objects from the environment (rm / remove)</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>When experimenting, we often want to remove temporary objects from the environment to avoid name clashes and stale values</li>
<li><code>ls()</code> (or <code>objects()</code>) lists what is currently in the environment</li>
<li><code>rm(x)</code> removes a single object; <code>rm(a, b)</code> removes multiple</li>
<li>For scripts, prefer <code>rm(list = ...)</code> where the list is built programmatically</li>
<li>Common patterns</li>
<ul>
  <li>Keep only a few: <code>rm(list = setdiff(ls(), c("adsl","adlb")))</code></li>
  <li>Remove by pattern: <code>rm(list = ls(pattern = "^tmp_"))</code></li>
  <li>Clear all (use with care): <code>rm(list = ls())</code></li>
</ul>
<li>Think of this like cleaning the SAS WORK library between steps—keeps our workspace predictable while we iterate</li>
</ul>
    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-r">#==============================================================================
# List, remove and manage objects in the R workspace
#==============================================================================

#------------------------------------------------------------------------------
# List objects currently in the global environment (like SAS WORK listing)
#------------------------------------------------------------------------------
ls()            # or objects()
...
#------------------------------------------------------------------------------
# Remove multiple objects in one call
#------------------------------------------------------------------------------
a &lt;- 10
b &lt;- 20
c &lt;- 30
...
#------------------------------------------------------------------------------
# Remove objects by character vector (reproducible in scripts)
#------------------------------------------------------------------------------
to_drop &lt;- c(&quot;b&quot;, &quot;c&quot;)
rm(list = to_drop)
...
#------------------------------------------------------------------------------
keep &lt;- c(&quot;adsl&quot;, &quot;adlb&quot;)          # objects we want to keep
...
rm(list = ls())</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>This lesson has no separate tidyverse variant &mdash; <code>ls()</code>, <code>rm()</code> and friends are all base R, with no tidyverse equivalents.</li>
<li>To list objects currently in the R session, we use <code>ls()</code> (or its synonym <code>objects()</code>). This is the R analogue of browsing the SAS WORK library.</li>
<li>To remove a single object by name, we pass it to <code>rm()</code> directly &mdash; <code>rm(x)</code>.</li>
<li>To remove several at once, we can pass names as arguments (<code>rm(a, b)</code>) or as a character vector via <code>list =</code> (<code>rm(list = c("b", "c"))</code>).</li>
<li>The <code>list =</code> form is friendlier in scripts because it takes a vector we can build dynamically.</li>
<li>To keep a handful of objects and drop everything else, we combine <code>setdiff(ls(), keep)</code> with <code>rm(list = ...)</code>. Useful after a long interactive session when we only want the final datasets in memory.</li>
<li>To remove objects matching a name pattern, we use <code>ls(pattern = "regex")</code> and pass the result into <code>rm()</code>. The pattern <code>"^tmp_\\d+$"</code> matches tmp_01, tmp_02 etc. but not tmp_final.</li>
<li><code>rm(list = ls())</code> clears the entire environment &mdash; equivalent to starting a fresh SAS WORK library mid-session. Use with care.</li>
<li>After each <code>rm()</code>, we can run <code>ls()</code> again to confirm the objects are gone before moving on.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"R (base)","code":"#==============================================================================\n# List, remove and manage objects in the R workspace\n#==============================================================================\n\n#------------------------------------------------------------------------------\n# List objects currently in the global environment (like SAS WORK listing)\n#------------------------------------------------------------------------------\nls()            # or objects()\n\n#------------------------------------------------------------------------------\n# Remove a single object by name\n#------------------------------------------------------------------------------\nx <- 1\nrm(x)           # x is now removed\n\"x\" %in% ls()   # FALSE\n\n#------------------------------------------------------------------------------\n# Remove multiple objects in one call\n#------------------------------------------------------------------------------\na <- 10\nb <- 20\nc <- 30\nrm(a, b)\nls()\n\n#------------------------------------------------------------------------------\n# Remove objects by character vector (reproducible in scripts)\n#------------------------------------------------------------------------------\nto_drop <- c(\"b\", \"c\")\nrm(list = to_drop)\n\n#------------------------------------------------------------------------------\n# Keep only selected objects (drop everything else)\n#------------------------------------------------------------------------------\nkeep <- c(\"adsl\", \"adlb\")          # objects we want to keep\nrm(list = setdiff(ls(), keep))\n\n#------------------------------------------------------------------------------\n# Remove objects matching a name pattern\n#------------------------------------------------------------------------------\ntmp_01    <- 1\ntmp_02    <- 2\ntmp_final <- 3\nrm(list = ls(pattern = \"^tmp_\\\\d+$\"))  # removes tmp_01 and tmp_02, keeps tmp_final\n\n#------------------------------------------------------------------------------\n# Clear the entire environment (use with care)\n#------------------------------------------------------------------------------\n# Equivalent to starting a fresh WORK library in SAS mid-session\nrm(list = ls())","language":"r","html":"<pre><code class=\"language-r\">#==============================================================================\n# List, remove and manage objects in the R workspace\n#==============================================================================\n\n#------------------------------------------------------------------------------\n# List objects currently in the global environment (like SAS WORK listing)\n#------------------------------------------------------------------------------\nls()            # or objects()\n\n#------------------------------------------------------------------------------\n# Remove a single object by name\n#------------------------------------------------------------------------------\nx &lt;- 1\nrm(x)           # x is now removed\n&quot;x&quot; %in% ls()   # FALSE\n\n#------------------------------------------------------------------------------\n# Remove multiple objects in one call\n#------------------------------------------------------------------------------\na &lt;- 10\nb &lt;- 20\nc &lt;- 30\nrm(a, b)\nls()\n\n#------------------------------------------------------------------------------\n# Remove objects by character vector (reproducible in scripts)\n#------------------------------------------------------------------------------\nto_drop &lt;- c(&quot;b&quot;, &quot;c&quot;)\nrm(list = to_drop)\n\n#------------------------------------------------------------------------------\n# Keep only selected objects (drop everything else)\n#------------------------------------------------------------------------------\nkeep &lt;- c(&quot;adsl&quot;, &quot;adlb&quot;)          # objects we want to keep\nrm(list = setdiff(ls(), keep))\n\n#------------------------------------------------------------------------------\n# Remove objects matching a name pattern\n#------------------------------------------------------------------------------\ntmp_01    &lt;- 1\ntmp_02    &lt;- 2\ntmp_final &lt;- 3\nrm(list = ls(pattern = &quot;^tmp_\\\\d+$&quot;))  # removes tmp_01 and tmp_02, keeps tmp_final\n\n#------------------------------------------------------------------------------\n# Clear the entire environment (use with care)\n#------------------------------------------------------------------------------\n# Equivalent to starting a fresh WORK library in SAS mid-session\nrm(list = ls())<\/code><\/pre>"}] || [];
