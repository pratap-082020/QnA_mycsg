...
<title>mycsg SASnR2 9310_read-or-load-multiple-xpt-files-or-sas-datasets-simultaneously-into-r</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Read or load multiple xpt files or sas datasets simultaneously into R</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Clinical studies usually ship data as a folder of XPT files (transport files) &mdash; one file per SDTM or ADaM domain, such as DM.xpt, AE.xpt, LB.xpt and so on.</li>
<li>When we start an analysis program, we rarely need just one of them. A typical task touches DM, a couple of findings domains and one events domain together.</li>
<li>In SAS this is almost free: we point a <code>libname</code> at the folder and every XPT (or sas7bdat) file in it is immediately available as a dataset reference.</li>
<li>R does not do that for us. We have to ask it to read each file and give each one a name in the current session.</li>
<li>In this lesson we look at three patterns for loading a set of domains at once:
<ul>
<li>a SAS libref pointing at a folder of XPT files,</li>
<li>a tidyverse pipeline using <code>map()</code> and <code>list2env()</code>,</li>
<li>a plain base R loop using <code>for</code> and <code>assign()</code>.</li>
</ul>
</li>
<li>All three end up with <code>dm</code>, <code>ae</code> and <code>cm</code> sitting in memory as independent datasets, ready to be merged and analysed.</li>
...
* Assign a libref to the folder that holds our XPT files                       ;
* Using the XPORT engine so SAS reads them directly                            ;
*==============================================================================;
libname sdtmxpt xport &quot;D:\SAS\Home\dev\clinical_sas_samples\mycsg\data\shared\xpt\sdtm&quot;;
libname work    &quot;%sysfunc(getoption(work))&quot;;

*==============================================================================;
* Copy every member into WORK so we can use them as regular datasets          ;
*==============================================================================;
proc copy in = sdtmxpt out = work;
  select dm ae cm;
run;

*------------------------------------------------------------------------------;
* After PROC COPY, work.dm, work.ae and work.cm are all available            ;
*------------------------------------------------------------------------------;
proc contents data = work._all_ nods;
run;</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To read a folder of XPT files in one go, we assign a libref with the XPORT engine &mdash; <code>libname sdtmxpt xport "path/to/folder";</code>.</li>
<li>The XPORT engine tells SAS &ldquo;treat this folder as a library of transport files&rdquo;, after which every .xpt inside becomes a library member.</li>
<li>We typically copy the members we want into WORK with <code>PROC COPY</code>, so downstream DATA and PROC steps can treat them as ordinary datasets.</li>
<li>The <code>select</code> statement inside PROC COPY lets us load only the domains we actually need, rather than every file in the folder.</li>
<li>After PROC COPY finishes, <code>work.dm</code>, <code>work.ae</code> and <code>work.cm</code> all exist and can be referenced directly.</li>
<li>For sas7bdat files (not XPT), we can skip the <code>xport</code> keyword &mdash; a plain <code>libname sdtmlib "path/to/folder";</code> already exposes every .sas7bdat as a library member.</li>
<li>One gotcha: the XPORT engine exposes all members as read-only. We usually PROC COPY first, then modify in WORK, rather than editing the source library directly.</li>
<li>After running, we check the log for "NOTE: LIBREF SDTMXPT refers to..." and "NOTE: Copying SDTMXPT.DM to WORK.DM..." lines &mdash; one per domain copied.</li>
...
#==============================================================================
# Point at the folder that holds our XPT files
#==============================================================================
sdtmloc &lt;- &quot;D:/SAS/Home/dev/clinical_sas_samples/mycsg/data/shared/xpt/sdtm&quot;

#==============================================================================
# Read every domain we care about and put each one in the global environment
#==============================================================================
c(&quot;dm&quot;, &quot;ae&quot;, &quot;cm&quot;) %&gt;%
  set_names() %&gt;%
  map(~ read_xpt(glue(&quot;{sdtmloc}/{.x}.xpt&quot;))) %&gt;%
  list2env(envir = .GlobalEnv)

#------------------------------------------------------------------------------
# Verify — dm, ae, cm should now be regular tibbles in our session
#------------------------------------------------------------------------------
ls()
glimpse(dm)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To load several XPT files at once in tidyverse, we build a vector of domain names and push it through a short pipeline.</li>
<li><code>c("dm", "ae", "cm")</code> is the equivalent of a <code>select dm ae cm;</code> statement in PROC COPY &mdash; the explicit list of domains we want.</li>
<li><code>set_names()</code> makes the vector name itself, so each element carries its own domain name as a label. This is what makes <code>dm</code> end up called <code>dm</code> at the end (and not <code>[[1]]</code>).</li>
<li><code>map(~ read_xpt(glue("{sdtmloc}/{.x}.xpt")))</code> iterates over each name and reads the corresponding XPT file. <code>glue()</code> builds the full path by substituting <code>.x</code> (the current domain name) into the template.</li>
<li>At this point we have a named list: one tibble per domain, keyed by domain name.</li>
<li><code>list2env(envir = .GlobalEnv)</code> unpacks the list &mdash; each element is promoted to a top-level object in the global environment. So <code>dm</code>, <code>ae</code> and <code>cm</code> become standalone tibbles we can reference directly.</li>
<li>Scaling up is one line: to add the VS (vital signs) domain, we just extend the initial vector to <code>c("dm", "ae", "cm", "vs")</code>. Nothing else in the pipeline changes.</li>
<li>After running, we check with <code>ls()</code> that the expected objects exist and use <code>glimpse(dm)</code> to confirm the first domain loaded with its variables and row counts.</li>
...
#==============================================================================
# Loop over each domain, read the XPT, and assign it to a top-level object
#==============================================================================
for (d in domains) {
  file_path &lt;- file.path(sdtmloc, paste0(d, &quot;.xpt&quot;))
  assign(d, haven::read_xpt(file_path), envir = .GlobalEnv)
}

#------------------------------------------------------------------------------
# Verify — dm, ae, cm should now exist as standalone data frames
#------------------------------------------------------------------------------
ls()
str(dm)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>For a base R equivalent without tidyverse, we drive the same task with a plain <code>for</code> loop.</li>
<li>We keep the list of domains in a character vector <code>domains</code>, exactly like the tidyverse version.</li>
<li>Inside the loop we build the path with <code>file.path()</code> and <code>paste0()</code> &mdash; the base R alternative to <code>glue()</code>. Same output, no external dependency beyond haven.</li>
<li><code>haven::read_xpt()</code> reads the XPT file and returns a data frame. We keep the <code>haven::</code> prefix so we do not have to <code>library(haven)</code> at the top &mdash; a common pattern when we want to keep the script self-contained.</li>
<li><code>assign(d, value, envir = .GlobalEnv)</code> is the base R equivalent of <code>list2env()</code>. It says &ldquo;create an object named <code>d</code> in the global environment and give it this value.&rdquo;</li>
<li>Scaling works the same way: we add more names to <code>domains</code> and the loop handles them automatically.</li>
<li>Trade-off: the loop uses three variables (<code>d</code>, <code>file_path</code>, the loop counter) and is more imperative, but it reads easily for anyone with a basic R background and carries zero tidyverse dependencies.</li>
<li>After running, we run <code>ls()</code> to confirm the objects exist, and <code>str(dm)</code> to check the first domain loaded as expected.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"*==============================================================================;\n* Assign a libref to the folder that holds our XPT files                       ;\n* Using the XPORT engine so SAS reads them directly                            ;\n*==============================================================================;\nlibname sdtmxpt xport \"D:\\SAS\\Home\\dev\\clinical_sas_samples\\mycsg\\data\\shared\\xpt\\sdtm\";\nlibname work    \"%sysfunc(getoption(work))\";\n\n*==============================================================================;\n* Copy every member into WORK so we can use them as regular datasets          ;\n*==============================================================================;\nproc copy in = sdtmxpt out = work;\n  select dm ae cm;\nrun;\n\n*------------------------------------------------------------------------------;\n* After PROC COPY, work.dm, work.ae and work.cm are all available            ;\n*------------------------------------------------------------------------------;\nproc contents data = work._all_ nods;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">*==============================================================================;\n* Assign a libref to the folder that holds our XPT files                       ;\n* Using the XPORT engine so SAS reads them directly                            ;\n*==============================================================================;\nlibname sdtmxpt xport &quot;D:\\SAS\\Home\\dev\\clinical_sas_samples\\mycsg\\data\\shared\\xpt\\sdtm&quot;;\nlibname work    &quot;%sysfunc(getoption(work))&quot;;\n\n*==============================================================================;\n* Copy every member into WORK so we can use them as regular datasets          ;\n*==============================================================================;\nproc copy in = sdtmxpt out = work;\n  select dm ae cm;\nrun;\n\n*------------------------------------------------------------------------------;\n* After PROC COPY, work.dm, work.ae and work.cm are all available            ;\n*------------------------------------------------------------------------------;\nproc contents data = work._all_ nods;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\nlibrary(haven)\nlibrary(glue)\n\n#==============================================================================\n# Point at the folder that holds our XPT files\n#==============================================================================\nsdtmloc <- \"D:\/SAS\/Home\/dev\/clinical_sas_samples\/mycsg\/data\/shared\/xpt\/sdtm\"\n\n#==============================================================================\n# Read every domain we care about and put each one in the global environment\n#==============================================================================\nc(\"dm\", \"ae\", \"cm\") %>%\n  set_names() %>%\n  map(~ read_xpt(glue(\"{sdtmloc}\/{.x}.xpt\"))) %>%\n  list2env(envir = .GlobalEnv)\n\n#------------------------------------------------------------------------------\n# Verify \u2014 dm, ae, cm should now be regular tibbles in our session\n#------------------------------------------------------------------------------\nls()\nglimpse(dm)","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\nlibrary(haven)\nlibrary(glue)\n\n#==============================================================================\n# Point at the folder that holds our XPT files\n#==============================================================================\nsdtmloc &lt;- &quot;D:\/SAS\/Home\/dev\/clinical_sas_samples\/mycsg\/data\/shared\/xpt\/sdtm&quot;\n\n#==============================================================================\n# Read every domain we care about and put each one in the global environment\n#==============================================================================\nc(&quot;dm&quot;, &quot;ae&quot;, &quot;cm&quot;) %&gt;%\n  set_names() %&gt;%\n  map(~ read_xpt(glue(&quot;{sdtmloc}\/{.x}.xpt&quot;))) %&gt;%\n  list2env(envir = .GlobalEnv)\n\n#------------------------------------------------------------------------------\n# Verify \u2014 dm, ae, cm should now be regular tibbles in our session\n#------------------------------------------------------------------------------\nls()\nglimpse(dm)<\/code><\/pre>"},{"label":"R (base)","code":"#==============================================================================\n# Point at the folder that holds our XPT files\n#==============================================================================\nsdtmloc <- \"D:\/SAS\/Home\/dev\/clinical_sas_samples\/mycsg\/data\/shared\/xpt\/sdtm\"\n\ndomains <- c(\"dm\", \"ae\", \"cm\")\n\n#==============================================================================\n# Loop over each domain, read the XPT, and assign it to a top-level object\n#==============================================================================\nfor (d in domains) {\n  file_path <- file.path(sdtmloc, paste0(d, \".xpt\"))\n  assign(d, haven::read_xpt(file_path), envir = .GlobalEnv)\n}\n\n#------------------------------------------------------------------------------\n# Verify \u2014 dm, ae, cm should now exist as standalone data frames\n#------------------------------------------------------------------------------\nls()\nstr(dm)","language":"r","html":"<pre><code class=\"language-r\">#==============================================================================\n# Point at the folder that holds our XPT files\n#==============================================================================\nsdtmloc &lt;- &quot;D:\/SAS\/Home\/dev\/clinical_sas_samples\/mycsg\/data\/shared\/xpt\/sdtm&quot;\n\ndomains &lt;- c(&quot;dm&quot;, &quot;ae&quot;, &quot;cm&quot;)\n\n#==============================================================================\n# Loop over each domain, read the XPT, and assign it to a top-level object\n#==============================================================================\nfor (d in domains) {\n  file_path &lt;- file.path(sdtmloc, paste0(d, &quot;.xpt&quot;))\n  assign(d, haven::read_xpt(file_path), envir = .GlobalEnv)\n}\n\n#------------------------------------------------------------------------------\n# Verify \u2014 dm, ae, cm should now exist as standalone data frames\n#------------------------------------------------------------------------------\nls()\nstr(dm)<\/code><\/pre>"}] || [];
