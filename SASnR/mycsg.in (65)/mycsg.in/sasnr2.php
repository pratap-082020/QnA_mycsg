...
<title>mycsg SASnR2 9550_run-multiple-programs-for-a-single-file</title>
...
<li class="nav-item dropdown d-flex align-items-center">
  ...
  <div class="dropdown-menu dropdown-menu-end dropdown-menu-right p-2"
       ...
       style="min-width: 320px; max-width: min(92vw, 420px); overflow-wrap:anywhere;">
        ...
        <div class="px-1 text-muted" style="font-size:.75rem;">As of 17 May 2026, 23:54 IST</div>
      </div>
    </li>  
      <li class="nav-item">
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Run multiple programs from a single file</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>In clinical programming, we routinely build a deliverable by running several programs in sequence &mdash; for example, one program per SDTM domain, then one per ADaM dataset, then one TFL per table.</li>
<li>In SAS, a shared project team usually handles this through the institutional batch submission tool (SAS Grid, a shell wrapper, or a drop-folder that a scheduler watches). There is no single portable SAS file that reliably submits every other SAS file for us, which is why this lesson has no SAS variant &mdash; the equivalent step lives outside SAS itself.</li>
<li>In R, we can drive the whole batch from a single R file that calls each individual program in its own fresh R session. This gives us predictable isolation, per-program log files and an explicit execution order &mdash; all the things a SAS batch setup usually provides.</li>
<li>In this lesson we set up a tiny worked example:
<ul>
<li>three small R programs that each create one XPT file (DM, AE, LB),</li>
<li>one driver file <code>run_all.R</code> that runs the three programs in order and writes one log file per program.</li>
</ul>
</li>
<li>Folder layout we assume:
<ul>
<li><code>r_batch_xpt_example/</code>
<ul>
<li><code>run_all.R</code> (the batch driver, at the top level)</li>
<li><code>programs/</code>
<ul>
<li><code>01_make_dm_xpt.R</code></li>
<li><code>02_make_ae_xpt.R</code></li>
<li><code>03_make_lb_xpt.R</code></li>
</ul>
</li>
<li><code>output/xpt/</code> (created automatically when we run)</li>
<li><code>output/logs/</code> (created automatically when we run)</li>
</ul>
</li>
</ul>
</li>
<li>Each individual R program should be fully self-contained &mdash; it should read or build its own data, process it and write its output without depending on objects left behind by another program.</li>
<li>To kick off a batch run, we open a terminal in the project folder and execute <code>Rscript run_all.R</code>. The driver runs each program in a clean R session, captures its output to a per-program log file, and continues to the next one.</li>
<li>After the run, we expect to find <code>DM.xpt</code>, <code>AE.xpt</code> and <code>LB.xpt</code> under <code>output/xpt/</code>, plus one .log file per program under <code>output/logs/</code>.</li>
<li>Adding a new program is a two-step change: drop a new file like <code>04_make_ex_xpt.R</code> into the <code>programs/</code> folder, then add its path to the <code>scripts</code> vector in <code>run_all.R</code>.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-r-tidyverse">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-tidyverse" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (tidyverse)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-r">#==============================================================================
# programs/01_make_dm_xpt.R — build and write DM.xpt
#==============================================================================
library(tidyverse)
library(haven)
...
dm &lt;- tribble(
  ...
  mutate(across(where(is.character), ~ str_sub(.x, 1, 200)))

write_xpt(dm, file.path(out_dir, &quot;DM.xpt&quot;))


#==============================================================================
# programs/02_make_ae_xpt.R — build and write AE.xpt
#==============================================================================
library(tidyverse)
library(haven)
...
write_xpt(ae, file.path(out_dir, &quot;AE.xpt&quot;))


#==============================================================================
# programs/03_make_lb_xpt.R — build and write LB.xpt
#==============================================================================
library(tidyverse)
library(haven)
...
write_xpt(lb, file.path(out_dir, &quot;LB.xpt&quot;))


#==============================================================================
# run_all.R — batch driver (sits at the project root)
#==============================================================================
library(tidyverse)

setwd(&quot;C:/path/to/r_batch_xpt_example&quot;)
...
<li>To run several R programs from a single driver file in tidyverse style, we list the programs in order and iterate with <code>walk()</code>.</li>
<li><code>scripts</code> is an ordered character vector &mdash; the same idea as listing SAS programs in a batch submit file. The execution order is the vector order.</li>
<li>Inside each individual program, we include <code>library(tidyverse)</code> and <code>library(haven)</code>. That is critical for batch mode: <code>Rscript</code> does not auto-load any packages, so each program has to be self-sufficient.</li>
<li>We use <code>across(where(is.character), ~ str_sub(.x, 1, 200))</code> to cap every character column at 200 bytes. This matches XPT v5 limits and keeps the write step safe for SDTM-style data.</li>
<li>In the driver, <code>walk()</code> is the <code>purrr</code> equivalent of a for loop &mdash; we iterate over <code>scripts</code> and execute a block for each one. Using <code>walk()</code> instead of <code>map()</code> signals &ldquo;we care about the side effect, not a return value&rdquo;.</li>
<li>Inside the walk body, we build a log file name by stripping the directory and extension from the script name, and redirect <code>stdout</code> and <code>stderr</code> into it with <code>system2()</code>.</li>
<li><code>system2(command = "Rscript", args = s, ...)</code> launches a brand-new R session for each program. That gives us clean isolation &mdash; a stray object or loaded package in one program cannot leak into the next.</li>
<li>After the loop finishes, we print &ldquo;Batch run completed&rdquo; so the console shows a clear end-of-batch marker.</li>
<li>After running <code>Rscript run_all.R</code>, we check that <code>output/xpt/</code> contains DM.xpt, AE.xpt and LB.xpt, and <code>output/logs/</code> contains one .log file per program.</li>
<li>If any program fails, we open its specific log file, fix the issue inside that program, and rerun <code>Rscript run_all.R</code> &mdash; the driver is safe to re-run end-to-end.</li>
</ul></div>    </div>  </div></div><div class="card mb-2 code-card lang-r-base">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-r-base" data-toggle="collapse" data-target="#collapse1" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse1">    <h6 class="mb-0"><span class="badge badge-success mr-2">R</span> (base)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse1">Copy</button>      <span class="toggle-indicator" data-target="collapse1">+</span>    </div>  </div>  <div id="collapse1" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse1"><pre><code class="language-r">#==============================================================================
# run_all.R — batch driver (base R version, no tidyverse in the driver)
#==============================================================================
...
cat(&quot;Batch run completed\n&quot;)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>For a base R driver with no tidyverse dependency, we swap <code>walk()</code> for a plain <code>for</code> loop &mdash; the rest of the structure is identical.</li>
<li><code>system2()</code>, <code>file.path()</code>, <code>dir.create()</code> and <code>tools::file_path_sans_ext()</code> are all base R, so this driver runs on a fresh R install with zero extra packages.</li>
<li>The individual programs in <code>programs/</code> can still use tidyverse internally &mdash; the driver does not care. Only the driver itself is tidyverse-free.</li>
<li>This base R version is useful when the automation server runs a locked-down or validated R environment where we can rely on only base packages in the scheduler-side code.</li>
<li>Everything else we confirm after a run (output XPT files, log files, re-runnable on failure) works the same way as the tidyverse version.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"R (tidyverse)","code":"#==============================================================================\n# programs\/01_make_dm_xpt.R \u2014 build and write DM.xpt\n#==============================================================================\nlibrary(tidyverse)\nlibrary(haven)\n\nout_dir <- file.path(\"output\", \"xpt\")\ndir.create(out_dir, recursive = TRUE, showWarnings = FALSE)\n\ndm <- tribble(\n  ~studyid, ~domain, ~usubjid,     ~subjid, ~sex, ~age,\n  \"CSG001\", \"DM\",    \"CSG001-0001\", \"0001\", \"M\",   35,\n  \"CSG001\", \"DM\",    \"CSG001-0002\", \"0002\", \"F\",   29\n) %>%\n  mutate(across(where(is.character), ~ str_sub(.x, 1, 200)))\n\nwrite_xpt(dm, file.path(out_dir, \"DM.xpt\"))\n\n\n#==============================================================================\n# programs\/02_make_ae_xpt.R \u2014 build and write AE.xpt\n#==============================================================================\nlibrary(tidyverse)\nlibrary(haven)\n\nout_dir <- file.path(\"output\", \"xpt\")\ndir.create(out_dir, recursive = TRUE, showWarnings = FALSE)\n\nae <- tribble(\n  ~studyid, ~domain, ~usubjid,     ~aeseq, ~aeterm,    ~aeser,\n  \"CSG001\", \"AE\",    \"CSG001-0001\",     1, \"Headache\", \"N\",\n  \"CSG001\", \"AE\",    \"CSG001-0002\",     1, \"Nausea\",   \"N\"\n) %>%\n  mutate(across(where(is.character), ~ str_sub(.x, 1, 200)))\n\nwrite_xpt(ae, file.path(out_dir, \"AE.xpt\"))\n\n\n#==============================================================================\n# programs\/03_make_lb_xpt.R \u2014 build and write LB.xpt\n#==============================================================================\nlibrary(tidyverse)\nlibrary(haven)\n\nout_dir <- file.path(\"output\", \"xpt\")\ndir.create(out_dir, recursive = TRUE, showWarnings = FALSE)\n\nlb <- tribble(\n  ~studyid, ~domain, ~usubjid,     ~lbtestcd, ~lbtest,      ~lbstresn, ~lbstresu,\n  \"CSG001\", \"LB\",    \"CSG001-0001\", \"HGB\",    \"Hemoglobin\", 13.2,      \"g\/dL\"\n) %>%\n  mutate(across(where(is.character), ~ str_sub(.x, 1, 200)))\n\nwrite_xpt(lb, file.path(out_dir, \"LB.xpt\"))\n\n\n#==============================================================================\n# run_all.R \u2014 batch driver (sits at the project root)\n#==============================================================================\nlibrary(tidyverse)\n\nsetwd(\"C:\/path\/to\/r_batch_xpt_example\")\n\nscripts <- c(\n  file.path(\"programs\", \"01_make_dm_xpt.R\"),\n  file.path(\"programs\", \"02_make_ae_xpt.R\"),\n  file.path(\"programs\", \"03_make_lb_xpt.R\")\n)\n\nlog_dir <- file.path(\"output\", \"logs\")\ndir.create(log_dir, recursive = TRUE, showWarnings = FALSE)\n\nwalk(scripts, function(s) {\n  log_file <- file.path(\n    log_dir,\n    paste0(tools::file_path_sans_ext(basename(s)), \".log\")\n  )\n  system2(\n    command = \"Rscript\",\n    args    = s,\n    stdout  = log_file,\n    stderr  = log_file\n  )\n})\n\ncat(\"Batch run completed\\n\")","language":"r","html":"<pre><code class=\"language-r\">#==============================================================================\n# programs\/01_make_dm_xpt.R \u2014 build and write DM.xpt\n#==============================================================================\nlibrary(tidyverse)\nlibrary(haven)\n\nout_dir &lt;- file.path(&quot;output&quot;, &quot;xpt&quot;)\ndir.create(out_dir, recursive = TRUE, showWarnings = FALSE)\n\ndm &lt;- tribble(\n  ~studyid, ~domain, ~usubjid,     ~subjid, ~sex, ~age,\n  &quot;CSG001&quot;, &quot;DM&quot;,    &quot;CSG001-0001&quot;, &quot;0001&quot;, &quot;M&quot;,   35,\n  &quot;CSG001&quot;, &quot;DM&quot;,    &quot;CSG001-0002&quot;, &quot;0002&quot;, &quot;F&quot;,   29\n) %&gt;%\n  mutate(across(where(is.character), ~ str_sub(.x, 1, 200)))\n\nwrite_xpt(dm, file.path(out_dir, &quot;DM.xpt&quot;))\n\n\n#==============================================================================\n# programs\/02_make_ae_xpt.R \u2014 build and write AE.xpt\n#==============================================================================\nlibrary(tidyverse)\nlibrary(haven)\n\nout_dir &lt;- file.path(&quot;output&quot;, &quot;xpt&quot;)\ndir.create(out_dir, recursive = TRUE, showWarnings = FALSE)\n\nae &lt;- tribble(\n  ~studyid, ~domain, ~usubjid,     ~aeseq, ~aeterm,    ~aeser,\n  &quot;CSG001&quot;, &quot;AE&quot;,    &quot;CSG001-0001&quot;,     1, &quot;Headache&quot;, &quot;N&quot;,\n  &quot;CSG001&quot;, &quot;AE&quot;,    &quot;CSG001-0002&quot;,     1, &quot;Nausea&quot;,   &quot;N&quot;\n) %&gt;%\n  mutate(across(where(is.character), ~ str_sub(.x, 1, 200)))\n\nwrite_xpt(ae, file.path(out_dir, &quot;AE.xpt&quot;))\n\n\n#==============================================================================\n# programs\/03_make_lb_xpt.R \u2014 build and write LB.xpt\n#==============================================================================\nlibrary(tidyverse)\nlibrary(haven)\n\nout_dir &lt;- file.path(&quot;output&quot;, &quot;xpt&quot;)\ndir.create(out_dir, recursive = TRUE, showWarnings = FALSE)\n\nlb &lt;- tribble(\n  ~studyid, ~domain, ~usubjid,     ~lbtestcd, ~lbtest,      ~lbstresn, ~lbstresu,\n  &quot;CSG001&quot;, &quot;LB&quot;,    &quot;CSG001-0001&quot;, &quot;HGB&quot;,    &quot;Hemoglobin&quot;, 13.2,      &quot;g\/dL&quot;\n) %&gt;%\n  mutate(across(where(is.character), ~ str_sub(.x, 1, 200)))\n\nwrite_xpt(lb, file.path(out_dir, &quot;LB.xpt&quot;))\n\n\n#==============================================================================\n# run_all.R \u2014 batch driver (sits at the project root)\n#==============================================================================\nlibrary(tidyverse)\n\nsetwd(&quot;C:\/path\/to\/r_batch_xpt_example&quot;)\n\nscripts &lt;- c(\n  file.path(&quot;programs&quot;, &quot;01_make_dm_xpt.R&quot;),\n  file.path(&quot;programs&quot;, &quot;02_make_ae_xpt.R&quot;),\n  file.path(&quot;programs&quot;, &quot;03_make_lb_xpt.R&quot;)\n)\n\nlog_dir &lt;- file.path(&quot;output&quot;, &quot;logs&quot;)\ndir.create(log_dir, recursive = TRUE, showWarnings = FALSE)\n\nwalk(scripts, function(s) {\n  log_file &lt;- file.path(\n    log_dir,\n    paste0(tools::file_path_sans_ext(basename(s)), &quot;.log&quot;)\n  )\n  system2(\n    command = &quot;Rscript&quot;,\n    args    = s,\n    stdout  = log_file,\n    stderr  = log_file\n  )\n})\n\ncat(&quot;Batch run completed\\n&quot;)<\/code><\/pre>"},{"label":"R (base)","code":"#==============================================================================\n# run_all.R \u2014 batch driver (base R version, no tidyverse in the driver)\n#==============================================================================\n\nsetwd(\"C:\/path\/to\/r_batch_xpt_example\")\n\nscripts <- c(\n  file.path(\"programs\", \"01_make_dm_xpt.R\"),\n  file.path(\"programs\", \"02_make_ae_xpt.R\"),\n  file.path(\"programs\", \"03_make_lb_xpt.R\")\n)\n\nlog_dir <- file.path(\"output\", \"logs\")\ndir.create(log_dir, recursive = TRUE, showWarnings = FALSE)\n\nfor (s in scripts) {\n  log_file <- file.path(\n    log_dir,\n    paste0(tools::file_path_sans_ext(basename(s)), \".log\")\n  )\n  system2(\n    command = \"Rscript\",\n    args    = s,\n    stdout  = log_file,\n    stderr  = log_file\n  )\n}\n\ncat(\"Batch run completed\\n\")","language":"r","html":"<pre><code class=\"language-r\">#==============================================================================\n# run_all.R \u2014 batch driver (base R version, no tidyverse in the driver)\n#==============================================================================\n\nsetwd(&quot;C:\/path\/to\/r_batch_xpt_example&quot;)\n\nscripts &lt;- c(\n  file.path(&quot;programs&quot;, &quot;01_make_dm_xpt.R&quot;),\n  file.path(&quot;programs&quot;, &quot;02_make_ae_xpt.R&quot;),\n  file.path(&quot;programs&quot;, &quot;03_make_lb_xpt.R&quot;)\n)\n\nlog_dir &lt;- file.path(&quot;output&quot;, &quot;logs&quot;)\ndir.create(log_dir, recursive = TRUE, showWarnings = FALSE)\n\nfor (s in scripts) {\n  log_file &lt;- file.path(\n    log_dir,\n    paste0(tools::file_path_sans_ext(basename(s)), &quot;.log&quot;)\n  )\n  system2(\n    command = &quot;Rscript&quot;,\n    args    = s,\n    stdout  = log_file,\n    stderr  = log_file\n  )\n}\n\ncat(&quot;Batch run completed\\n&quot;)<\/code><\/pre>"}] || [];
