...
<title>mycsg SASnR2 1400_Subset-observations-and-variables</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Subset observations and variables</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Real analysis rarely uses a raw dataset as-is &mdash; we usually filter to the rows we care about AND pick just the columns we need in the same step.</li>
<li>SAS does both in one DATA step: a <code>WHERE</code> clause for rows, plus <code>keep</code> or <code>drop</code> for columns.</li>
<li>In tidyverse we chain <code>filter()</code> and <code>select()</code> with the pipe. In base R we combine a logical row index and a column index in a single <code>[</code> call.</li>
<li>This lesson keeps only the male subjects and drops the sex column in one step &mdash; a typical pattern when we subset to one treatment group and no longer need the grouping variable.</li>
<li>After running, we have a <code>males</code> dataset with only the male rows and no Sex column.</li>
...
<li>To filter rows and drop columns in a single pass, we combine <code>WHERE</code> and <code>drop</code> inside one DATA step.</li>
<li><code>set class;</code> reads the input. <code>where sex="M";</code> restricts the incoming rows. <code>drop sex;</code> removes the sex column from the output.</li>
<li>Order matters conceptually: the WHERE filter runs as rows are read, and the DROP removes variables from the output buffer. So the sex value is available for the WHERE test even though it does not appear in the final dataset.</li>
<li>We could equally write <code>keep name age height weight;</code> &mdash; the result is identical. <code>drop</code> is handier when there are only a few columns to remove; <code>keep</code> is clearer when there are only a few to keep.</li>
<li>Output <code>males</code> has the male rows from CLASS and every column except sex.</li>
...
males&lt;-class %&gt;% filter(Sex==&quot;M&quot;) %&gt;% select(-Sex)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In tidyverse we pipe the tibble through <code>filter()</code> and then <code>select()</code>.</li>
<li><code>class %&gt;% filter(Sex == "M") %&gt;% select(-Sex)</code> reads as three steps: start with class, keep rows where Sex is M, then drop the Sex column.</li>
<li>The pipe operator <code>%&gt;%</code> passes the output of each step as the first argument to the next, so we do not have to name intermediate tibbles.</li>
<li>The order of <code>filter()</code> and <code>select()</code> does not affect the result in this simple case, but filter-then-select is usually clearer because we focus on rows first and worry about columns afterwards.</li>
<li>The result is a tibble with only the male rows and no Sex column.</li>
...
males &lt;- males_tmp</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R we combine the row and column parts of the <code>[</code> subscript in one call.</li>
<li><code>class[class$Sex == "M", !names(class) %in% "Sex"]</code> does both jobs: the first slot is a logical row filter, the second is a logical column filter.</li>
<li>Alternatively, we can break it into two assignments &mdash; first <code>males &lt;- class[class$Sex == "M", ]</code> and then <code>males &lt;- NULL</code>. Setting a column to <code>NULL</code> is how base R deletes it in place.</li>
<li>The single-bracket form is more compact but the two-step version reads more naturally when the drop logic is non-trivial.</li>
<li>The result is a data frame with only the male rows and no Sex column.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data CLASS;\n infile datalines dlm='|' dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nCarol|F|14|62.8|102.5\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\n;;;;\n run;\n\ndata males;\n    set class;\n    where sex=\"M\";\n   drop sex;\n run;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data CLASS;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\nBarbara|F|13|65.3|98\nCarol|F|14|62.8|102.5\nHenry|M|14|63.5|102.5\nJames|M|12|57.3|83\n;;;;\n run;\n\ndata males;\n    set class;\n    where sex=&quot;M&quot;;\n   drop sex;\n run;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse) \r\n\r\nclass<-tribble( \r\n~Name,~Sex,~Age,~Height,~Weight, \r\n\"Alfred\",\"M\",14,69,112.5, \r\n\"Alice\",\"F\",13,56.5,84, \r\n\"Barbara\",\"F\",13,65.3,98, \r\n\"Carol\",\"F\",14,62.8,102.5, \r\n\"Henry\",\"M\",14,63.5,102.5, \r\n\"James\",\"M\",12,57.3,83, ) \r\n\r\nmales<-class %>% filter(Sex==\"M\") %>% select(-Sex)","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse) \r\n\r\nclass&lt;-tribble( \r\n~Name,~Sex,~Age,~Height,~Weight, \r\n&quot;Alfred&quot;,&quot;M&quot;,14,69,112.5, \r\n&quot;Alice&quot;,&quot;F&quot;,13,56.5,84, \r\n&quot;Barbara&quot;,&quot;F&quot;,13,65.3,98, \r\n&quot;Carol&quot;,&quot;F&quot;,14,62.8,102.5, \r\n&quot;Henry&quot;,&quot;M&quot;,14,63.5,102.5, \r\n&quot;James&quot;,&quot;M&quot;,12,57.3,83, ) \r\n\r\nmales&lt;-class %&gt;% filter(Sex==&quot;M&quot;) %&gt;% select(-Sex)<\/code><\/pre>"},{"label":"R (base)","code":"class <- data.frame(\r\n  Name = c(\"Alfred\", \"Alice\", \"Barbara\", \"Carol\", \"Henry\", \"James\"),\r\n  Sex = c(\"M\", \"F\", \"F\", \"F\", \"M\", \"M\"),\r\n  Age = c(14, 13, 13, 14, 14, 12),\r\n  Height = c(69, 56.5, 65.3, 62.8, 63.5, 57.3),\r\n  Weight = c(112.5, 84, 98, 102.5, 102.5, 83)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nmales_tmp <- class\r\n\r\nmales_tmp <- males_tmp[males_tmp$Sex==\"M\", ]\r\n\r\nmales_tmp <- males_tmp[, names(males_tmp) != \"Sex\"]\r\n\r\nmales <- males_tmp","language":"r","html":"<pre><code class=\"language-r\">class &lt;- data.frame(\r\n  Name = c(&quot;Alfred&quot;, &quot;Alice&quot;, &quot;Barbara&quot;, &quot;Carol&quot;, &quot;Henry&quot;, &quot;James&quot;),\r\n  Sex = c(&quot;M&quot;, &quot;F&quot;, &quot;F&quot;, &quot;F&quot;, &quot;M&quot;, &quot;M&quot;),\r\n  Age = c(14, 13, 13, 14, 14, 12),\r\n  Height = c(69, 56.5, 65.3, 62.8, 63.5, 57.3),\r\n  Weight = c(112.5, 84, 98, 102.5, 102.5, 83)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nmales_tmp &lt;- class\r\n\r\nmales_tmp &lt;- males_tmp[males_tmp$Sex==&quot;M&quot;, ]\r\n\r\nmales_tmp &lt;- males_tmp[, names(males_tmp) != &quot;Sex&quot;]\r\n\r\nmales &lt;- males_tmp<\/code><\/pre>"}] || [];
