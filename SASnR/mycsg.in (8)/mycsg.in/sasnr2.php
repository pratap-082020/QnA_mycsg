...
<title>mycsg SASnR2 1500_Renaming-variables</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Renaming variables</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>Renaming variables is a routine step &mdash; we might rename to meet an analysis spec, to match a standard like SDTM, or simply to make the column self-documenting.</li>
<li>SAS does this inside a DATA step with the <code>rename</code> statement (or as a dataset option).</li>
<li>In tidyverse we use <code>rename()</code> from dplyr. In base R we assign to <code>names()</code> directly.</li>
<li>This lesson renames the Age column of CLASS to age_in_years so we can compare the three approaches side by side.</li>
<li>After running, we should have a new dataset identical to CLASS except the Age column is now called age_in_years.</li>
...
<li>To rename variables in SAS, we write a DATA step that reads the input with <code>SET</code> and adds a <code>rename</code> statement listing <code>old = new</code> pairs.</li>
<li><code>rename age = age_in_years;</code> changes the column name in the output only &mdash; the source dataset is untouched.</li>
<li>We can rename several columns in one statement: <code>rename age = age_in_years height = height_cm;</code>.</li>
<li>An equally common form uses the dataset option: <code>set class (rename=(age = age_in_years));</code>. The option form is handy when we want to rename while still in the SET step and do not want a separate rename line.</li>
<li>After the DATA step, the output dataset has the same rows and the same other columns, with just the Age name replaced.</li>
...
renamed&lt;-rename(class,Age_years=Age, Height_in=Height)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>To rename columns in tidyverse, we use <code>rename()</code> from dplyr.</li>
<li>The syntax is <code>rename(data, new_name = old_name)</code> &mdash; note the new name goes on the left of the equals sign, opposite to SAS.</li>
<li>We can rename several columns in one call: <code>rename(class, age_in_years = Age, height_cm = Height)</code>.</li>
<li><code>rename()</code> returns a tibble with the updated column names; all other columns are unchanged.</li>
<li>For pattern-based renaming, dplyr offers <code>rename_with()</code> &mdash; for example <code>rename_with(class, tolower)</code> lowercases every column name at once.</li>
...
names(renamed)[names(renamed) == &#039;Height&#039;] &lt;- &#039;Height_in&#039;</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, we rename columns by assigning to <code>names()</code> &mdash; <code>names(df)[names(df) == "old"] <- "new"</code>.</li>
<li>This reads: find the slot in <code>names(df)</code> that equals "old" and overwrite it with "new". It works for single or multiple renames.</li>
<li>A shorter form when we know the position: <code>names(df)[3] <- "age_in_years"</code> &mdash; but relying on position is brittle if columns get reordered upstream.</li>
<li>For renaming many columns we build a named vector: <code>names(df)[match(c("Age", "Height"), names(df))] <- c("age_in_years", "height_cm")</code>.</li>
<li>The base R form is more verbose than <code>rename()</code> but needs no external packages.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"data CLASS;\n infile datalines dlm='|' dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\n;;;;\n run;\n\ndata renamed;\n     set class;\n     rename age=age_years height=height_in;\n run;","language":"sas","html":"<pre><code class=\"language-sas-mini\">data CLASS;\n infile datalines dlm=&#039;|&#039; dsd missover;\ninput Name : $8. Sex : $1. Age : best32. Height : best32. Weight : best32.;\nlabel ;\n format ;\n datalines4;\nAlfred|M|14|69|112.5\nAlice|F|13|56.5|84\n;;;;\n run;\n\ndata renamed;\n     set class;\n     rename age=age_years height=height_in;\n run;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"class<-tribble(\r\n~Name,~Sex,~Age,~Height,~Weight,\r\n\"Alfred\",\"M\",14,69,112.5,\r\n\"Alice\",\"F\",13,56.5,84,\r\n)\r\n\r\n\u00a0\r\n\r\nrenamed<-rename(class,Age_years=Age, Height_in=Height)","language":"r","html":"<pre><code class=\"language-r\">class&lt;-tribble(\r\n~Name,~Sex,~Age,~Height,~Weight,\r\n&quot;Alfred&quot;,&quot;M&quot;,14,69,112.5,\r\n&quot;Alice&quot;,&quot;F&quot;,13,56.5,84,\r\n)\r\n\r\n\u00a0\r\n\r\nrenamed&lt;-rename(class,Age_years=Age, Height_in=Height)<\/code><\/pre>"},{"label":"R (base)","code":"class <- data.frame(\r\n  Name = c(\"Alfred\", \"Alice\"),\r\n  Sex = c(\"M\", \"F\"),\r\n  Age = c(14, 13),\r\n  Height = c(69, 56.5),\r\n  Weight = c(112.5, 84)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nrenamed <- class\r\n\r\nnames(renamed)[names(renamed) == 'Age'] <- 'Age_years'\r\n\r\nnames(renamed)[names(renamed) == 'Height'] <- 'Height_in'","language":"r","html":"<pre><code class=\"language-r\">class &lt;- data.frame(\r\n  Name = c(&quot;Alfred&quot;, &quot;Alice&quot;),\r\n  Sex = c(&quot;M&quot;, &quot;F&quot;),\r\n  Age = c(14, 13),\r\n  Height = c(69, 56.5),\r\n  Weight = c(112.5, 84)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nrenamed &lt;- class\r\n\r\nnames(renamed)[names(renamed) == &#039;Age&#039;] &lt;- &#039;Age_years&#039;\r\n\r\nnames(renamed)[names(renamed) == &#039;Height&#039;] &lt;- &#039;Height_in&#039;<\/code><\/pre>"}] || [];
