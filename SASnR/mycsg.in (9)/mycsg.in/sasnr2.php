...
<title>mycsg SASnR2 1501_validvarname_upcase</title>
...
</script><div class='csg-adwrap' id='sasnr-lesson-wrap'><div class='csg-center-col'><center><h2>Convert all the variable names to uppercase</h2></center><hr><div class="card mb-3 code-card lang-default">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-default" data-toggle="collapse" data-target="#lessonDesc" style="cursor:pointer;" role="button" aria-expanded="true" aria-controls="lessonDesc">    <h6 class="mb-0">Lesson Description</h6>    <span class="toggle-indicator" data-target="lessonDesc">-</span>  </div>  <div id="lessonDesc" class="collapse show">    <div class="card-body variant-description"><ul>
<li>In clinical programming, variable names are often ALL CAPS to match SDTM and ADaM conventions (USUBJID, STUDYID, ADSL, and so on).</li>
<li>When we import data from CSV, Excel or an external vendor, the names rarely arrive uppercase &mdash; we have to force them ourselves.</li>
<li>SAS gives us the <code>options validvarname=upcase;</code> global setting that makes every variable name uppercase from the point it is set.</li>
<li>R has no global option like this, but <code>rename_with(toupper)</code> (tidyverse) or a direct assignment to <code>names()</code> (base R) does the same job at the dataset level.</li>
<li>This lesson shows how to normalise all column names to uppercase after reading in a dataset.</li>
<li>After running, every column name in the output is ALL CAPS.</li>
</ul>    </div>  </div></div><div id="codeAccordion" class="mb-4"><div class="card mb-2 code-card lang-sas">  <div class="card-header p-2 d-flex justify-content-between align-items-center accordion-header lang-sas" data-toggle="collapse" data-target="#collapse0" style="cursor:pointer;" role="button" aria-expanded="false" aria-controls="collapse0">    <h6 class="mb-0"><span class="badge badge-primary mr-2">SAS</span> (Base SAS)</h6>    <div class="d-flex align-items-center">      <button type="button" class="btn btn-light btn-sm copy-code-btn mr-2" data-target-code="code-block-collapse0">Copy</button>      <span class="toggle-indicator" data-target="collapse0">+</span>    </div>  </div>  <div id="collapse0" class="collapse">    <div class="card-body">      <div class="variant-code" id="code-block-collapse0"><pre><code class="language-sas-mini">options validvarname=upcase;
...
<li>To force SAS to uppercase every variable name, we set <code>options validvarname=upcase;</code> before the DATA step.</li>
<li><code>validvarname</code> controls how SAS treats variable names it creates or reads. The <code>upcase</code> value means &ldquo;if a name arrives in mixed case, store it in upper case&rdquo;.</li>
<li>After the option is set, any dataset we create (including via <code>proc import</code> or a DATA step reading external files) has uppercase variable names by default.</li>
<li>The option is global &mdash; it stays in force for the rest of the session unless we reset it. For a one-off, wrap it with <code>options validvarname=v7;</code> at the end to restore default behaviour.</li>
<li>After running, a <code>proc contents</code> confirms every variable name is uppercase.</li>
...
class_upper &lt;- class %&gt;%
rename_with(toupper)</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In tidyverse, we uppercase every column name with a single call to <code>rename_with()</code>.</li>
<li><code>rename_with(data, toupper)</code> applies the <code>toupper</code> function to each column name and replaces the old name with the result.</li>
<li>This is the tidyverse analogue of the SAS <code>validvarname=upcase</code> option, applied per-dataset rather than globally.</li>
<li>The same helper works for other transformations: <code>tolower</code> for lower case, <code>make.names</code> for valid R identifiers, or a custom function for complex renames.</li>
<li>After running, <code>names(class)</code> prints every column name in upper case.</li>
...
class_upper &lt;- class
names(class_upper) &lt;- toupper(names(class_upper))</code></pre>      </div><div class="variant-description mt-2"><ul>
<li>In base R, we uppercase every column name by assigning <code>toupper(names(df))</code> back to <code>names(df)</code>.</li>
<li><code>names(df) <- toupper(names(df))</code> reads the current names, upper-cases them as a vector, and writes them back in one step.</li>
<li>Same idea as the tidyverse version, just without the helper &mdash; <code>toupper</code> is base R, and assigning to <code>names()</code> is the standard way to rename columns in place.</li>
<li>For lower case we would swap <code>toupper</code> with <code>tolower</code>.</li>
<li>After running, <code>names(df)</code> shows all column names in upper case.</li>
</ul></div>    </div>  </div></div></div></div></div><script>
    const variants = [{"label":"SAS (Base SAS)","code":"options validvarname=upcase;\n\ndata class;\n    input name $ sex $ age height weight;\n    datalines;\nAlfred M 14 69 112.5\nAlice F 13 56.5 84\n;\nrun;","language":"sas","html":"<pre><code class=\"language-sas-mini\">options validvarname=upcase;\n\ndata class;\n    input name $ sex $ age height weight;\n    datalines;\nAlfred M 14 69 112.5\nAlice F 13 56.5 84\n;\nrun;<\/code><\/pre>"},{"label":"R (tidyverse)","code":"library(tidyverse)\r\n\r\nclass <- tribble(\r\n~name, ~sex, ~age, ~height, ~weight,\r\n\"Alfred\", \"M\", 14, 69, 112.5,\r\n\"Alice\", \"F\", 13, 56.5, 84\r\n)\r\n\r\n \r\n#Convert column names to uppercase\r\n\r\nclass_upper <- class %>%\r\nrename_with(toupper)","language":"r","html":"<pre><code class=\"language-r\">library(tidyverse)\r\n\r\nclass &lt;- tribble(\r\n~name, ~sex, ~age, ~height, ~weight,\r\n&quot;Alfred&quot;, &quot;M&quot;, 14, 69, 112.5,\r\n&quot;Alice&quot;, &quot;F&quot;, 13, 56.5, 84\r\n)\r\n\r\n \r\n#Convert column names to uppercase\r\n\r\nclass_upper &lt;- class %&gt;%\r\nrename_with(toupper)<\/code><\/pre>"},{"label":"R (base)","code":"class <- data.frame(\r\n  name = c(\"Alfred\", \"Alice\"),\r\n  sex = c(\"M\", \"F\"),\r\n  age = c(14, 13),\r\n  height = c(69, 56.5),\r\n  weight = c(112.5, 84)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nclass_upper <- class\r\nnames(class_upper) <- toupper(names(class_upper))","language":"r","html":"<pre><code class=\"language-r\">class &lt;- data.frame(\r\n  name = c(&quot;Alfred&quot;, &quot;Alice&quot;),\r\n  sex = c(&quot;M&quot;, &quot;F&quot;),\r\n  age = c(14, 13),\r\n  height = c(69, 56.5),\r\n  weight = c(112.5, 84)\r\n  , stringsAsFactors = FALSE\r\n)\r\n\r\nclass_upper &lt;- class\r\nnames(class_upper) &lt;- toupper(names(class_upper))<\/code><\/pre>"}] || [];
