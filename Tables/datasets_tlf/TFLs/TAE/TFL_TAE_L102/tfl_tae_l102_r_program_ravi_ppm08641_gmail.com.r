...
#==============================================================================;
#Create numeric version of severity variable;
#==============================================================================;
...
temp02<-temp01 %>%
 filter(trtemfl=="Y") %>%
 arrange(usubjid,aebodsys,asevn,aeseq) %>%
...
temp03<-temp02 %>%
 filter(trtemfl=="Y") %>%
 arrange(usubjid,aebodsys,aedecod,asevn,aeseq) %>%
...
#==============================================================================;
#Replicate the rows for total severity;
#==============================================================================;
 
adae04<-adae03 %>%
 bind_rows(adae03 %>% mutate(asevn=100))
...
soc_count <- adae04 %>%
 filter(aoccsifl=="Y") %>%
 group_by(aebodsys, treatment,asevn) %>%
...
pt_count <- adae04 %>%
 filter(aoccpifl=="Y") %>%
 group_by(aebodsys, aedecod, treatment,asevn) %>%
 summarise(count = n_distinct(usubjid)) %>%
 ungroup()
...
dummy02<-dummy01 %>%
 cross_join(tibble(asevn=c(1,2,3,100)))
...
counts02<-dummy02 %>%
 left_join(counts01,by=c("label","aebodsys", "aedecod", "treatment","asevn")) %>%
...
trans01 <- counts05 %>%
 pivot_wider(
 id_cols = c(aebodsys, aedecod, label,asevn),
 names_from = treatment,
 values_from = cp,
 names_prefix = "trt"
 ) %>%
 arrange(aebodsys,aedecod,label,asevn)
...
trans02 <- trans01 %>%
 ...
 select(aebodsys, aedecod, asevn, label,severity, trt0, trt54, trt81, label)
