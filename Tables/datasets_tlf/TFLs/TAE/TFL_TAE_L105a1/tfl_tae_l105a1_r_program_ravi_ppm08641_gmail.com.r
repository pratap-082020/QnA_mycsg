...
adae02<-adae01 %>% mutate(treatment=trtan) %>%
 bind_rows(adae01 %>% mutate(treatment=4))
...
trans02<-trans01 %>%
 ...
 cnt4=as.numeric(word(trt4,1,sep=fixed("("))),
 cnt3=as.numeric(word(trt3,1,sep=fixed("("))))
 
trans03<-trans02 %>%
 arrange(section,desc(cnt4),desc(cnt3),aedecod) %>%
 group_by(section) %>%
 mutate(order=row_number()) %>%
 ungroup()
 
 
output<-trans03 %>% select(aedecod,label,trt1,trt2,trt3,trt4)
