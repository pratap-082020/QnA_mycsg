...
dummy_trttotals <- tibble(treatment = c(0,54,81))
...
events01_pre <- counts05 %>%
 filter(treatment %in% c(0,54))
 
 
# Filter distinct aedecod-label pairs with sum(count) > 0
nonzerodecods <- events01_pre %>%
 group_by(aedecod, label) %>%
 filter(sum(count) > 0) %>%
 distinct(aedecod,label)
 
# Join events02 with nonzerodecods
events01 <- events01_pre %>%
 inner_join(nonzerodecods, by = c("aedecod", "label"))
...
output<-trans02 %>% select(aedecod,trt0,trt54,trt81, pval)
