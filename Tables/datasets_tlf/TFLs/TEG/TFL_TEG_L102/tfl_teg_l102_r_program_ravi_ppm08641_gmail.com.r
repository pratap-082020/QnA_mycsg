...
adeg03 <- adeg02 %>%
 mutate(
 dp = case_when(
 paramcd %in% c("EGHRMN", "PR", "QR", "QRS", "QTCB", "QTCF", "RR", "QT") ~ 0,
 TRUE ~ NA
 )
 )
...
adeg04<-adeg03 %>%
 mutate(group=1,result=aval) %>%
 bind_rows(adeg03 %>%
 filter(!is.na(chg)) %>%
 mutate(group=2,result=chg))
...
aval_stats<-stats03 %>%
 filter(group==1) %>%
 rename(c2=n,c3=mean,c4=std,c5=min,c6=median,c7=max) %>%
 select(-group,-dp)
...
chg_stats<-stats03 %>%
 filter(group==2) %>%
 rename(c8=n,c9=mean,c10=std,c11=min,c12=median,c13=max) %>%
 select(-group,-dp)
...
stats07<-stats06 %>%
 select(paramn,param,treatment,treatmentc,avisitn,avisit,starts_with("c"))
