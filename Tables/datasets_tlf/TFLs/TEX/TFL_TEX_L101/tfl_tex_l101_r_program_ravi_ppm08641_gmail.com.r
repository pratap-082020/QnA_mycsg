...
numeric01<-adsl02 %>% mutate(vargroup=2,result=trdurn,dp=0) %>%
 bind_rows(adsl02 %>% mutate(vargroup=3,result=trcpl,dp=1))
...
stats05 <- stats04 %>%
 ...
 vargroup==2 ~ "Duration (days)",
 vargroup==3 ~ "Treatment Compliance (%)",
 TRUE ~ ""
 ),
 name = toupper(name),
 intord = case_when(
...
categorical01<-adsl02 %>%
 mutate(group=1,statistic=if_else(is.na(trdgr1)|trdgr1=="","Missing",trdgr1))
...
dummy01<-tribble(
 ...
 1,"Duration (days) ",1,"<7",
 1,"Duration (days) ",2,">=7 to <14",
 1,"Duration (days) ",3,">=14 to <28",
 1,"Duration (days) ",4,">=28 to <42",
 1,"Duration (days) ",5,">=42",
 1,"Duration (days) ",99,"Missing",
)
