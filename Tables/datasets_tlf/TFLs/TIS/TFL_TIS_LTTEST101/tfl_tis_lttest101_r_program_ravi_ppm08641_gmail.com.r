...
source("./TFL_TIS_LTTEST101_data.r")
...
bmi01 <- advs %>%
 filter(fasfl=="Y", paramcd=="BMI", avisit=="Month 1") %>%
 mutate(group=1, dp=2, treatment=trt01pn, result=aval)
...
ttest_res <- t.test(bmi01$result, mu = 28.4)
...
pval01 <- ttest %>%
 ...
 intord=6,
 statistic="p-value",
 grouplabel="Body Mass Index (kg/m2)",
 trt_1=if_else(p_value < 0.0001, "<0.0001", sprintf("%0.4f", p_value))) %>%
 select(group, grouplabel,intord, trt_1, statistic, trt_1)
