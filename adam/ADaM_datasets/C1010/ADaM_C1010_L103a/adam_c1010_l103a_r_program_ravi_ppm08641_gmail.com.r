...
source("./ADaM_C1010_L103a_data.r")
...
vs02_base<-vs02 %>% filter(avisitn <= 0)
vs02_postbase<-vs02 %>% filter(avisitn >0)
...
template03 <- full_join(template02, vs02_postbase, by = c("usubjid", "paramn", "avisitn", "avisit")) %>%
...
vs03 <- left_join(template03, vs02_postbase %>% select(-avisitn, -avisit, -inb),
 by = c("usubjid", "paramn", "seq"))  %>%
 mutate(dtype = if_else(is.na(inb) & !is.na(aval), "LOCF", ""))
 
vs04<-vs03 %>%
 filter(!is.na(inb) | !is.na(seq))
 
#==============================================================================;
#Append baseline data and template data;
#==============================================================================;
 
vs05<-bind_rows(vs04,vs02_base) %>%
